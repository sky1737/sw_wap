...
unset($_SESSION['user']);
unset($_SERVER['openid']);
unset($_SERVER['oauthed']);
redirect('./');
//if (IS_POST) {
...
//
//			$save_user_data =
//				array('login_count' => $get_result['user']['login_count'] + 1, 'last_time' => $_SERVER['REQUEST_TIME'],
...
//			if ($database_user->field('`uid`')->where(array('phone' => $_POST['phone']))->find()) json_return(1014,
//				'手机号码已存在');
//			$data = array();
...
//
////回调地址
//$redirect_uri = $_GET['referer']
//	? $_GET['referer']
//	: ($_SERVER['HTTP_REFERER']
//		? $_SERVER['HTTP_REFERER']
//		: ($_COOKIE['wap_store_id']
//			? './home.php?id=' . $_COOKIE['wap_store_id']
//			: $config['site_url']));
//
