...
if(empty($action)){
	...
	$cartList = D('')->field('`s`.`store_id`,`s`.`name` AS `store_name`,`uc`.`id`,`uc`.`product_id`,`uc`.`pro_num`,`uc`.`pro_price`,`uc`.`sku_id`,`uc`.`sku_data`,`p`.`name`,`p`.`image`,`p`.`quantity`,`p`.`status`')->table(array('User_cart'=>'uc','Product'=>'p','Store'=>'s'))->where($cart_where)->order('`id` DESC')->select();
...
//	//分享配置 end
	...
	foreach($_POST['ids'] as $value){
		$now_cart = D('')->field('`uc`.*,`p`.`quantity`,`p`.`status`')->table(array('User_cart'=>'uc','Product'=>'p'))->where($cart_where." AND `uc`.`id`='$value'")->find();
		if(empty($now_cart)){
