...
if($nowProduct['image_size'] == '0') {
	...
	D('Product')->where(array('product_id' => $product_id))
...
//		$tmp_product_info =
//			D('Product')->field('store_id')->where(array('product_id' => $nowProduct['original_product_id']))->find();
...
$now_url = 'http://' . $_SERVER['HTTP_HOST'] . '/wap/good.php?id=' . $nowProduct['product_id'];
...
if(!empty($nowProduct['buyer_quota'])) {
	if(empty($_SESSION['user'])) { //游客购买
		...
		$buy_quantity = 0;
		//购物车
		$carts = D('User_cart')->field('pro_num')->where(array('product_id' => $nowProduct['product_id'],
		...
		if(!empty($orders)) {
			foreach ($orders as $order) {
				$products = D('Order_product')->field('pro_num')->where(array('product_id' => $nowProduct['product_id'],
		...
		//购物车
		$carts = D('User_cart')->field('pro_num')->where(array('product_id' => $nowProduct['product_id'],
		                                                       'store_id'   => $store_id, 'uid' => $uid))->select();
		if(!empty($carts)) {
			foreach ($carts as $cart) {
				$buy_quantity += $cart['pro_num'];
			}
...
//	$allow_drp = true;
//	if ($_SESSION['user']['uid'] != $now_store['uid']) {
//		$uid = $_SESSION['user']['uid'];
//		//查看是否已经是当前店铺的分销商
//		$seller_store =
//			D('Store')->field('store_id')->where(array('uid' => $uid, 'drp_supplier_id' => $store_id))->find();
//		if (!empty($seller_store)) {
//			//查看是否已经分销过此商品
//			$fx_product =
//				D('Product')->where(array('source_product_id' => $nowProduct['product_id'], 'supplier_id' => $store_id,
//				                          'store_id'          => $seller_store['store_id']))->find();
//			if (!empty($fx_product)) {
...
//		$drp_register_url =
//			'./drp_register.php?id=' . $store_id . '&product_id=' . $nowProduct['product_id'] . '&a=share';
//	}
//}
...
$share_conf = array(
	...
	'desc'    => str_replace(array("\r", "\n"), array('', ''),
		!empty($nowProduct['intro']) ? $nowProduct['intro'] : $nowProduct['name']), // 分享描述
