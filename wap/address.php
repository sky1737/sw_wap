...
switch ($action) {
	case 'add':
		if (IS_POST) {
			...
			$data_user_address['province'] =
				!empty($_POST['province']) ? $_POST['province'] : json_return(1002, '请选择地区');
	...
	case 'postage':
		if (IS_POST) {
			...
			if (!empty($address_id)) {
				$nowAddress =
					D('User_address')->field('`province`')->where(array('address_id' => $_POST['address_id']))->find();
			...
			if (empty($nowOrder['address'])) {
				...
				foreach ($nowOrder['proList'] as $key => $product) {
					...
					if ($product['postage_template_id']) {
						$postage_template =
							$postage_template_model->get_tpl($product['postage_template_id'], $product['supplier_id']);
						...
						foreach ($postage_template['area_list'] as $area) {
							...
							if (in_array($nowAddress['province'], explode('&', $area[0]))) {
								if (isset($has_tpl_postage_arr[$product['supplier_id'] . '_' .
									$product['protage_template_id']])) {
									$has_tpl_postage_arr[$product['supplier_id'] . '_' .
									$product['postage_template_id']]['weight'] += $product['pro_num'] *
										$product['pro_weight'];
								}
								else {
									$has_tpl_postage_arr[$product['supplier_id'] . '_' .
									$product['postage_template_id']]['weight'] =
										$product['pro_num'] * $product['pro_weight'];
									$has_tpl_postage_arr[$product['supplier_id'] . '_' .
									$product['postage_template_id']]['area'] = $area;
								}
				...
				foreach ($has_tpl_postage_arr as $key => $postage_detail) {
					...
					if ($postage_detail['weight'] > $postage_detail['area']['1'] && $postage_detail['area'][3] > 0 &&
						$postage_detail['area'][4] > 0
					) {
						$supplier_postage_arr[$supplier_id] +=
							ceil(($postage_detail['weight'] - $postage_detail['area']['1']) /
								$postage_detail['area'][3]) * $postage_detail['area']['4'];
						$postageCount += ceil(($postage_detail['weight'] - $postage_detail['area']['1']) /
								$postage_detail['area'][3]) * $postage_detail['area']['4'];
					}
				}
						...
						if (!empty($postage_template['area_list'])) {
							...
							foreach ($postage_template['area_list'] as $v) {
								if (in_array($nowAddress['province'], explode('&', $v[0]))) {
									if (!empty($v[3]) && $v[3] > 0) {
										$postage_arr[$value['supplier_id']][] = $value['pro_num'] > $v[1] ?
											$v[2] + floor(($value['pro_num'] - $v[1]) / $v[3]) * $v[4] : $v[2];
									}
									else {
				...
				if (!empty($postage_details)) {
					foreach ($postage_details as $supplier_id => $postage_detail) {
						...
						$fx_postage[$supplier_id] =
							($hasTplPostage ? array_sum($postage_detail) : min($postage_detail));
					}
