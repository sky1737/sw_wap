...
if (!$is_mobile && $_SESSION['user'] && option('config.synthesize_store')) {
	if (isset($_GET['ps']) && $_GET['ps'] == '800') {
		...
		$url = 'http://' . $store_id . $config['site_domain'];
...
$drp_register_url = '';
$seller_name = ''; //分销商
if (!empty($_SESSION['user']['uid']) && !empty($now_store['open_drp_guidance']) &&
	$_SESSION['user']['uid'] != $now_store['uid']
) {
	$user = M('User');
	...
	if ($allow_drp) {
		...
		if ($now_store['uid'] != $_SESSION['user']['uid']) { //他人店铺
			//判断是否已经是分销商
			$seller_store =
				D('Store')->where(array('drp_supplier_id' => $store_id, 'uid' => $_SESSION['user']['uid'],
				                        'status'          => 1))->find();
			$seller = $store_supplier->getSeller(array('seller_id' => $store_id, 'type' => 1));
			if (!empty($seller_store)) {
				$seller_info =
					D('Store')->field('store_id,name')->where(array('store_id' => $seller_store['store_id']))->find();
				$seller_name = $seller_info['name'];
				$is_seller = true;
				$drp_notice = true;
				$drp_register_url = './drp_register.php?id=' . $seller_store['store_id'];
			}
			else if (((!empty($seller) && $seller['level'] <= $max_store_drp_level) || empty($seller)) &&
				empty($seller_store)
			) { //在最大分销级别内
				$drp_notice = true;
		...
		if (!empty($now_store['open_drp_limit']) && !empty($now_store['drp_limit_buy'])) {
			$msg = '亲爱的 <span style="color:#26CB40">' . $_SESSION['user']['nickname'] .
				'</span>，在本店消费满 <span style="color:red">' . $now_store['drp_limit_buy'] . '</span> 元，即可申请分销！';
		}
		else {
			$msg = '亲爱的 <span style="color:#26CB40">' . $_SESSION['user']['nickname'] . '</span>，申请分销即可分销赚佣金！';
		}
	}
}
