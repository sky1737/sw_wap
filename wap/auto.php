...
$order_id = $order_model->db->where(array('status' => 3, 'sent_time' => array('>=', $time)))->order('sent_time asc')->getField('order_id');
if ($order_id) {
...
$order_id = $order_model->db->where(array('status' => array('<=',1), 'add_time' => array('>=', $time)))->order('sent_time asc')->getField('order_id');
if ($order_id) {
    $order_model->cancelOrder($nowOrder);
}

echo '最后执行时间:' . date('Y-m-d H:i:s', time());
echo '<script type="text/javascript">setTimeout(function(){location.reload();},300000);</script>';
