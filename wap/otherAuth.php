...
switch ($_GET['source']) {
	case 'pigcms':
		...
		$return['token'] = $token;
		$return['sex'] = $_SESSION['user']['sex'];
		$return['issub'] = isSub($_SESSION['openid']);
		$return['portrait'] = $_SESSION['user']['avatar'];
		$return['wechaname'] = $return['truename'] = $_SESSION['user']['nickname'];
		$return['province'] = $_SESSION['user']['province'];
		$return['city'] = $_SESSION['user']['city'];
		$postData = array(
			'option' => array(
				'where' => array(
		...
		if ($result['status'] == 0) {
			header('Location:' . $apiUrl . 'index.php?g=Home&m=Auth&a=oauth2&token=' . $token . '&wechat_id=' .
				$return['wecha_id']);
		}
		else {
...
{
	import('WechatApi');
	$tokenObj = new WechatApi(array('appid'     => option('config.wechat_appid'),
	                                'appsecret' => option('config.wechat_appsecret')));
	$access_token = $tokenObj->get_access_token();
	$url = 'https://api.weixin.qq.com/cgi-bin/user/info?openid=' . $openid . '&access_token=' .
