...
$payClass = new Weixin(array(), $payMethodList[$payType]['config'], '', 'recharge');
$payInfo = $payClass->notice();
...
{
	if($payInfo['err_code'] === 0) {
		$database_order = D('Recharge_order');
		...
		if($nowOrder && $nowOrder['status'] == 1) {
			...
			if($database_order->where($condition_order)->data($data_order)->save()) {
				// 充值+充值记录
				M('User')->recharge($nowOrder['uid'], $nowOrder['total'], $nowOrder['point']);

				// 充值立返，往上三级
				M('User')->rechargePromoter($nowOrder['uid'], $nowOrder['order_no'], $nowOrder['profit'], 0);
