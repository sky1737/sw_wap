...
switch ($action) {
    ...
    case 'get_product_list':
        ...
        $condition['status'] = '1';
        //$condition['is_fx'] = '1';
        $condition['quantity'] = array('>=', '1');

        $condition['category_id'] = array('in',$catids);
        ...
        $json_return['list'] = D('Product')
            ->field('`product_id`,`name`,`image`,`price`,`market_price`,`cost_price`,`sales`,`discount`,`store_id`')
            ->where($condition)
            ->order($sort_by)
            ->limit((($page - 1) * 10) . ',10')
            ->select();
    ...
    case 'nearshops':
        ...
        import('Http');
        $http_class = new Http();
        $callback = $http_class->curlGet('http://api.map.baidu.com/geoconv/v1/?coords=' . $long . ',' . $lat .
            '&from=1&to=5&ak=4c1bb2055e24296bbaef36574877b4e2');
        ...
        $store_list = D('')->table(array('Store_contact' => 'sc', 'Store' => 's'))
            ->field("`s`.`store_id`,`s`.`name`,`s`.`intro`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli")
            ->where("`sc`.`store_id`=`s`.`store_id` AND `s`.`status`='1'")->order("`juli` ASC")->limit(12)->select();
        ...
        foreach ($store_list as &$value) {
            $value['url'] = $config['wap_site_url'] . '/?twid=' . $value['store_id'] . '&platform=1';
            //$condition_product['store_id'] = $value['store_id'];
            $value['product_list'] =
                $database_product->field('`product_id`,`name`,`image`,`price`')->where($condition_product)
                    ->order('`product_id` DESC')->limit(3)->select();
            foreach ($value['product_list'] as &$product_value) {
                ...
                $product_value['url'] =
                    $config['wap_site_url'] . '/good.php?id=' . $product_value['product_id'] . '&twid=' .
                    $now_store['store_id'] . '&platform=1';
            }
        }
    ...
    case 'butt_nearshops':
        ...
        $store_list = D('')->table(array('Store_contact' => 'sc', 'Store' => 's', 'User' => 'u'))
            ->field("`s`.`store_id`,`s`.`name`,`s`.`intro`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli")
            ->where("`sc`.`store_id`=`s`.`store_id` AND `s`.`status`='1' AND `u`.`uid`=`s`.`uid` AND `u`.`source_site_url`='$domain'")
            ->order("`juli` ASC")->limit(20)->select();
        foreach ($store_list as $key => $value) {
            if($value['juli'] > 20000) {
                unset($store_list[$key]);
            }
    ...
    case 'butt_nearshops_search':
        ...
        $database_store_contact = D('Store_contact');
        $store_list = D('')->table(array('Store_contact' => 'sc', 'Store' => 's', 'User' => 'u'))
            ->field("`s`.`store_id`,`s`.`name`,`s`.`intro`,`sc`.`long`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli")
            ->where("`sc`.`store_id`=`s`.`store_id` AND `s`.`status`='1' AND `u`.`uid`=`s`.`uid` AND `u`.`source_site_url`='$domain' AND `s`.`name` LIKE '%$keyword%'")
            ->order("`juli` ASC")->limit(20)->select();
        $database_product = D('Product');
        $condition_product['status'] = 1;
        foreach ($store_list as &$value) {
            ...
            foreach ($value['product_list'] as &$product_value) {
                $product_value['price'] = floatval($product_value['price']);
                $product_value['image'] = getAttachmentUrl($product_value['image']);
                $product_value['url'] = $config['wap_site_url'] . '/good.php?id=' .
                    $product_value['product_id'] . '&twid=' . $store_session['store_id'];
            }
        }
        json_return(0, $store_list);
        break;
    ...
    case 'nearstore':
        ...
        $database_store_contact = D('Store_contact');
        $store_list = D('')->table(array('Store_contact' => 'sc', 'Store' => 's'))
            ->field("`s`.`store_id`,`s`.`name`,`s`.`logo`,`s`.`intro`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli")
            ->where("`sc`.`store_id`=`s`.`store_id` AND `s`.`status`='1'")->order("`juli` ASC")->limit(4)->select();
        foreach ($store_list as $key => $value) {
            if($value['juli'] > 200000) {
                ...
                $store_list[$key]['url'] =
                    $config['wap_site_url'] . '/?twid=' . $value['store_id'] . '&platform=1';
                if(empty($value['logo'])) {
    ...
    case 'neargoods':
        ...
        $goods_list = D('')->table("Product as p")
            ...
            ->where("`p`.`status`='1' AND p.is_fx = 1")// and `p`.`is_recommend` = 1
            ->group('p.store_id')
            ->order("`juli` asc")
            ->limit(4)
            ->select();
        ...
        foreach ($goods_list as $key => &$value) {
            /*if($value['juli'] > 200000){
                ...
                if ($value['price'] > $value['market_price']) {
                    ...
                    $value['youhui_price'] = $value['market_price'] - $value['price'];
                }
            }
            */
            $goods_list[$key]['url'] = './good.php?id=' . $value['product_id'] . '&twid=' . $now_store['store_id'];
            $goods_list[$key]['image'] = getAttachmentUrl($value['image']);
