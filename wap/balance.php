...
if ($_GET['a'] == 'statistics')
{
    ...
    if (IS_POST)
    {
        ...
        else if (strtolower($type) == 'yesterday')
        { //昨日佣金
...
else if ($_GET['a'] == 'exchange')
{
    if (IS_POST)
    {
        ...
        if ($data['point'] < option('config.point_exchange') * 1.00 || ($data['point'] % option('config.point_exchange') != 0))
        {
    ...
    $wap_user = D('User')->where(array('uid' => $wap_user['uid'], 'status' => 1))->find();
    if ($_SESSION['user'] != $wap_user) $_SESSION['user'] = $wap_user;
    ...
    if (IS_POST)
    {
        ...
        if ($user_cash->data($data)->add())
        {
            ...
            else
            {
                ...
                $user_cash->where(array(
                    'uid' => $wap_user['uid'], 'trade_no' => $data['trade_no']))->data(array(
                    'status' => 1, 'complate_time' => time()))->save();
...
else if ($_GET['a'] == 'applywithdrawal')
{
    // 提现申请页面
    if (IS_POST)
    {
        ...
        if (!$data['bank_id'] || !$data['card_id'])
        {
            json_return(1002, '请先添加银行卡信息！');
        }

        $card = M('User_card')->getCard($wap_user['uid'], $data['card_id']);
        if ($card)
        {
            $bank = M('Bank')->getBank($card['bank_id']);
            if (!$bank)
            {
                json_return(1000, '银行信息不正确！');
            }
            $card['bank'] = $bank['name'];
        }
        else
        {
            json_return(1000, '银行卡八信息不正确！');
        }
        ...
        //$store_info = $store->getStore($now_store['store_id']);

        $data['trade_no'] = date('YmdHis', $_SERVER['REQUEST_TIME']) . mt_rand(100000, 999999);
        $data['uid'] = $wap_user['uid'];
        $data['store_id'] = $now_store['store_id'];

        $data['opening_bank'] = $card['bank_name'];
        $data['bank_card'] = $card['card_no'];
        $data['bank_card_user'] = $card['card_user'];
    ...
    include display('balance_applywithdrawal');
    echo ob_get_clean();
}
else if ($_GET['a'] == 'withdraw_account')
{ // 提现账号
    $db = M('User_card');
    if (IS_POST)
    {
        ...
        if (!$card_id)
        {
            ...
            {
                $db->add(array(
                    'uid' => $wap_user['uid'], 'bank_id' => $bank_id, 'bank_name' => $bank_name, 'card_no' => $card_no,
                    'card_user' => $card_user, 'add_time' => time()));
                json_return(0, '添加银行卡成功！');
            }
            $card_id = $card['card_id'];
        }

        $db->save(array('uid' => $wap_user['uid'], 'card_id' => $card_id), array(
            'bank_id' => $bank_id, 'bank_name' => $bank_name, 'card_no' => $card_no, 'card_user' => card_user));
        json_return(0, '保存成功！');
    }
...
{ //佣金明细
    if (IS_AJAX && $_POST['type'] == 'brokeragetab')
    {
        ...
        $list = $user_income->getRecords(array('uid' => $wap_user['uid']), '`income_id` desc', $page->firstRow, $page->listRows);
        $html = '';
        foreach ($list as $item)
        {
            //SELECT `income_id`, `uid`, `order_id`, `income`, `point`, `type`, `add_time`, `status`, `remarks` FROM `tp_user_income` WHERE 1
            $html .= '<tr>';
            $html .= '<td style="text-align: left"><font style="color:#f00;">' . $user_income->typeTxt($item['type']) . '</font><br/>' . (empty($item['order_no']) ? '' : '订单号：' . $item['order_no'] . '<br/>') . ($item['income'] != 0 ? '金额：' . $item['income'] . '<br/>' : '') . ($item['point'] != 0 ? '积分：' . $item['point'] : '') . '</td>';
    ...
    if (IS_AJAX && $_POST['type'] == 'extracttab')
    {
        ...
        import('source.class.user_page');
        $page = new Page($count, $page_size);
        $list = $user_cash->getRecords(array('c.uid' => $wap_user['uid']), $page->firstRow, $page->listRows);
        $html = '';
        foreach ($list as $cash)
        {
            ...
            $html .= '<td style="text-align: center">' . $user_cash->getStatus($cash['status']) . '</td>';
            $html .= '</tr>';
        }
        echo json_encode(array('count' => $count, 'data' => $html));
        exit;
...
else if ($_GET['a'] == 'recharge')
{
    if (IS_POST)
    {
        ...
        $time = time();
        $recharge = D('Recharge')->where(array(
            'amount' => $balance, 'start_time' => array('<', $time), 'end_time' => array('>', $time),
            'status' => 1))->find();
        if (empty($recharge))
        {
        ...
        $nowOrder = $db_recharge_order->where(array(
            'uid' => $wap_user['uid'], 'total' => $recharge['amount'], 'point' => $recharge['point'],
            'status' => 1))->find();
        if (empty($nowOrder))
        {
            $nowOrder = array(
                ...
                'uid' => $wap_user['uid'], 'total' => $recharge['amount'], 'point' => $recharge['point'],
                'profit' => $recharge['profit'], 'pay_type' => $payType, 'status' => 1, 'add_time' => time(),
    ...
    $list = D('Recharge')->where(array(
        'start_time' => array('<', $time), 'end_time' => array('>', $time), 'status' => 1))->select();
