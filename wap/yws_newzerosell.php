...
{
    if($file['error'] != 4) {
        $upload_dir = "./upload/newzerosell/".date('Y/m/d',time())."/";
...
if($post){
    ...
    //var_dump($_FILES);
    //个人身份证
    $files['idcard_pic'] = $_FILES['idcard'];
    //个人营业执照license
    $files['license_pic'] = $_FILES['license'];
    //银行信息bank
    $files['bank_pic'] = $_FILES['bank'];
    //个人照片persioanal1
    $files['peo1_pic'] = $_FILES['persional1'];
    //个人照片persioanal2
    $files['peo2_pic'] = $_FILES['persional2'];
    $pic='';
    ...
    if(strpos($res,'message')===false){
        ...
        foreach ($pic as $k=>$v){
            if(strpos($v,'message')!==false){
                switch ($k){
                    case 'idcard_pic':
                        $error[]="您的身份证正面".str_replace('message:','',$v);
                        break;
                    case 'license_pic':
                    ...
                    case 'peo1_pic':
                        ...
                        break;
                    case 'peo2_pic':
                        $error[]="身份证反面".str_replace('message:','',$v);
                        break;
                }
            }else{
                @unlink($v);
