...
//当前页面的地址
$now_url = 'http://' . $_SERVER['HTTP_HOST'] . '/wap/ucenter.php?id=' . $now_store['store_id'];
...
if ($allow_drp) {
	...
	$tmp_store_info = $store->getUserDrpStore($_SESSION['user']['uid'], 0, 0); //有分销店铺
	if ((!empty($seller) && $seller['level'] <= $max_store_drp_level) || !empty($tmp_store_info)) { //分销级别没有超出最大限制
		$drp_link = true;
	}
	if ($store_info['uid'] == $_SESSION['user']['uid']) {
		$drp_link_url = './drp_register.php?id=' . $store_id;
	}
	else {
		$seller_store = D('Store')->field('store_id')->where(array('drp_supplier_id' => $store_id,
