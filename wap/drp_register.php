...
if (IS_POST) {
    if ($_POST['type'] == 'check_store') {
        ...
        if (!$parent_uid)
            json_return(1, '请先获取推广码，扫码后成为' . option('config.site_name') . '的会员！');

        $agent_id = I('post.agent_id', 0, 'trim,intval');
        if (!$agent_id) json_return(1, '请选择开店类型！');

        $agent = D('Agent')->where(array('price' => array('>', 0), 'agent_id' => $agent_id))->find();
        if (empty($agent)) json_return(1, '请重新选择开店类型！');

        $payType = 'weixin';
        $balance = $agent['price'] * 1.00; // $config['payfor_store'] * 1.00;
        $db_Payfor_order = D('Payfor_order');
        $nowOrder = $db_Payfor_order->where(array('uid' => $wap_user['uid'], 'agent_id' => $agent_id, 'total' => $balance, 'status' => 1))->find();
        if (empty($nowOrder)) {
            $nowOrder = array(
...
if (option('config.payfor_store') * 1 > 0) {
    ...
    $parent_uid = D('User')->where(array('uid' => $wap_user['uid']))->getField('parent_uid');
    if ($parent_uid) {
        $agents = D('Agent')->where(array('price' => array('>', 0)))->select();
        //var_dump(D('Agent')->last_sql);
