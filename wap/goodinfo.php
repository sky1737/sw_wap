		...
		case 'getSku':
			...
			if ($nowProduct['has_property']) {
				...
				if (!empty($skuList)) {
					...
					$tmpPropertyList =
						D('')->field('`pp`.`pid`,`pp`.`name`')->table(array('Product_to_property' => 'ptp',
						                                                    'Product_property'    => 'pp'))
							->where('`ptp`.`product_id`=\'' . $product_id . '\' AND `pp`.`pid`=`ptp`.`pid`')
							->order('`ptp`.`id` ASC')->select();
					...
					if (!empty($tmpPropertyList)) {
						$tmpPropertyValueList = D('')->field('`ppv`.`vid`,`ppv`.`value`,`ppv`.`pid`')
							...
							->where('`ptpv`.`product_id`=\'' . $product_id . '\' AND `ppv`.`vid`=`ptpv`.`vid`')
							->order('`ptpv`.`id` ASC')->select();

						if (!empty($tmpPropertyValueList)) {
							foreach ($tmpPropertyValueList as $value) {
								$propertyValueList[$value['pid']][] =
									array('vid' => $value['vid'], 'value' => $value['value']);
							}

							foreach ($tmpPropertyList as $value) {
								$newPropertyList[] = array('pid'    => $value['pid'], 'name' => $value['name'],
			...
			$returnArr['customFieldList'] =
				D('Product_custom_field')->field('`field_name`,`field_type`,`multi_rows`,`required`')
					->where(array('product_id' => $product_id))->order('`id` ASC')->select();
