...
<nav class="ui-nav-table clearfix">
	<ul class="pull-left js-list-filter-region">
		<li id="js-list-nav-all" <?php echo $type == 'all' ? 'class="active"' : '' ?>> <a href="#all">所有券</a> </li>
		<li id="js-list-nav-future" <?php echo $type == 'future' ? 'class="active"' : '' ?>> <a href="#future">未开始</a> </li>
		<li id="js-list-nav-on" <?php echo $type == 'on' ? 'class="active"' : '' ?>> <a href="#on">进行中</a> </li>
		<li id="js-list-nav-end" <?php echo $type == 'end' ? 'class="active"' : '' ?>> <a href="#end">已结束</a> </li>
	</ul>
</nav>
<div class="widget-list">
	<div class="js-list-filter-region clearfix ui-box" style="position:relative;">
		<div> <a href="#create" class="ui-btn ui-btn-primary">新建优惠券</a>
	...
	<table class="ui-table ui-table-list" style="padding:0px;">
		...
		<tbody class="js-list-body-region">
			...
			<tr class="js-present-detail" service_id="<?php echo $coupon['id']?>">
				...
				<td><p class="text-left"><?php echo date('Y-m-d H:i:s', $coupon['start_time']) ?> 至</p>
					<p class="text-left"><?php echo date('Y-m-d H:i:s', $coupon['end_time']) ?></p></td>
				<td><?php if($coupon['most_have'] > 0) {?>
					一人<?php echo$coupon['most_have']?>张
					<?php }else {?>
					不限张数
					<?php }?>
					<p class="gray">库存：<?php echo ($coupon['total_amount']-$coupon['number']);?></p></td>
				...
				<td><?php  if($person_num = $unique_person_have[$coupon['id']]['counts']){?>
					...
					<?php }else{echo '0';};?>
					/<?php echo $coupon['number'];?></td>
				<td><?php echo $coupon['used_number'];?>份</td>
				<td class="text-right js-operate" data-coupon_id="<?php echo $coupon['id'] ?>"><a href="javascript:"  class="js-link">链接</a>-
					<?php
						if ($coupon['status']) {
							?>
					<a href="#edit/<?php echo $coupon['id']?>" class="js-edit">编辑资料</a> <span>-</span> <a href="javascript:" class="js-disabled">使失效</a> <span>-</span> <a href="javascript:void(0);" class="js-delete">删除</a>
					<?php
						} else {
							?>
					已失效
