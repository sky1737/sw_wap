$(function(){
	...
	$('.js-edit-second').live('click', function(e){
		var obj = $(this);
		var id = parseInt($(this).parents('.js-second-field').attr('data-id'));
		button_box($(this),e,'left','edit_txt',$(this).attr('data-title'),function(){
			var title = $('.js-rename-placeholder').val();
			$.post('/user.php?c=weixin&a=save_menu',{'id':id, 'title':title}, function(data){
				golbal_tips(data.msg, data.error);
				if (data.error == 0) {
					...
					if (obj.attr('class') == 'js-edit-first') {
						...
						$('#menu_' + id).html('<a class="js-submneu-a" data-id="' + id + '" href="#" target="_blank">' + title + '</a>');
					}
				}
				$('.popover').remove();
			}, 'json');
	...
	$('.js-edit-first').live('click', function(e){
		var obj = $(this);
		var id = parseInt($(this).parents('.js-first-field-li').attr('data-id'));
		button_box($(this),e,'left','edit_txt',$(this).attr('data-title'),function(){
			var title = $('.js-rename-placeholder').val();
			$.post('/user.php?c=weixin&a=save_menu',{'id':id, 'title':title}, function(data){
				golbal_tips(data.msg, data.error);
				if (data.error == 0) {
					$('#title_' + id).find('.h5').text(title);
					$('#menu_' + id).text(title);
				}
				$('.popover').remove();
			}, 'json');
...
//			html += '</div>';
			...
			for (var thisid in response.data) {
				...
				for (var imageid in response.data[thisid].list) {
					i ++;
					title += pre + response.data[thisid].list[imageid].id + '#' + response.data[thisid].list[imageid].title;
					pre = '|';
					html += '<div class="ng-item">';
				...
				html += '<td class="opts">';
				html += '<div class="td-cont">';
				html += '<button class="btn js-choose" href="#" data-id="' + response.data[thisid].id + '" data-url="" data-type="wentu" data-title="' + title + '" data-alias="">选取</button>';
				html += '</div>';
				html += '</td>';
