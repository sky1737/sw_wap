...
$(function () {
	...
	function location_page(mark, dom) {
		...
		switch (mark_arr[0]) {
			...
			case '#physical_store_edit':
				$('.js-app-nav.list').addClass('active').siblings('.js-app-nav').removeClass('active');
				load_page('.app__content', load_url, {page: 'physical_edit_content', id: mark_arr[1]}, '', function () {
					if ($('.js-regions-wrap').data('province') == '') {
						getProvinces('s1', '');
					}
					else {
			...
			case '#user':
				$(".dianpu_left .user").addClass("active").siblings().removeClass("active");
				$('.js-app-nav.user').addClass('active').siblings('.js-app-nav').removeClass('active');
				load_page('.app__content', load_url, {page: 'user_content'}, '');
				break;

			case '#user_add':
				$(".dianpu_left .user").addClass("active").siblings().removeClass("active");
				$('.js-app-nav.user').addClass('active').siblings('.js-app-nav').removeClass('active');
				load_page('.app__content', load_url, {page: 'user_add_content'}, '');
				break;
	...
	$('.physical_list .js-delete').live('click', function (e) {
		...
		button_box($(this), e, 'left', 'confirm', '确定删除？', function () {
			$.post(store_physical_del_url, {'id': id}, function (data) {
				if (!data.err_code) {
