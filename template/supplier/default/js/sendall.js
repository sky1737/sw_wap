$(function(){
	...
	$('.js-choose_btn').live('click', function(){
		...
		$('.kdt_news_preview_wrap, .no_mab').remove();
		$.get('/user.php?c=weixin&a=fetch_wentu', {'id':parseInt($(this).attr('data-id'))}, function(response){
			if (response.error) {
				golbal_tips(response.msg, response.error);
			} else {
				$('#J_source_id').val(response.id);
				var html = '';
				if (response.type == 0) {
					...
					html += '</div>';
					html += '</div>';
					html += '<a class="btn-edit hide" href="/user.php?c=weixin&a=one&id='+response.id+'" style="display: inline;">编辑</a>';
					html += '</div>';
				} else {
					html += '<div class="control-group no_mab">';
					html += '<div class="controls">';
					...
					html += '</div>';
					html += '</div>';
					html += '<a class="btn-edit hide" href="/user.php?c=weixin&a=multi&id='+response.id+'" style="display: inline;">编辑</a>';
					html += '</div>';
				}
...
//			html += '</div>';
			...
			for (var thisid in response.data) {
				...
				html += '<td class="opts">';
				html += '<div class="td-cont">';
				html += '<button class="btn js-choose_btn" href="#" data-id="' + response.data[thisid].id + '" data-url="" data-type="wentu" data-title="' + title + '" data-alias="">选取</button>';
				html += '</div>';
				html += '</td>';
