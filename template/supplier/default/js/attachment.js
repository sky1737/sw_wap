$(function(){
	...
	$('.js-rename').live('click',function(event){
		var id = $(this).closest('tr').attr('pigcms-id');
		...
		button_box($(this),event,'left','edit_txt',$.trim(content_dom.html()),function(){
			...
			if(new_content == ''){
				...
				$.post(edit_name_url,{id:id,name:new_content},function(result){
					if(result.err_code == 0){
						close_button_box();
						layer_tips(0,'修改成功');
	...
	$('.js-batch-btn').live('click',function(){
		...
		$.each($('.js-check-toggle'),function(i,item){
			if($(this).prop('checked')){
				ids.push($(this).closest('tr').attr('pigcms-id'));
			}
		});
		$.post(del_more_url,{id:ids},function(result){
			if(result.err_code == 0){
