...
//
    ...
    $("input[name='bank_name']").live('blur', function(){
        if ($("input[name='bank_name']").val() != '') {
            $("input[name='bank_name']").closest('.control-group').removeClass('error');
            $("input[name='bank_name']").next('.error-message').remove();
        }
    })

    $("input[name='card_no']").live('blur', function(){
        if (isNaN($("input[name='card_no']").val())) {
    ...
    $("input[name='card_user']").live('blur', function(){
...
//    })
    ...
    $('.js-submit-btn').live('click', function(){
        ...
        var card_user = $("input[name='card_user']").val();
        ...
        if (bank_name == '') {
            flag = false;
            $("input[name='bank_name']").closest('.control-group').addClass('error');
            $("input[name='bank_name']").after('<p class="help-block error-message">不能为空</p>');
        }
        $('.widget-account-pretty').closest('.control-group').removeClass('error');
        $('.widget-account-pretty').next('.error-message').remove();
        if (card_no == '') {
        ...
        $("input[name='card_user']").closest('.control-group').removeClass('error');
        $("input[name='card_user']").next('.error-message').remove();
        if (card_user == '') {
            flag = false;
            $("input[name='card_user']").closest('.control-group').addClass('error');
            $("input[name='card_user']").after('<p class="help-block error-message">不能为空</p>');
        }
        ...
        if (flag) {
        	// 'withdrawal_type': withdrawal_type, , 'tel': tel, 'verify_code': verify_code
            $.post(settingwithdrawal_url, {'card_id':card_id, 'bank_id': bank_id, 'bank_name': bank_name, 'card_no': card_no, 'card_user': card_user}, function(data) {
                if (data.err_code == 0) {
                    $('.notifications').html('<div class="alert in fade alert-success">' + data.err_msg + '</div>');
                    t = setTimeout('msg_hide()', 3000);
                    if (!$(obj).hasClass('account')) {
                        window.location.hash = '#income_apply';
                        var page_content = 'income_apply_content'; //申请提现
                        load_page('.app__content', load_url, {'page': page_content}, '', function () {});
                    } else {
    ...
    $('.js-submit').live('click', function(){
        ...
        if (flag) {
        	var card_id = $('.js-bank-list-region > ul > .bank').data('id');
...
//		'bank_id': bank_id, 'opening_bank': opening_bank, 'bank_card_user': bank_card_user, 'bank_card': bank_card, 'withdrawal_type': withdrawal_type
            $.post(applywithdrawal_url, {'card_id': card_id, 'amount': money}, function(data){
                if (!data.err_code) {
                    $('.notifications').html('<div class="alert in fade alert-success">提现成功！</div>');
...
//    })
    ...
    $('.js-add-bankcard').live('click', function(){
        var page_content = 'income_setting_content';
        load_page('.app__content',load_url,{'page': page_content},'', function(){});
    })

    //首页-提现账号
    $('.js-setup-account').live('click', function(){
        var page_content = 'income_edit_content';
        window.location.hash = '#income_edit/account';
