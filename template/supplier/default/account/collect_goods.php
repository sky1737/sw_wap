...
<script type="text/javascript">
	var load_url = "<?php dourl('load');?>",
		save_url = "<?php dourl('collect_goods'); ?>";

	$(function () {
		load_page('.app__content', load_url, {page: 'collect_goods_content'}, '');
		
		$(".js-delete").live('click', function () {
			if (!confirm("您确定要取消收藏此商品吗？")) {
				return;
			}

			var url = "/index.php?c=collect&a=cancel&id=" + $(this).attr("data-id") + "&type=" + 1;
		...
		$("#pages a").live('click',function () {
			...
			var url = "<?php echo url("account:collect_goods") ?>&page=" + page;
			location.href = url;
		});
	});
</script>
