...
$(function(){
	...
	$('.btn-save-rule').live('click', function(){
		var name = $('.js-txt').val(), obj = $(this), id = $('#id').val();
		$.post('/user.php?c=weixin&a=save_rule', {'name':name, 'id':id}, function(response){
...
//			html += '</div>';
			...
			for (var thisid in response.data) {
				...
				html += '<td class="opts">';
				html += '<div class="td-cont">';
				html += '<button class="btn js-choose" href="#" data-id="' + response.data[thisid].id + '" data-url="'+info_url+'" data-type="wentu" data-title="' + title + '" data-alias="多条图文">选取</button>';
				html += '</div>';
				html += '</td>';
