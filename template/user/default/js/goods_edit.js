...
$(function () {
    ...
    $('.js-batch-factory').live('click', function () {
        ...
        $('.js-batch-form').show();
        $('.js-batch-type').hide();
        $('.js-batch-txt').attr('placeholder', '请输入厂成本价格');
        $('.js-batch-txt').focus();
    });
        ...
        if (!is_edit_sku) {
            if ($('.table-sku-stock > tbody > .sku').length > 0) {
                $('.table-sku-stock > tbody > .sku').each(function (i) {
                    ...
                    var factory = $(this).find('.js-factory').val();
            ...
            $('.table-sku-stock > tbody > .sku').each(function (i) {
                ...
                var sku_factory = $(this).find('.js-factory').val();
...
//                        return false;
                    ...
                    if (sku_factory.length > 0) {
                        if (isNaN(sku_factory)) {
                            var float_val = 0;
                        }
                        else {
                            var float_val = parseFloat(sku_factory);
                        }
                    }
...
//                    return false;
                ...
                if ($("input[name='factory']").val() == 0 || $("input[name='factory']").val() == '' || isNaN($("input[name='factory']").val())) {
                    $('.factory').val('0');
//                    layer_tips(1, '商品成本价不能为空');
//                    $('.cost').val('');
//                    $('.cost').focus();
            ...
            $.each(sku_arr[current_i].find('.sku-atom'), function (i, item) {
                ...
                try {
                    ...
                    if (typeof sku_data_obj[tmp_key] != "undefined") {
                        ...
                        js_sku_cost = sku_data_obj[tmp_key].factory;
                ...
                html += '<td><input type="text" name="sku_factory" class="js-factory input-mini" value="' + js_sku_factory + '" maxlength="10" /></td>';
                ...
                $.each(sku_arr[current_i2].find('.sku-atom'), function (j, jtem) {
                    ...
                    try {
                        ...
                        if (typeof sku_data_obj[tmp_key] != "undefined") {
                            js_sku_price = sku_data_obj[tmp_key].price;
                            js_sku_market = sku_data_obj[tmp_key].market;
                            js_sku_factory = sku_data_obj[tmp_key].factory;
                            js_sku_num = sku_data_obj[tmp_key].quantity;
                            js_sku_weight = sku_data_obj[tmp_key].weight;
                            js_sku_code = sku_data_obj[tmp_key].code;
                            js_sku_sales = parseInt(sku_data_obj[tmp_key].sales) + 0;
