$(function () {
    ...
    var search_suUid_kw = function (e) {
    ...
    $("select[name='supplier_id']").live('change',search_suUid_kw);

    //分页
    $('.js-page-list > a').live('click', search_suUid_kw)
    ...
    $('.js-promotion-btn').live('click', function () {
        ...
        $.post(get_qrcode_activity_url, {'product_id': product_id}, function (data) {
            ...
            /*if (!is_fx) {
             ...
             li += '<li class="clearfix type-' + data[i]['type'] + '" data-id="' + data[i]['id'] + '"><a href="javascript:;" class="js-delete-qrcode pull-left btn-delete-qrcode" data-id="' + data[i]['id'] + '">删除</a>' + title + '</li>';
             qrcode_arr[data[i]['id']] = '<img class="loading" src="./source/qrcode.php?type=good&id=' + product_id + '&activity=' + data[i]['id'] + '" /><p>' + desc + '</p><div class="clearfix qrcode-links"><a class="pull-left" href="./source/qrcode.php?type=good&id=' + product_id + '&activity=' + data[i]['id'] + '" download="true" target="_blank">下载二维码</a><a class="pull-right js-edit" data-id="' + data[i]['id'] + '" href="javascript:;">修改优惠</a></div>';
             }
             }*/
            var html = '<div class="widget-promotion" style="top: ' + ($(obj).offset().top - 68) + 'px; left: ' + ($(obj).offset().left - 260) + 'px;">' +
                //'<ul class="widget-promotion-tab clearfix js-tab"><li class="active" data-tab="qrcode">商品二维码</li><!--<li data-tab="link">商品链接</li>--></ul><div class="widget-promotion-content js-tabs-content"><div class="js-link-region js-tab-content js-tab-content-link" style="display: none;"><div><div class="widget-promotion-main"><div class="alert">分享才有更多人看到哦</div><div class="widget-promotion-content"><label>商品页链接</label><div class="input-append"><input type="text" class="" readonly="true" value="' + url + '"><button type="button" class="btn js-btn-copy" data-clipboard-text="' + url + '">复制</button></div><label>直接弹出购买界面的链接</label><div class="input-append"><input type="text" class="" readonly="true" value="' + buy_url + '"><button type="button" class="btn js-btn-copy" data-clipboard-text="' + buy_url + '">复制</button></div></div></div></div></div><div class="js-qrcode-region js-tab-content js-tab-content-qrcode"><div><div class="widget-promotion-main"><div class="js-qrcode-content"><div class="alert">扫一扫，在手机上查看并分享<!--a class="new-window qrcode-help pull-right" href="#" target="_blank">帮助</a--></div><div class="qrcode-content"><div class="qrcode-left-sidebar js-qrcode-left-sidebar"><div class="qrcode-left-lists"><ul><li class="clearfix active" data-id="-1">直接购买商品</li><!--<li class="clearfix" data-id="-2">关注后购买商品</li>-->' + li + '</ul></div>' + create_qrcode_link + '</div><div class="qrcode-right-sidebar js-qrcode-right-sidebar"><div class="text-center"><img class="loading" src="' + qrcode_url + '" /><p>扫码后直接访问商品</p><div class="clearfix qrcode-links"><a class="pull-left" download="" href="' + qrcode_url + '" target="_blank">下载二维码</a></div></div></div></div></div><div class="qrcode-create-content js-qrcode-create-content"></div></div></div></div></div>'+
                //'<ul class="widget-promotion-tab clearfix js-tab"><li class="active" data-tab="qrcode">商品二维码</li></ul>'+
                '<div class="widget-promotion-content js-tabs-content"> <div class="js-qrcode-region js-tab-content js-tab-content-qrcode"><div><div class="widget-promotion-main"><div class="js-qrcode-content"><div class="alert">扫一扫，在手机上查看并分享<!--a class="new-window qrcode-help pull-right" href="#" target="_blank">帮助</a--></div><div class="qrcode-content"><div class="text-center"><img class="loading" src="' + qrcode_url + '" /><p>扫码后直接访问商品</p><p><a download="" href="' + qrcode_url + '" target="_blank">下载二维码</a></p></div></div></div></div></div></div></div>' +
                '</div>';
            $('body').append(html);
            if (data.length >= 3) {
    ...
    $('.qrcode-links > .js-edit').live('click', function () {
        ...
        for (i in qrcode_data) {
            if (qrcode_data[i]['id'] == activity_id) {
    ...
    /*//扫码活动保存
     ...
     li = '<li class="clearfix active type-' + data.data.type + '" data-id="' + data.data.id + '"><a href="javascript:;" class="js-delete-qrcode pull-left btn-delete-qrcode" data-id="' + data.data.id + '">删除</a>' + title + '</li>';
     ...
     qrcode_arr[data.data.id] = '<img class="loading" src="./source/qrcode.php?type=good&id=' + product_id + '&activity=' + data.data.id + '" /><p>' + desc + '</p><div class="clearfix qrcode-links"><a class="pull-left" href="./source/qrcode.php?type=good&id=' + product_id + '&activity=' + data.data.id + '" download="true" target="_blank">下载二维码</a><a class="pull-right js-edit" data-id="' + data.data.id + '" href="javascript:;">修改优惠</a></div>';
     $('.js-qrcode-right-sidebar > .text-center').html(qrcode_arr[data.data.id]);
