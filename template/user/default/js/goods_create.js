var cat_id = '', cat_name = '', group_ids = '', arrs2 = '';
var sys_propert = {};
		...
		if ($(".sxsx").length) {
			if ($(".sxsx .red").length > 0) {
				$(".sxsx .red").each(function (i) {
					...
					sys_fields[i] = {
						'sys_property_id': sys_property_id,
						'sys_property_value_id': sys_property_value_id
					};
				})
			}
			else {
		...
		case '1':
			if (cat_id == '' || cat_name == '') {
				...
				if (next_id == 2) {
					...
					if ($("input[name='market']").val() == '' || isNaN($("input[name='market']").val()) || $("input[name='market']").val() < 0) {
						$('.js-step').hide();
						$('#step-2').show();
						$('.js-step-1').removeClass('active');
						$('.js-step-2').addClass('active');
...
function create_sku() {
	...
	switch (sku_dom_count) {
		case 1:
			...
			$.each(sku_arr[0].find('.sku-atom'), function (i, item) {
				html += '<tr sku-ids="' + ($(item).attr('atom-id')) + '">';
				html += '<td rowspan="1">' + ($(item).find('span').html()) + '</td>';
				html += tbody;
			});
		...
		case 2:
			html += '<th class="text-center">' + (sku_arr[0].siblings('h3').find('span.select2-chosen').html()) + '</th>';
			html += '<th class="text-center">' + (sku_arr[1].siblings('h3').find('span.select2-chosen').html()) + '</th>';
			html += thead;
			$.each(sku_arr[0].find('.sku-atom'), function (i, item) {
				$.each(sku_arr[1].find('.sku-atom'), function (j, jtem) {
					...
					html += '<td>' + ($(jtem).find('span').html()) + '</td>';
					html += tbody;
				});
			});
		...
		case 3:
			html += '<th class="text-center">' + (sku_arr[0].siblings('h3').find('span.select2-chosen').html()) + '</th>';
			html += '<th class="text-center">' + (sku_arr[1].siblings('h3').find('span.select2-chosen').html()) + '</th>';
			html += '<th class="text-center">' + (sku_arr[2].siblings('h3').find('span.select2-chosen').html()) + '</th>';
			...
			$.each(sku_arr[0].find('.sku-atom'), function (i, item) {
				$.each(sku_arr[1].find('.sku-atom'), function (j, jtem) {
					$.each(sku_arr[2].find('.sku-atom'), function (k, ktem) {
						...
						html += '<td>' + ($(ktem).find('span').html()) + '</td>';
	...
	html += '</tbody>' +
		'<tfoot><tr>' +
		'<td colspan="7"><div class="batch-opts">批量设置：' +
