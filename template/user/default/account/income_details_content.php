				...
				<form class="form-horizontal ui-box list-filter-form" onsubmit="return false;">
					...
					<div class="control-group">
						...
						<div class="controls">
							...
							<a href="javascript:;" class="ui-btn ui-btn-primary js-filter" data-loading-text="正在查询...">查询</a> </div>
					</div>
				</form>
			</div>
		</div>
		...
		<table class="ui-table ui-table-list" style="padding:0px;">
			...
			<tbody class="js-list-body-region">
				...
				<tr class="widget-list-item">
					...
					<td><span><?php echo $types[$record['type']]; ?></span> | <span class="c-gray"><?php echo empty($record['order']) ? '' : $record['order']['order_no']; ?></span></td>
					<td class="text-right ui-money ui-money-income"><?php echo ($record['income'] == 0 ? '-' : ($record['income'] > 0 ? '+' : '-')) . number_format(abs($record['income']), 2, '.', ''); ?></td>
					<td class="text-right ui-money ui-money-outlay"><?php echo ($record['point'] == 0 ? '-' : ($record['point'] > 0 ? '+' : '-')) . abs($record['point']); ?></td>
