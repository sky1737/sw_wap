...
var is_logistics = <?php echo ($order_store['open_logistics'] || $is_all_supplierproduct) ? 'true' : 'false' ?>;
var is_selffetch = <?php echo ($order_store['buyer_selffetch'] && $is_all_selfproduct) ? 'true' : 'false' ?>;
</script>
...
<div class="order_content">
	<div class="Breadcrumbs">
		您的位置：<a href="/">首页</a> &gt; <a href="<?php echo url('user:account:index') ?>">会员中心</a> &gt; <a href="<?php echo url('account:order') ?>" class="current">我的订单</a>
	</div>
	...
	<div class="order_contetn_add">
		<?php 
		if ($order_store['buyer_selffetch'] || $order_store['open_logistics']) { ?>
		<div class="order_add">
				<div class="tab1" id="tab1">
					<div class="menu">
						<ul class="clearfix js-tab">
							...
							if ($order_store['open_logistics'] || $is_all_supplierproduct) {
								$is_open_logistics = true;
								echo '<li class="off" data-for="buyerInfo">快递配送</li>';
							}
							if ($order_store['buyer_selffetch'] && $is_all_selfproduct) {
								echo '<li '.($is_open_logistics ? '' : 'class="off"').' data-for="self">'.($order_store['buyer_selffetch_name'] ? $order_store['buyer_selffetch_name'] : '到店自提').'</li>';
							}
							if ($order_store['open_friend'] && $is_open_logistics) {
								echo '<li data-for="friend">送朋友</li>';
							} ?>
						</ul>
					</div>
							...
							<div class="js_address_detail" id="con_one_<?php echo $is_open_logistics ? '2' : '1' ?>" <?php echo !$is_open_logistics ? '' : 'style="display: none;"' ?> >
								...
								<div class="order_add_list">
									<ul class="order_add_list_ul js-self-address-list">
										...
										?>
											<li class="<?php echo $class ?>" id="self_address_<?php echo $tmp['id'] ?>" data-address-id="<?php echo $tmp['id'] ?>">
												<span></span>
												<div class="order_add_name"><?php echo htmlspecialchars($tmp['name']) ?></div>
		...
		?>
			...
			.payment_list .selected {border:1px solid rgb(245, 38, 72); color:rgb(245, 38, 72); font-weight:bold;}
			</style>
		...
		<div class="order_order">
			...
			<ul class="order_add_table">
				...
				<li>
					<dl>
						<dt>
							<div  class="order_dec">店铺:<a href="<?php echo url('store:index', array('id' => $order_store['store_id'])) ?>"><span><?php echo htmlspecialchars($order_store['name']) ?></span></a></div>
						</dt>
