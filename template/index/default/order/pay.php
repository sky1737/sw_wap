...
<div class="order_content">
	<div class="Breadcrumbs"> 您的位置：<a href="/">首页</a> &gt; <a href="<?php echo url('user:account:index') ?>">会员中心</a> &gt;
		<a href="<?php echo url('account:order') ?>" class="current">我的订单</a></div>
			...
			<ul>
				...
				if($order['payment_method'] == 'codpay') {
					if($order['shipping_method'] == 'selffetch') {
						echo '<li style="padding:0;"><label>货到付款</label><span class="f14">此订单为货到付款，请' .
							($order_store['buyer_selffetch_name'] ? $order_store['buyer_selffetch_name'] : '到店自提') .
							'后，进行付款。</span></li>';
					}
					else {
						echo '<li style="padding:0;"><label>货到付款</label><span class="f14">此订单为货到付款，请收到货时，直接将货款付给送货员。</span></li>';
					}
				} ?>
				<li style="padding:0;">
					<label>应付总额</label>
...
if($order['payment_method'] != 'codpay') { ?>
	<script type="text/javascript">
		...
		function getPayUrl() {
			...
			$.post('<?php dourl('recognition:see_tmp_qrcode',
				array('type'      => 'order',
				      'qrcode_id' => $order['order_id'],
				      'store_id'  => $store_session['store_id']));?>', function (result) {
				if (result.err_code != 0) {
					...
					if (result.err_msg.error_code != 0) {
						...
						$('.pay-code .code .qrcode_img').attr('src', result.err_msg.qrcode);
					}
				}
			});
		...
		function checkPay(type) {
			...
			if (checkPayNow == false) {
				$.getJSON("<?php dourl('order:check',
					array('order_id' => $order['order_no_txt']));?>", function (result) {
					checkPayNow = false;
					if (result.err_msg == 'ok') {
						window.location.href = "<?php dourl('order:detail',
							array('order_id' => $order['order_no_txt']));?>";
					}
				});
			}
		}
