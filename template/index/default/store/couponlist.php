...
<title>欢迎光临<?php echo htmlspecialchars($order_store['name']) ?>-<?php echo $config['seo_title'] ?></title>
...
.keyilin li {
	...
	<SCRIPT type=text/javascript>
		...
		function changePage(page) {
			...
			var url = "<?php echo url('store:couponlist',array('storeid' => $order_store['store_id'])) ?>&page=" + page;
...
<div class="shop_header">
	<div class="shop_header_left">
		<div class="shop_header_left_img"> <img src="<?php echo $order_store['logo'] ?>" /> </div>
		<div class="shop_header_left_list">
			<ul>
				<li>
					<div class="shop_name"><?php echo htmlspecialchars($order_store['name']) ?></div>
					<div class="shop_comment"><div class="shop_comment_bg" style="width:<?php echo $comment_type_count['satisfaction_pre'] ?>;"> </div></div>
					<?php
					if ($order_store['approve']) {
					?>
				...
				<li>
					...
					<div class="shop_list_txt"><?php echo date('Y-m-d', $order_store['date_added']) ?></div>
					<div class="shop_list_txt"></div>
				</li>
				<li>
					<dl class="js-store-operation">
						<!--<dd><a href="###">分享店铺</a></dd>-->
						<dd><a href="javascript:userCollect('<?php echo $order_store['store_id'] ?>', '2')">收藏店铺</a></dd>
						<dd><a href="javascript:userAttention('<?php echo $order_store['store_id'] ?>', '2')">关注店铺</a></dd>
					</dl>
				</li>
			</ul>
		</div>
	...
	<div class="shop_header_content">
		<ul>
			<li class="peisong">
				<p>收藏数</p>
				<p><em class="store_collect_<?php echo $order_store['store_id'] ?>"><?php echo $order_store['collect'] ?></em></p>
			</li>
			<li class="qijia">
				<p>关注数</p>
				<p><em class="store_attention_<?php echo $order_store['store_id'] ?>"><?php echo $order_store['attention_num'] ?></em></p>
			</li>
			<li class="songda">
				<p>商品数</p>
	...
	<div class="shop_header_right"><img src="/source/qrcode.php?type=home&id=<?php echo $order_store['store_id'] ?>" style="width:120px; height:119px;" />
		<p>微信访问</p>
	</div>
</div>
<?php
