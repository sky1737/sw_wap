		...
		<div class="shopping_menu">
			<div class="shopping_menu_left">
				<div class="mod18">
					<div id="picBox" class="picBox">
						<ul class="cf">
							<li>
								<a href="javascript:;"><img src="<?php echo $product['image'] ?>" alt="<?php echo htmlspecialchars($product['name']) ?>" /></a>
							</li>
							<?php
					...
					<div id="listBox" class="listBox">
						<ul class="cf">
							...
							foreach ($product_image_list as $product_image) {
								...
								?>
								<li>
									<i class="arr2"></i><img src="<?php echo $product_image['image'] ?>" alt="<?php echo htmlspecialchars($product['name']) ?>" />
								</li>
								<?php
							}
							?>
			...
			<div class="shopping_menu_right">
				<ul>
					...
					<li class="shop_info">
						<span class="js-express">请设置您的位置</span><em>|</em><span class="js-distance" data-long="<?php echo $store_contact['long'] ?>" data-lat="<?php echo $store_contact['lat'] ?>"><i></i>0km</span>
						<?php /*<em>|</em><span><span><?php echo $product['drp_seller_qty'] ?></span>个分销商</span>*/ ?>
						<?php
						if($store['drp_level'] < 3 || $product['supplier_id'] == 0) {
							/*,'pszlr':'<?php echo $product[drp_profit];?>','fxsl':'<?php echo $product[drp_seller_qty]?>','fxlr':'<?php echo ($product[min_fx_price]-$product[cost_price]).'~'.($product[max_fx_price]-$product[cost_price])?>'
							var rate = parseInt("<?php echo $config['buyer_ratio']; ?>");
					var exch = parseInt("<?php echo $config['point_exchange']; ?>");
					var is_point = "<?php echo $config['default_point']; ?>" == "1";
							*/ ?>
							<span data-json="{'name':'<?php echo $product[name] ?>','price':'<?php echo $product[price] ?>','intro':'<?php echo $product[intro] ?>','type':'product','qrcode':'<?php echo $product['qrcode']['qrcode'] ?>','is_point':<?php echo $config['default_point']; ?>, 'point':<?php echo $product['point']; ?>, 'profit':<?php echo $product['profit']; ?>}" class="shopping_sale wyfx cursor">我要分销</span>
						<?php } ?>
					</li>
					<?php
							...
							<li class="item_property_detail" data-index="<?php echo $key ?>">
								...
								<div style="float:left;width:480px;">
									...
									foreach ($property['values'] as $tmp) {
										if($tmp['image']) { ?>
											<div class="shopping_li_img js_selected_property normal" id="sku_<?php echo $property['pid'] .
												'_' .
												$tmp['vid'] ?>" data-pid="<?php echo $property['pid'] ?>" data-vid="<?php echo $tmp['vid'] ?>" data-sku="<?php echo $property['pid'] .
												':' . $tmp['vid'] ?>" title="<?php echo $tmp['value'] ?>">
												<img src="<?php echo $tmp['image'] ?>" alt="<?php echo $tmp['value']; ?>" />
											</div>
										<?php }
										else { ?>
											<div class="shopping_li_size js_selected_property normal" id="sku_<?php echo $property['pid'] .
												'_' .
												$tmp['vid'] ?>" data-pid="<?php echo $property['pid'] ?>" data-vid="<?php echo $tmp['vid'] ?>" data-sku="<?php echo $property['pid'] .
												':' .
												$tmp['vid'] ?>" title="<?php echo $tmp['value'] ?>"><?php echo $tmp['value'] ?></div>
										<?php }
									} ?>
									<div class="clear"></div>
								</div>
					...
					<li>
						<div style="float:left; width:200px; color:#6c6c6c;"> 立返&nbsp;&nbsp;<?php
							echo intval($config['default_point']) ? round($product['point'],0) . ' 积分' : '￥' . round
								($product['profit'],2);
							?>
							</dd>
						</div>
					...
					<li>
						...
						<form onsubmit="return false">
							...
							?>
							<div style="float:left; width:200px; color:#6c6c6c;">
								<?php
								if($product['show_sku']) {
									?>
								...
								if($product['buyer_quota']) {
									?>
									&nbsp;&nbsp;每位会员限购
									<span class="js_buyer_quota"><?php echo $product['buyer_quota'] ?></span>件
									<?php
								}
								?>
							</div>
			...
			<div class="shopping_content_title">
				<div class="shopping_dingwei">
					<div class="shopping_tabd">
						<dl>
							<dd onclick="$(this).addClass('title_curn');$(this).next().removeClass('title_curn');" class="title_curn">
								<a href="#product_dec">商品介绍</a></dd>
							<dd onclick="$(this).addClass('title_curn');$(this).prev().removeClass('title_curn');">
								<a href="#product_comment">商品评价</a></dd>
							<dt>
							<form onsubmit="return false;">
								<button class="dt-go">立即购买</button>
							</form>
							<div class="shopping_shaoma">
								<span class="shaoma_img"></span><span class="shaoma_jian"></span>
								<div class="shopping_shaoma_erweima">
									<P>手机扫一扫&nbsp;&nbsp;轻松下单</P>
									<img src="<?php echo $product['qrcode']['qrcode'] ?>" /></div>
							</div>
							</dt>
						</dl>
					</div>
						...
						<div class="zzsc">
							<div class="tab">
								<div class="tab_title">
									...
									<a href="javascript:;" data-tab="HAO">满意(<span><?php echo $comment_type_count['t3'] +
												0 ?></span>)</a>
									<a href="javascript:;" data-tab="ZHONG">一般(<span><?php echo $comment_type_count['t2'] +
												0 ?></span>)</a>
									<a href="javascript:;" data-tab="CHA">不满意(<span><?php echo $comment_type_count['t1'] +
												0 ?></span>)</a></div>
								<div class="tab_form">
									<div class="form_sec">
							...
							<div class="content_tab">
								<ul>
									<li style="display:block;">
										<div style="height:24px; line-height:24px; padding-top:20px;" class="js_default">加载中</div>
									</li>
									<li>
										<div style="height:24px; line-height:24px; padding-top:20px;" class="js_default">加载中</div>
									</li>
							...
							/*<div class="shop_pingjia"><a name="fabiao"></a>
											...
											<ul>
												<li id="shop_add">
													<form enctype="multipart/form-data" id="upload_image_form" target="iframe_upload_image" method="post" action="<?php echo url('comment:attachment') ?>">
														<div class="updat_pic">
															<a href="javascript:" id="upload_message"><img src="<?php echo TPL_URL; ?>images/jiahao.png" /></a>
								...
								<div class="button">
									<div class="button_txt"><span>文明上网</span> <span>礼貌发帖</span> <span></span>
										<span class="js-word-number">0/300</span></div>
		...
		<div class="shop_info_tab">
			<form onsubmit="return false;">
					...
					<div class="shop_info_tab_list">
						<ul>
							<li class="shop_info_tab_list_name">
								<a target="_blank" href="<?php echo url_rewrite('store:index',
									array('id' => $store['store_id'])) ?>"> <?php echo htmlspecialchars($store['name']) ?> </a>
							</li>
							<?php /*<li class="shop_info_tab_list_dec">(<?php echo $sale_category ?>)</li>*/ ?>
							<li>
							...
							<li class="phone_number"><span></span><?php echo $store_contact['phone1']
									? $store_contact['phone1'] . '-' : '' ?><?php echo $store_contact['phone2'] ?></li>
						</ul>
					</div>
				</div>
				<div class="shop_info_tab_bottom">
					...
					<button class="go_shop" onclick="location.href='<?php echo $config['site_url'] . '/?twid=' .
						$store['store_id']; ?>'">进入店铺
					</button>
					<a href="<?php echo url_rewrite('store:index',
						array('id' => $product['store_id'])) ?>">查看店铺详情</a>
										...
										<li class="cf" style="opacity: 1; display: <?php echo $display ?>;">
											<div class="content_conmment">
												<div class="content_asit_img">
													<a href="javascript:;"><img src="<?php echo $comment_list['user_list'][$tmp['uid']]['avatar'] ?>" /></a>
												</div>
												<div class="content_custom">
													<div class="content_time">
														<a href="javascript:;"><?php echo anonymous($comment_list['user_list'][$tmp['uid']]['nickname']) ?></a>
													</div>
		...
		<div class="shopping_list">
			...
			<ul>
				...
				foreach ($similar_product_list as $key => $similar_product) {
					...
					?>
					<li style="<?php echo $border ?>"><a href="<?php echo url_rewrite('goods:index',
							array('id' => $similar_product['product_id'])) ?>">
							<div class="content_list_img"><img src="<?php echo $similar_product['image'] ?>" /></div>
							<div class="content_list_txt">
								<div class="content_shop_name"><?php echo htmlspecialchars($similar_product['name']) ?></div>
								<div class="content_shop_jion"><?php echo $similar_product['sales'] < 10 ? '10笔以内'
										: $similar_product['sales'] ?></div>
							</div>
						</a></li>
					<?php
				}
				...
				foreach ($sns_list as $key => $tmp) {
					...
					<dd style="<?php echo $border ?>">
						<div class="content_conmment">
							...
							<div class="content_custom">
								<div class="content_time"><a href="javascript:;"><?php echo $tmp['name'] ?></a></div>
