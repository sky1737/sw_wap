...
<style type="text/css">
.content .content_shear .content_list_shear ul li .content_txt {
	word-wrap: break-word
}
.content .content_list_nameplat .content_nameplate_left img {
	width: 305px;
}
.content .content_shear .content_list_shear ul li .content_txt {
	word-wrap: break-word
...
.banner .banner_content .banner_content_asid .banner_content_txt {
	padding-bottom: 6px;
}
.banner .banner_content .banner_content_asid .banner_content_asid_bottom {
	padding: 6px 12px 0;
}
...
<div class="content index_content"> 
  ...
  <!--边栏楼层导航--> 
  <!--热门品牌/最新活动-->
  <div class="content_commodity content_nameplate" id="f4" style="margin-top: 30px;">
    <div class="content_list_nameplat" style="height:430px;">
      <div class="readme">
          ...
          <?php
					...
					if($hot_brands) {
						foreach ($hot_brands as $key => $val) {
							$key++;
							echo '<a href="' . $val['url'] . '" class="box' . ($key < 10 ? '0' . $key : $key) .
								'"><img src="' . $val['pic'] . '" alt="' . $val['name'] . '" data-url="'.$val['qrcode'].'" /></a>';
						}
					}
					?>
        </div>
        <script type="text/javascript">
					$(function() {
						$('.lrtk img').each(function(){
							var src = $(this).attr('src');
							var qrcode = $(this).data('url');
							$(this).hover(function(){
								$(this).attr('src',qrcode);
							},function(){
								$(this).attr('src', src);
							});
						});
  ...
  <div class="content_list_shear">
    ...
    <ul class="content_list_ul content_shear_left">
      ...
      <?php if($k < 3) { ?>
      <li <?php if($k == 0) { ?>class="content_shear_left_big"<?php } ?>> <a href="<?php echo $v['ilink']; ?>"> <img <?php if($k == 0) { ?>width="390px" height="390px" <?php } else { ?>width="185px"
								 height="185px"<?php } ?> src="<?php echo $v['file'] ?>"> </a>
