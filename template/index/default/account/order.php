		...
		foreach ($order_list as $order) {
			...
			<div class="order_list">
				<div class="order_list_title">
					<span class="mr20">订单编号：<a href="###"><?php echo option('config.orderid_prefix') .
								$order['order_no'] ?></a></span>
						...
						foreach ($order['product_list'] as $key => $product) {
							...
							if(!$key) {
								...
								<li class="head_6">
									...
									if($order['status'] < 2) {
										?>
										<p><a target="_blank" href="<?php echo url('order:pay&order_id=' .
												option('config.orderid_prefix') . $order['order_no']) ?>">付款</a></p>
										<p><a href="javascript:" data-id="<?php echo option('config.orderid_prefix') .
												$order['order_no'] ?>" class="cancl_order">取消订单</a></p>
										<?php
									}
									else if($order['status'] == 3) {
										?>
					...
					foreach ($order['product_list'] as $key => $product) {
						...
						<ul class="order_list_list" style="border-bottom:0;">
							<li class="head_1">
								<div class="order_list_img"><img src="<?php echo $product['image'] ?>"></div>
								<div class="order_list_txt">
									<a href="<?php echo url_rewrite('goods:index',
										array('id' => $product['product_id'])) ?>" target="_blank">
							...
							<li class="head_6">
								...
								if($order['status'] < 2) {
									...
									?>
									<p><a href="javascript:" data-id="<?php echo option('config.orderid_prefix') .
											$order['order_no'] ?>" class="over_order">确认收货</a></p>
									<?php
								}
								?>
								<p><a target="_blank" href="<?php echo url('order:detail&order_id=' .
										option('config.orderid_prefix') . $order['order_no']) ?>">查看详情</a></p>
							</li>
						</ul>
						<?php
					}
