...
<style type="text/css">
	.order_tianjia_form .order_tianjian_form_select select {
		color: #000
	}
</style>
<div id="con_one_5">
	...
	<div class="danye_content_title">
		<div class="danye_suoyou"><a href="###"><span>银行卡</span></a></div>
	</div>
	<div class="order_add_list">
		<ul class="order_add_list_ul">
			...
			foreach ($address_list as $tmp) {
				?>
				<li><span class="e-modify" data_id="<?php echo $tmp['address_id'] ?>"
				          data_name="<?php echo htmlspecialchars($tmp['name']) ?>"
				          data_province="<?php echo $tmp['province'] ?>" data_city="<?php echo $tmp['city'] ?>"
				          data_area="<?php echo $tmp['area'] ?>" data_tel="<?php echo $tmp['tel'] ?>"
				          data_address="<?php echo htmlspecialchars($tmp['address']) ?>"
				          data_default="<?php echo $tmp['default'] ?>"></span>

					<div
						class="order_add_add"><?php echo $tmp['province_txt'] ?><?php echo $tmp['city_txt'] ?><?php echo $tmp['area_txt'] ?><?php echo htmlspecialchars($tmp['address']) ?></div>
					<div class="order_add_name"><?php echo htmlspecialchars($tmp['name']) ?></div>
					<div class="order_add_shouji"><?php echo $tmp['tel'] ?></div>
					<div class="order_add_caozuo"><a href="javascript:void(0)" class="e-modify"
					                                 data_id="<?php echo $tmp['address_id'] ?>"
					                                 data_name="<?php echo htmlspecialchars($tmp['name']) ?>"
					                                 data_province="<?php echo $tmp['province'] ?>"
					                                 data_city="<?php echo $tmp['city'] ?>"
					                                 data_area="<?php echo $tmp['area'] ?>"
					                                 data_tel="<?php echo $tmp['tel'] ?>"
					                                 data_address="<?php echo htmlspecialchars($tmp['address']) ?>"
					                                 data_default="<?php echo $tmp['default'] ?>"><i>修改</i></a>|<a
							href="javascript:deleteAddress('<?php echo $tmp['address_id'] ?>')"
							class="e-delete"><i>删除</i></a></div>
				</li>
			<?php
			}
			?>
			<li id="order_tianjian"><span class="order_curn"></span>添加新卡</li>
		</ul>
		<div class="order_tianjia_form">
			<form name="address" id="address" method="post">
				<ul>
					...
					<li>
						<div class="order_tianjian_form_left"><span></span>银行名称:</div>
						<div class="order_tianjian_form_right order_input">
							<input id="jiedao" class="jiedao inputText" placeholder="精确到分行支行"
							       maxlength="120" name="address"/>
						</div>
					</li>
					<li>
						<div class="order_tianjian_form_left"><span></span>姓名:</div>
						<div class="order_tianjian_form_right">
							<input id="name" class="inputText" type="text" maxlength="12" name="name"/>
						</div>
					</li>
					<li>
						<div class="order_tianjian_form_left"><span></span>手机:</div>
						<div class="order_tianjian_form_right">
