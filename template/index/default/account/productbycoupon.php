<?php include display('public:person_header'); ?>
	...
	<style type="text/css">
		...
		.youhui_list .youhui_table .youhui_r {
			border: 0
		}

		.youhui_list .youhui_table .youhui_r .youhui_r_list {
			margin-top: 30px;
		...
		.youhuiquan_content .content_commodity li .content_shop_name {
			float: none
		}
	</style>
			...
			<li>
				...
				<div class="youhui_table clearfix">
					<div class="youhuiquan_info  clearfix">
						<div class="youhiquan_linqu"><?php if ((time() > $coupon['start_time'] &&
									time() < $coupon['end_time']) && ($coupon['is_use'] == 0)
							) {
								echo '未过期';
							}
							else if ($coupon['is_use']) {
								echo '已使用';
							}
							else {
								echo '已过期';
							} ?></div>
						<div class="youhiquan_shuoming">订单金额满<?php
							if ($coupon['limit_money'] > '0') {
								echo '订单满￥<em>' . $coupon['limit_money'] . '</em>可用';
							}
							else {
								echo '无限制';
							} ?></div>
						<div class="youhiquan_price">￥<span><?php
								echo $coupon['face_money'] ?></span></div>
						<div class="youhiquan_data">有效期限:<span><?php
								echo date("Y-m-d", $coupon['start_time']) . '至' .
									date("Y-m-d", $coupon['end_time']); ?></span></div>
					</div>
					<div class="youhui_c">
						<div class="youhui_c_list">•限购【<a target="_blank" href="<?php echo url_rewrite('store:index',
								array('id' => $coupon['store_id'])) ?>"><?php echo $store_name; ?></a>】店铺商品
						</div>
						<div class="youhui_c_list">•<?php if ($coupon['is_all_product'] == '0') {
								echo '可在  全平台  使用';
							}
							else {
								echo '可在  全平台部分商品中  使用';
					...
					<div class="youhui_r">
						<div class="youhui_r_list"><a href="<?php echo url_rewrite('store:index',
								array('id' => $coupon['store_id'])) ?>">进入店铺</a></div>
						...
						<li><a href="<?php echo $v['link'] ?>">
								<div class="content_list_img"><img src="<?php echo $v['image'] ?>">
									...
									<div class="content_list_erweima">
										<div class="content_list_erweima_img"><img src="<?php echo
												$config['site_url'] . '/source/qrcode.php?type=good&id=' .
												$v['product_id']; ?>"></div>
