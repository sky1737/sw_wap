...
function alertlogin() {
	...
	w = 250;
	h = 250;
	msg = "<iframe id='loginIframe' width='" + w + "' height='" + h + "' frameborder='0' src='/index.php?c=recognition&a=see_login_qrcode'></iframe>";
	alertWin('微信扫码登录', msg, w, h);
}
...
function getUserDistance() {
	var web_user = getCookie("Web_user");
	if (web_user) {
		return eval("(" + web_user + ")");
	}
	return false;
}
	...
	switch (type) {
		...
		case 'wyfx':
			...
			switch (sobj.type) {
				...
				//商品
				default:
					var p1 = "价格：<span> ￥" + sobj.price + "</span>";
					var p2 = "简介：<span> " + sobj.intro + "</span>";
					var p3 = "购买" + (sobj.is_point==1 ? "返积分" : "返佣") + "：<span>" + (sobj.is_point==1 ? sobj.point : sobj.profit) + "</span>";
			...
			$(".myfx").find("img.wx_image").attr("src", qrcode);
			$(".myfx").show();
