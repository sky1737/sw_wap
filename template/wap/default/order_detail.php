...
//								echo '未' .
//									($this_store['buyer_selffetch_name'] ? $this_store['buyer_selffetch_name'] : '到店消费');
//							}
//							else {
...
//						else {
							echo $nowOrder['status_txt'];
							if($wap_user['uid'] == $nowOrder['uid'] && $nowOrder['status'] == 3){
								echo '<a href="javascript:;" data-id="'.$nowOrder['order_id'].'" class="btn btn-xxsmall btn-pink pull-right js-order_confirm">确认收货</a>';
							}
//						}
...
//						}
					...
					<hr/>
					<?php
					if($now_store['store_id'] == $nowOrder['agent_id'] && $now_store['uid'] == $wap_user['uid']) {
						echo '<p>代理商发货: 请登录PC端填写发货运单信息！</p>';
					}
					?>
						...
						foreach ($nowOrder['package_list'] as $value) {
							...
							<div class="block-item font-size-12">
								<p class="express-title">
									<?php echo $value['express_company']; ?>&nbsp;&nbsp;&nbsp;<span
										class="c-gray">运单编号：</span><?php echo $value['express_no']; ?>
								</p>
								<p class="express-context">
									<a href="javascript:" data-type="<?php echo $value['express_code'] ?>"
									   data-order_no="<?php echo $nowOrder['order_no_txt'] ?>"
									   data-express_no="<?php echo $value['express_no'] ?>" style="color:#1B9C46;">查看物流信息</a>
									<span class="js-express_close" style="display:none;">关闭物流信息</span>
								</p>
				...
				<div class="block block-order block-border-top-none">
					<div class="header">
						<span>店铺：<?php echo $this_store['name']; ?></span>
					</div>
						...
						<?php foreach ($nowOrder['proList'] as $value) { ?>
							<div class="block-item name-card name-card-3col clearfix js-product-detail">
								<a href="javascript:;" class="thumb">
									<img class="js-view-image" src="<?php echo $value['image']; ?>"
								...
								<div class="right-col">
									<div class="price">￥<span><?php echo number_format($value['pro_num'] *
												$value['pro_price'], 2); ?></span><?php echo $value['is_present'] == 1
											? '<span style="color:#f60;">赠品</span>' : '' ?></div>
					...
					<div class="block-item " style="padding:5px 0;">
						...
						if ($nowOrder['address_arr']) {
							if ($nowOrder['shipping_method'] == 'selffetch') {
								?>
								<ul>
									<li><?php echo $this_store['buyer_selffetch_name']
											? $this_store['buyer_selffetch_name'] : '上门自提' ?></li>
									<li>
										门店电话：<a style="display:inline-block; overflow:visible; padding:0px; margin:0px;"
										        href="tel:<?php echo $nowOrder['address_arr']['address']['tel'] ?>"><?php echo $nowOrder['address_arr']['address']['tel'] ?></a>
										<?php
										if ($nowOrder['address_arr']['address']['physical_id']) {
											?>
											<a style="display:inline-block; overflow:visible; padding:0px; margin:0px;"
											   href="./physical_detail.php?id=<?php echo $nowOrder['address_arr']['address']['physical_id'] ?>">查看地图</a>
										<?php
										}
										else {
											?>
											<a style="display:inline-block; overflow:visible; padding:0px; margin:0px;"
											   href="./physical_detail.php?store_id=<?php echo $this_store['store_id'] ?>">查看地图</a>
										<?php
										}
										?>
									</li>
									...
									<li>联系人：<?php echo $nowOrder['address_arr']['user']; ?>
										，<?php echo $nowOrder['address_arr']['tel']; ?></li>
									<li>
										预约时间：<?php echo $nowOrder['address_arr']['address']['date']; ?> <?php echo $nowOrder['address_arr']['address']['time']; ?></li>
								</ul>
							<?php
							}
							else {
								?>
								<ul>
									<li><?php echo $nowOrder['address_arr']['user']; ?>
										，<?php echo $nowOrder['address_arr']['tel']; ?></li>
									<li><?php echo $nowOrder['address_arr']['address']['province']; ?> <?php echo $nowOrder['address_arr']['address']['city']; ?> <?php echo $nowOrder['address_arr']['address']['area']; ?></li>
									<li><?php echo $nowOrder['address_arr']['address']['address']; ?></li>
								</ul>
					...
					<div class="block block-bottom-0">
						<div class="js-order-total block-item order-total" style="text-align:left;">
							...
							foreach ($order_ward_list as $order_ward) {
								...
								?>
								<p><span
										style="padding:0px 5px; background:#f60; color:white; border-radius:3px;">满减</span><?php echo getRewardStr($order_ward['content']) ?>
								</p>
							<?php
							}
							if ($order_coupon) {
								...
								?>
								<span
									style="padding:0px 5px; background:#f60; color:white; border-radius:3px;">优惠券</span>
								<a href="<?php echo url('coupon:detail',
									array('id' => $order_coupon['coupon_id'])) ?>"><?php echo $order_coupon['name'] ?></a>,优惠金额：<?php echo $order_coupon['money'] ?>元
							<?php
							}
							?>
						</div>
				...
				<div class="block block-bottom-0">
					<div class="js-order-total block-item order-total">
						<p>￥<?php echo $nowOrder['sub_total']; ?> + ￥<?php echo $nowOrder['postage']; ?> 运费
							<?php
							if($nowOrder['balance']){
								echo ' - ￥' .$nowOrder['balance'] .' 账户余额支付';
							}
							if($nowOrder['point']){
								echo ' - ￥' .round(intval($nowOrder['point']) / floatval($config['point_exchange']),2).' '.$nowOrder['point'].'积分抵现';
							}
							echo $money ? ' - ￥' . $money . '优惠金额' : '' ?></p>
						<strong
							class="js-real-pay c-orange js-real-pay-temp">实付：￥<?php echo $nowOrder['pay_money']; ?></strong>
					</div>
					<?php if ($nowOrder['status'] > 1 && $nowOrder['status'] < 5) { ?>
						<div class="block-item paid-time">
							<div class="paid-time-inner">
								...
								<?php
								if ($nowOrder['payment_method'] != 'codpay') {
									echo '<p class="c-gray">'.date('Y-m-d H:i:s', $nowOrder['paid_time']).'<br/>完成付款</p>';
								}
								?>
							</div>
						</div>
