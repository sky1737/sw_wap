...
<head>
	...
	<title>创建店铺 -<?php echo option('config.site_name'); ?></title>
...
<div data-alert="" class="alert-box alert" style="display: none;" id="errerMsg">请输入微店名！<a href="#" class="close">×</a>
</div>
<div class="fixed">
	<nav class="tab-bar">
		<section class="left-small"><a onclick="window.history.go(-1)" class="menu-icon"><span></span></a></section>
		<section class="middle tab-bar-section">
			<h1 class="title">创建店铺</h1>
		</section>
		<section class="right-small right-small-text3"><a href="javascript:void(0)"
		                                                  class="button [radius round] top-button"
		                                                  id="nextBtn"><?php echo '保存'; // $open_drp_approve ? '保存' : '下一步'; ?></a>
		</section>
	</nav>
</div>
<ul class="breadcrumbs step-store" id="ul">
	<li class="first current" id="settitle"><a href="javascript:void(0)"><!--<i class="step-number">1</i>-->设置店铺信息</a>
	</li>
...
<form id="form1" class="setstroe-form">
	...
	<div class="row">
		<div class="large-12 columns">
			<input type="checkbox" style="height: auto;line-height: normal" class="agreement-checked" value="1"/>
			我已阅读并同意<a href="javascript:;" class="agreement">分销协议</a>
			<textarea readonly style="resize: none;font-size: 14px;line-height: 25px;height: 200px;display: none"
			          class="agreement-info"><?php echo $agreement; ?></textarea>
		</div>
	</div>
</form>
<?php
...
<div class="tip-means mb-20 mr-15">
	<h2 class="tip-means-title">
		...
		<span>温馨提示</span>
		<i class="icon-close" onclick="tip_means_close(this)"></i>
	</h2>
...
<script type="text/javascript">
	...
	$("#nextBtn").click(function () {
		...
		if (!$('.agreement-checked:checked').val()) {
			ShowMsg("请阅读并确认同意分销协议再继续！");
			return false;
		}
