...
function getProducts() {
    ...
    $.ajax({
        type: "POST",
        url: 'index_mystyle_ajax2.php?action=get_product_list',
        data: 'keyword=' + keyword + '&key_id=' + key_id + '&page=' + productPage + '&sort=' + productSort + "&prop=" + prop + "&suid=" + suid + "&cat_ids=" + cat_ids,
        dataType: 'json',
                ...
                if (result.err_msg.list.length > 0) {
                    ...
                    for (var i in result.err_msg.list) {
                        ...
                        productHtml += '<div class="hproduct ' + (productShowRows ? '' : 'item_long_cover') + '">' +
                            ...
                            '<p class="sku"><span style="margin:-2px 0 0 6px" class="comment_num ' + (product.sales == '0' ? '' : '') + '">销量 <span>' +(product.sales == '0' ? '1' : product.sales)  + '</span></span>&nbsp;<span class="stock hide">无货</span></p>' +
                            '</a></div>';
                    }
