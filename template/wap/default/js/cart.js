	...
	$('.js-go-pay').click(function () {
		...
		if (ids.length > 0) {
			$.post('./cart.php?action=pay', {storeId: storeId, ids: ids}, function (result) {
				if (result.err_code == 0) {
					var orderid = result.err_msg;
					if (orderid.indexOf(orderPrefix) != -1) {
						window.location.href = './pay.php?id=' + orderid + '&showwxpaytitle=1';
					}
					else {
						window.location.href = './mergepay.php?id=' + orderid + '&showwxpaytitle=1';
					}
				}
				else {
