$(function () {
	...
	$('.js-save').click(function () {
		...
		$('.js-login-form').submit(function () {
			...
			if (phoneDom.prop('disabled') === false) {
				...
				$.post('./login.php?action=reg', {
					phone: $('.js-login-form #phone').val(),
					pwd: passswordDom.val(),
					code: codeDom.val()
				}, function (result) {
					if (result.err_code) {
	...
	$(".js-order_confirm").click(function () {
		...
		$.post('./order.php?orderid=' + id, {'confirm': 1}, function (result) {
			// array('status' => true, 'msg' => '订单已完成！', 'data' => array('nexturl' => 'refresh'))
			$('html').css({'overflow': 'visible', 'height': 'auto', 'position': 'static'});
			...
			if (result.status) {
				location.href = './order.php?orderid=' + id;
			}
			else {
