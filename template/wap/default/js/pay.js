...
$(function () {
	...
	$(".js-show-message").click(function () {
		...
		var comment_html =
			'<div class="modal order-modal active">\
...
//		$.post('address.php?action=postage', {
//			orderNo: orderNo,
			...
			if ($(this).data('type') == 'selffetch') {
				...
				if ($('#selffetch_id').val() != '0' && selffetchEdit == false) {
					for (var i in selffetchListJson) {
						...
						if (selffetch.id == $('#selffetch_id').val()) {
							var myDate = new Date();
					...
					for (var i in selffetchListJson) {
						...
						if (selffetchListJson[i].id == '99999999_store') {
							selffetchListHtml += '		<a class="detail" target="_blank" href="./physical_detail.php?store_id=' + selffetchListJson[i].store_id + '">';
						}
						else {
							selffetchListHtml += '		<a class="detail" target="_blank" href="./physical_detail.php?id=' + selffetchListJson[i].id + '">';
						}
...
//		}
	...
	$('#confirm-pay-way-opts .btn-pay').click(function () {
		...
		var msg = '';
		if ($('.js-msg-container').length > 1) {
			$('.js-msg-container').each(function () {
				var s = $(this).val().replace(/\|/g, ' ');
				msg += (msg == '' ? '' : '||') + $(this).data('id') + '|' + s;
			});
		}
		else {
			msg = $('.js-msg-container').val();
		}

		var balance = $('#balance').length > 0 ? parseFloat($('#balance').val()) : 0.00;
		var point = $('#point').length > 0 ? parseInt($('#point').val()) : 0;
		var post_data = {
			payType: payType,
			orderNo: orderNo,
			msg: msg,
