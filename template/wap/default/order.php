						...
						<li class="block block-order animated">
							<div class="header">
								...
								<?php if ($order['status'] < 2) { ?>
									<a class="js-cancel-order pull-right font-size-12 c-blue" href="javascript:;"
									   data-id="<?php echo $order['order_id']; ?>">取消</a>
								<?php } ?>
							</div>
							...
							foreach ($order['order_product_list'] as $order_product) {
								...
								<div class="block block-list block-border-top-none block-border-bottom-none">
									<div class="block-item name-card name-card-3col clearfix">
										...
										<div class="detail">
											<a href="<?php echo $order['url']; ?>"><h3
													style="margin-bottom:6px;"><?php echo $order_product['name']; ?></h3>
											</a>
											<?php
										...
										<div class="right-col">
											<div class="price">
												¥&nbsp;<span><?php echo $order_product['pro_price']; ?></span></div>
											<div class="num">×<span
													class="num-txt"><?php echo $order_product['pro_num']; ?></span>
											</div>
										</div>
									</div>
								</div>
							...
							if ($order['shipping_method'] == 'selffetch') {
								...
								<div class="bottom">
									...
									if ($order['address']['physical_id']) {
										...
										<div class="opt-btn">
											<a class="btn btn-in-order-list"
											   href="./physical_detail.php?id=<?php echo $order['address']['physical_id'] ?>">查看</a>
										</div>
									<?php
									}
										...
										<div class="opt-btn">
											<a class="btn btn-in-order-list"
											   href="./physical_detail.php?store_id=<?php echo $order['address']['store_id'] ?>">查看</a>
										</div>
									<?php
									}
									?>
							...
							<div class="bottom">
								<?php if ($order['total']) { ?>
									<?php if (!empty($order['float_amount']) && $order['float_amount'] < 0) { ?>
										减免：<span class="c-red">￥<?php echo number_format(abs($order['float_amount']), 2,
												'.', ''); ?></span><br/>
									<?php } ?>
