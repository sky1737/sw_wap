...
<div class="panel extract">
    ...
    <div class="extract-number"><span class="span-title">已提现金额</span> <span class="number"
                                                                            id="MaxCashAmount">￥<?php echo $withdrawal; ?></span>
    </div>
    <div class="panel extract-account">
        <ul class="side-nav">
            <li><span class="bank-info">银行卡：</span><?php echo $card['card_no']; ?><br/>
    ...
    { ?>
        <div class="tip-text">最低提现金额为<b
                id="MinaAmountCash"><?php echo number_format($withdrawal_min, 2, '.', ''); ?></b>元
        </div>
    <?php } ?>
...
<script type="text/javascript">
    function btnSave() {
        ...
        $.post('./balance.php?a=applywithdrawal', {
            'amount': CashAmount,
            'bank_id': bank_id,
            'card_id': card_id
        }, function (data) {
            ...
            if (data.err_code == 0) {
                ...
                //window.location.href = "./balance.php?a=withdrawal_result&status=1&amount=" + data.err_msg;
            } else /*if (data.err_code == 1002)*/ {
                ShowMsg(data.err_msg);
                return false;
                //} else {
                //    window.location.href = "./balance.php?a=withdrawal_result&status=0&amount=" + data.err_msg;
            }
        }, 'JSON');
    }
