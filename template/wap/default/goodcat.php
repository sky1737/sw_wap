			...
			<div class="headerbar">
				<div class="headerbar-wrap clearfix">
					<div class="headerbar-preview">
						...
						<ul>
							<li>
								<a href="<?php echo $now_url; ?>&ps=320"
								   class="js-no-follow<?php if (empty($_GET['ps']) || $_GET['ps'] == '320'
								   ) echo ' active'; ?>">iPhone版</a>
							</li>
							<li>
								<a href="<?php echo $now_url; ?>&ps=540"
								   class="js-no-follow<?php if ($_GET['ps'] == '540') echo ' active'; ?>">三星Note3版</a>
							</li>
							<li>
								<a href="<?php echo $now_url; ?>&ps=800"
								   class="js-no-follow<?php if ($_GET['ps'] == '800') echo ' active'; ?>">PC版</a>
							</li>
						</ul>
					</div>
					<div class="headerbar-reedit">
						<a href="<?php dourl('user:goods:category', array(),
							true); ?>#edit/<?php echo $nowGroup['group_id'] ?>" class="js-no-follow">重新编辑</a>
					</div>
				</div>
			</div>
		<?php } ?>
			...
			if ($nowGroup['list_style_size'] == 0) {
				...
				<ul class="js-goods-list sc-goods-list pic clearfix size-0">
					...
					foreach ($goodList as $key => $value) {
						?>
						<li class="goods-card goods-list big-pic <?php if ($nowGroup['list_style_type'] == 2) {
							echo 'normal';
						}
						else {
							echo 'card';
						} ?>">
							<a href="<?php echo $config['wap_site_url']; ?>/good.php?id=<?php echo $value['product_id']; ?>"
							   class="js-goods link clearfix" target="_blank" title="<?php echo $value['name']; ?>">
								<div class="photo-block">
									<img class="goods-photo js-goods-lazy" src="<?php echo $value['image']; ?>"
									     style="display:inline;">
								</div>
								<div class="info clearfix <?php if ($nowGroup['is_show_product_name'] == 1) {
									echo 'info-title';
								}
								else {
									echo 'info-no-title';
								} ?> <?php if ($nowGroup['is_show_price'] == 1) {
									echo 'info-price';
								}
								else {
									echo 'info-no-price';
								} ?> btn<?php echo $nowGroup['buy_button_style']; ?> <?php if ($nowGroup['is_show_product_name'] ==
									0 || $nowGroup['is_show_price'] == 0
								) {
									echo 'hide';
								} ?>">
								...
								<?php if ($nowGroup['is_show_buy_button'] && $nowGroup['list_style_type'] == 0) { ?>
									<div
										class="goods-buy btn<?php echo $nowGroup['buy_button_style']; ?> info-no-title"></div>
									<div class="js-goods-buy buy-response"
									     data-id="<?php echo $value['product_id']; ?>"></div>
								<?php } ?>
							</a>
						</li>
						<?php
			...
			<?php
				...
				if ($nowGroup['list_style_type'] != 1) {
					...
					<ul class="js-goods-list sc-goods-list pic clearfix size-1">
						...
						foreach ($goodList as $key => $value) {
							?>
							<li class="goods-card goods-list small-pic <?php if ($nowGroup['list_style_type'] == 2) {
								echo 'normal';
							}
							else {
								echo 'card';
				...
				<ul class="js-goods-list sc-goods-list pic clearfix size-2">
					...
					foreach ($goodList as $key => $value) {
						?>
						<li class="goods-card goods-list <?php if ($i % 3 == 1) {
							echo 'big-pic';
						}
						else {
							echo 'small-pic';
						} ?> <?php if ($nowGroup['list_style_type'] == 2) {
							echo 'normal';
						}
						else {
							echo 'card';
				...
				<div class="section-detail">
					...
					<p class="text-center qr-code">
						<img width="158" height="158"
						     src="<?php echo $config['site_url']; ?>/source/qrcode.php?type=good_cat&id=<?php echo $nowGroup['group_id']; ?>">
					</p>
				</div>
			</div>
		</div>
