...
<?php if ($drp_notice) { ?>
	<div id="drp-notice">
		<ul>
			<li class="img-li"><img class="mp-image" width="24" height="24" src="<?php echo !empty($avatar)
					? $avatar : option('config.site_url') . '/static/images/avatar.png'; ?>"
			                        alt="<?php echo $now_store['name']; ?>"/></li>
			...
			<li class="btn-li"><a href="<?php echo $drp_register_url; ?>"
			                      class="button green"><?php if (empty($is_seller)) { ?>立即申请<?php }
					else { ?>分销管理<?php } ?></a></li>
...
<div class="container">
	<?php if (!empty($now_store['drp_approve'])) { ?>
					...
					<div class="headerbar-wrap clearfix">
						<div class="headerbar-preview">
							...
							<ul>
								...
								<li>
									<a href="<?php echo $now_url; ?>&ps=540" class="js-no-follow<?php if ($_GET['ps'] ==
										'540'
									) echo ' active'; ?>">三星Note3版</a>
								</li>
								<?php
								if (option('config.synthesize_store')) {
									...
									<li>
										<a href="<?php echo $now_url; ?>&ps=800"
										   class="js-no-follow<?php if ($_GET['ps'] == '800'
										   ) echo ' active'; ?>">PC版</a>
									</li>
								<?php
								}
						...
						<div class="headerbar-reedit">
							<a href="<?php dourl('user:store:wei_page', array(),
								true); ?>#edit/<?php echo $homePage['page_id'] ?>" class="js-no-follow">重新编辑</a>
						</div>
					</div>
				</div>
			<?php } ?>
		...
		<div class="content"
		     <?php if ($homePage['bgcolor']){ ?>style="background-color:<?php echo $homePage['bgcolor']; ?>;"<?php } ?>>
			<div class="content-body">
				<?php if ($pageHasAd && $pageAdPosition == 0 && $pageAdFieldCon) {
					echo $pageAdFieldCon;
				} ?>
				<?php foreach ($homeCustomField as $value) {
					echo $value['html'];
				} ?>
						...
						<div class="section-detail">
							...
							<p class="text-center qr-code">
								<img width="158" height="158"
								     src="<?php echo $config['site_url']; ?>/source/qrcode.php?type=home&id=<?php echo $now_store['store_id']; ?>">
							</p>
						</div>
					</div>
				</div>
...
<script>
	...
	$('.js-goods-buy').click(function () {
		...
		speed_shop(product_id, 1, function () {
			return;
		})
	});
</script>
