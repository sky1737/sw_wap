        ...
        if (!empty($userinfo)) {
            ...
            if (empty($data['app_id'])) {
                if (stripos($_POST['login_url'], 'Micrstore') > 0) { //pigcms
                    ...
                    if (stripos($_POST['login_url'], 'type=weidian') > 0) { //o2o
                        $data['app_id'] = 2;
                    } else {
                        $data['app_id'] = 0;
                    }
            ...
            _register(array('nickname'        => trim($data['wxname']),
                            'phone'           => trim($data['tel']),
                            'token'           => trim($data['token']),
                            'source_site_url' => trim($data['site_url']),
                            'payment_url'     => trim($data['payment_url']),
                            'notify_url'      => trim($data['notify_url']),
                            'oauth_url'       => trim($data['login_url']),
