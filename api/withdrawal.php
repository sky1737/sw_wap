...
if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
    ...
    if (!_checkSign($sign_key, $_POST)) {
        ...
        if (!empty($_POST['id']) && !empty($_POST['status'])) { //修改提现状态
            ...
            //if (!D('Store_withdrawal')->where(array('id' => intval(trim($_POST['id'])), 'store_id' => intval(trim($_POST['store_id']))))->data($data)->save()) {
            if (!D('Store_withdrawal')->where(array('id' => intval(trim($_POST['id']))))->data($data)->save()) {
                $error_code = 1004;
                $error_msg = '提现状态修改失败';
            }
        } else { //分销商提现记录
