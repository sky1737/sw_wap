                                ...
                                if (!empty($tmp_fx_products)) {
                                    foreach ($tmp_fx_products as $tmp_fx_product) {
                                        ...
                                        if (!empty($tmp_properties)) { //更新商品属性库存
                                            $database_product_sku->where(array('product_id' => $tmp_fx_product['product_id'], 'properties' => $tmp_properties))->setDec('quantity',
                                                $value['pro_num']);
                                        }
                                        if ($tmp_fx_product['product_id'] == $product['original_product_id'] || $tmp_fx_product['product_id'] == $value['product_id']) { //源商品或当前购买的商品
                                            ...
                                            if (!empty($tmp_properties)) { //更新商品属性库存
                                                $database_product_sku->where(array('product_id' => $tmp_fx_product['product_id'], 'properties' => $tmp_properties))->setInc('sales',
                                                    $value['pro_num']);
                                            }
                                        }
                                    }
                ...
                if ($new_order_id = $order->add($order_info)) { //向供货商提交一个新订单
                    ...
                    if ($store->setUnBalanceDec($data['seller_id'], $fx_order_info['cost_total']) && $store->setIncomeDec($data['seller_id'], $fx_order_info['cost_total'])) {
                        ...
                        file_put_contents($dir . 'error.txt',
                            '[' . date('Y-m-d H:i:s') . '] 付款给供货商失败，订单类型：分销，订单ID：' . $order_id . '，交易单号：' . $order_trade_no . '，供货商（收款方）：' . $supplier_name . '，分销商（付款方）：' . $seller_name . '，付款金额：' . $fx_order_info['cost_total'] . '元，请手动从 ' . $seller_name . ' 账户余额中减' . $fx_order_info['cost_total'] . '元' . PHP_EOL,
                            FILE_APPEND);
