<?php
define('TWIKER_PATH', dirname(dirname(__FILE__)) . '/');
...
// 				return $this->scan($id, $data['FromUserName']);
                ...
                if ($menu = D('Diymenu_class')->where(array('store_id' => $weixin_bind['store_id'], 'title' => $id))->find()) {
                    switch ($menu['type']) {
                        case 0:
                            ...
                            break;
                        case 1:
                            $source = D('Source_material')->where(array('id' => $menu['fromid']))->find();
                            $it_ids = unserialize($source['it_ids']);
                            $images = D('Image_text')->where(array('id' => array('in', $it_ids)))->select();
                            $return = array();
            ...
            case 'SUBSCRIBE'://首次关注的回复
                ...
                if ($rule = D("Rule")->where(array('store_id' => $weixin_bind['store_id'], 'type' => 1))->find()) {
                    return special_keyword($rule['id'], $weixin_bind['store_id']);
                } else {
...
function special_keyword($rule_id, $store_id)
{
    ...
    switch ($reply['type']) {
        case 0:
            ...
            $text = D('Text')->where(array('id' => $reply['cid']))->find();
            return array($text['content'] . $tail, 'text');
            break;
        case 1:
            $source = D('Source_material')->where(array('id' => $reply['cid']))->find();
            $it_ids = unserialize($source['it_ids']);
            $images = D('Image_text')->where(array('id' => array('in', $it_ids)))->select();
            $return = array();
            foreach ($images as $im) {
