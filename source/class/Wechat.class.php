...
class Wechat
{
	...
	public function encodeMsg($sRespData)
	{
		...
		$pc = new WXBizMsgCrypt($this->_config['wx_token'], $this->_config['wx_encodingaeskey'],
			$this->_config['wx_appid']);
	...
	{
		$this->data = array('ToUserName' => $this->data['FromUserName'], 'FromUserName' => $this->data['ToUserName'],
		                    'CreateTime' => NOW_TIME, 'MsgType' => $type);
		$this->$type($content);
	...
	private function news($news)
	{
		...
		foreach ($news as $key => $value) {
			list($articles[$key]['Title'], $articles[$key]['Description'], $articles[$key]['PicUrl'],
