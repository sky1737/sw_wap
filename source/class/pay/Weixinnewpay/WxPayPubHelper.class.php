...
class Transfer_pub extends Wxpay_client_pub
{
    ...
    public function postXmlAndCert($xml, $url, $second = 30)
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_TIMEOUT, $second);
        ...
        if ($err = curl_errno($ch)) {
            logs('CURL:' . $err . '|http://curl.haxx.se/libcurl/c/libcurl-errors.html', 'ERROR');
    ...
    {
        $xml            = $this->createXml();
        $this->response = $this->postXmlAndCert($xml, $this->url, $this->curl_timeout);
