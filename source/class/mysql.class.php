...
class mysql
{
    ...
    public function __construct($other = array())
    {
        ...
        if (!$this->conn =
            mysql_connect($_G['system']['DB_HOST'], $_G['system']['DB_USER'], $_G['system']['DB_PWD'], true, 131072)
            //mysql_connect($_G['system']['DB_HOST'] . ':' . $_G['system']['DB_PORT'], $_G['system']['DB_USER'], $_G['system']['DB_PWD'])
        ) {
        ...
        pigcms_tips('连接数据库失败，未找到您填写的数据库 <b>' . $_G['system']['DB_NAME'] . '</b>', 'none');
    }
    ...
    function where($where)
    {
        ...
        if (is_array($where)) {
            $connector = ' AND ';
            if (isset($where['connector']) && $where['connector'] == 'OR') {
                $connector = ' OR ';
                ...
                if (is_array($value)) {
                        ...
                        if ($value[0] == 'in') {
                            ...
                            if ($value[0] == 'or') {
                                ...
                                if ($value[0] == 'not in') {
                                    ...
                                    //						elseif($value[0] == 'gt' || $value[0] == '>') {
                                    //							$now_arr[] = '`' . $key . '`>' . "'" . $value[1] . "'";
                                    //						}
                                    //						elseif($value[0] == 'lt' || $value[0] == '<') {
                                    //							$now_arr[] = '`' . $key . '`<' . "'" . $value[1] . "'";
                                    //						}
                                    else {
                                        if ($value[1] === '') $value[1] = "''";
                    ...
                    if (strpos($key, '|')) {
                        ...
                        foreach ($key_arr as $v) {
                            $tmp_str .= (empty($tmp_str) ? '' : ' OR ') . '`' . $v . '` = ' . "'" . $value . "'";
                        }
                        $now_arr[] = '(' . $tmp_str . ')';
                    } else {
                        $now_arr[] = '`' . $key . '`=' . "'" . $value . "'";
    ...
    function getField($field, $order = null)
    {
        $this->field = '`' . $field . '`';
        $this->limit = '1';
        if (!$order) {
