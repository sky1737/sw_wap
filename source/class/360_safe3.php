...
function StopAttack($StrFiltKey, $StrFiltValue, $ArrFiltReq, $type = '')
{
    if (($type == 'post' && $StrFiltKey == 'referer') /*|| ($type=='referer'&&strpos($StrFiltValue,'ttp://'.DOMAIN_NAME))*/) {
        ...
        if (preg_match("/" . $ArrFiltReq . "/is", $StrFiltValue) == 1) {
            ...
            print '<div 
style=\"position:fixed;top:0px;width:100%;height:100%;background-color:white;color:green;font-weight:bold;border-bottom:5px solid #999;\"><br>' . $type . ' 不允许提交html、script等代码类数据，系统在您自己的服务器上不会碰到类似问题，请测试其他功能，感谢支持。!</div>';
            exit();
        }
        if (preg_match("/" . $ArrFiltReq . "/is", $StrFiltKey) == 1) {
            //slog("<br><br>操作IP: ".$_SERVER["REMOTE_ADDR"]."<br>操作时间: ".strftime("%Y-%m-%d %H:%M:%S")."<br>操作页面:".$_SERVER["PHP_SELF"]."<br>提交方式: ".$_SERVER["REQUEST_METHOD"]."<br>提交参数: ".$StrFiltKey."<br>提交数据: ".$StrFiltValue);
            print '<div style=\"position:fixed;top:0px;width:100%;height:100%;background-color:white;color:green;font-weight:bold;border-bottom:5px solid #999;\"><br>' . $type . ' 不允许提交html、script等代码类数据，系统在您自己的服务器上不会碰到类似问题，请测试其他功能，感谢支持</div>';
            exit();
        }
    }
}
