            ...
            <td>
                <form action="{pigcms{:U('Store/activityRecommend')}" method="get">
                    ...
                    <select name="type">
                        ...
                        <if condition="$_GET['type'] eq 'activity_id'">selected="selected"</if>
                        >活动编号</option>
                        <option value="activity_name"
                        <if condition="$_GET['type'] eq 'activity_name'">selected="selected"</if>
                        >活动名称</option>
                    ...
                    <select name="activity_type">
                        <option
                        <if condition="$_GET['activity_type'] eq 999">selected="selected"</if>
                        value="999">全部</option>
                        <php>
                            foreach($modelName as $key=>$val){
                        </php>
                        <option
                        <if condition="$_GET['activity_type'] eq $key">selected="selected"</if>
                        value="{pigcms{$key}">{pigcms{$val}</option>
                        <php>
                            }
                        </php>
                    </select>
                        ...
                        <td class="pagebar" colspan="8">
                            <div>
                                <div style="float: left">
                                    <label style="cursor: pointer;color: #3865B8;" class="label-choice-all">全选</label> /
                                    <label style="cursor: pointer;color: #3865B8;"
                                           class="label-choice-cancel">取消</label>
                                    <button class="activityRecommend" actType="activityRecommendRecAdd"
                                            style="height:30px;width:100px;">添加热门推荐
                                    </button>
                                    <button class="activityRecommend" actType="activityRecommendRecDel"
                                            style="height:30px;width:100px;">取消热门推荐
                                    </button>
                                    　　
                                    <button class="activityRecommend" actType="activityRecommendDel"
                                            style="height:30px;width:75px;">删除活动
                                    </button>
                                </div>
