        ...
        <if condition="$withdrawals neq ''">
            <volist name="withdrawals" id="withdrawal">
                <tr class="widget-list-item">
                    ...
                    <td width="195">
                        <if condition="$withdrawal['withdrawal_type'] eq 0">个人账户
                            <else/>
                            公司账户
                        </if>
                        |
                        <span class="js-bank-detail"
                              style="cursor: pointer;"><?php echo mb_substr($cash['bank'], 0, 4, 'UTF-8'); ?>
                            ...<?php echo substr($cash['bank_card'], -4); ?><b class="caret"
                                                                               data-bank="{pigcms{$withdrawal.bank}"
                                                                               data-opening-bank="{pigcms{$withdrawal.opening_bank}"
                                                                               data-bank-card="{pigcms{$withdrawal.bank_card}"
                                                                               data-bank-account="{pigcms{$withdrawal.bank_card_user}"></b></span>
                        <br>
                        <span class="c-gray">{pigcms{$withdrawal.trade_no}</span>
                    </td>
                    <td class="text-right ui-money-outlay">{pigcms{$withdrawal.amount}</td>
                    <td><?php if (!empty($cash['complate_time'])) { ?><?php echo date('Y-m-d H:i:s', $cash['complate_time']); ?><?php } ?></td>
