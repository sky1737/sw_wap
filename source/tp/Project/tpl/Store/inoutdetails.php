            ...
            <td>
                <form action="{pigcms{:U('Store/inoutdetail')}" method="get">
                    ...
                    <select name="type">
                        ...
                        <if condition="$_GET['type'] eq 'order_no'">selected="selected"</if>
                        >收支流水号</option>
                        <option value="trade_no"
                        <if condition="$_GET['type'] eq 'trade_no'">selected="selected"</if>
                        >交易单号</option>
                        <option value="store"
                    ...
                    <select name="record_type">
                        ...
                        <volist name="record_types" id="type">
                            ...
                            <if condition="$Think.get.record_type eq $key">selected="true"</if>
                            >{pigcms{$type}</option>
                        </volist>
                    </select>
                    &nbsp;&nbsp;支付方式：
                    <select name="payment_method">
                        ...
                        <volist name="payment_methods" id="payment_method">
                            <option value="{pigcms{$key}"
                            <if condition="$Think.get.payment_method eq $key">selected</if>
                            >{pigcms{$payment_method}</option>
                        </volist>
                    </select>
                    &nbsp;&nbsp;时间：
                    <input type="text" name="start_time" id="js-start-time" class="input-text Wdate"
                           style="width: 150px" value="{pigcms{$Think.get.start_time}"/>- <input type="text"
                                                                                                 name="end_time"
                                                                                                 id="js-end-time"
                                                                                                 style="width: 150px"
                                                                                                 class="input-text Wdate"
                                                                                                 value="{pigcms{$Think.get.end_time}"/>
    ...
    <div class="table-list">
        <table width="100%" cellspacing="0">
            ...
            <if condition="is_array($records)">
                <volist name="records" id="record">
                    <tr>
                        ...
                        <td><span class="ui-money-income"><if condition="$record['income'] gt 0">+
                                    {pigcms{$record.income}
                                </if></span></td>
                        <td>
                            <if condition="$record['income'] lt 0"><span class="ui-money-outlay">- {pigcms{$record.income|abs|number_format=###, 2, '.', ''}</span>
                            </if>
                        </td>
            ...
            <if condition="is_array($records)">
                ...
                <else/>
                <tr>
                    <td class="textcenter red" colspan="14">列表为空！</td>
                </tr>
            </if>
            </tfoot>
        </table>
