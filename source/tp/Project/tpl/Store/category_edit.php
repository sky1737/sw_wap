...
<form id="myform" method="post" action="{pigcms{:U('Store/category_edit')}" enctype="multipart/form-data">
    <table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
                ...
                <select name="parent_id">
                    ...
                    <volist name="categories" id="all_category">
                        <if condition="$all_category['cat_id'] neq $category['cat_id']">
                            ...
                            <if condition="$all_category['cat_id'] eq $category['parent_id']">selected</if>
                            >{pigcms{$all_category.name}</option>
                        </if>
                    </volist>
                </select>
            </td>
        ...
        <tr>
            <th width="80">分类名称</th>
            <td><input type="text" class="input fl" name="name" id="name" size="25" value="{pigcms{$category.name}"
        ...
        <tr>
            <th width="80">分类排序</th>
            <td><input type="text" class="input fl" name="order_by" value="{pigcms{$category.order_by}" size="10"
        ...
        <tr>
            ...
            <td>
                <span class="cb-enable"><label
                        class="cb-enable <if condition='$category.status eq 1'>selected</if>"><span>启用</span><input
                            type="radio" name="status" value="1"
                        <if condition="$category['status'] eq 1">checked="true"</if>
                        /></label></span>
                <span class="cb-disable"><label
                        class="cb-disable <if condition='$category.status eq 0'>selected</if>"><span>禁用</span><input
                            type="radio" name="status" value="0"
                        <if condition="$category['status'] eq 0">checked="true"</if>
                        /></label></span>
            </td>
        </tr>
        <tr>
            ...
            <td><textarea rows="3" style="width: 97%" class="fl" name="desc"
                          id="desc">{pigcms{$category.desc}</textarea></td>
        </tr>
    </table>
