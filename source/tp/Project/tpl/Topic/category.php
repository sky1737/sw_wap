    ...
    $(function () {
        $('.status-enable > .cb-enable').click(function () {
            if (!$(this).hasClass('selected') && parseFloat($(this).data('status')) == 1) {
                ...
                $.post("<?php echo U('Topic/category_status'); ?>", {'status': 1, 'cat_id': cat_id}, function (data) {
        ...
        $('.status-disable > .cb-disable').click(function () {
            if (!$(this).hasClass('selected') && parseFloat($(this).data('status')) == 1) {
                ...
                if (!$(this).hasClass('selected')) {
                    $.post("<?php echo U('Topic/category_status'); ?>", {
                        'status': 0,
                        'cat_id': cat_id
                    }, function (data) {
                        window.location.href = url;
...
<div class="mainbox">
	<div id="nav" class="mainnav_title">
		<ul>
			<li><a href="{pigcms{:U('Topic/category')}" class="on">专题分类</a> | <a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Topic/category_add')}','添加分类',480,310,true,false,false,addbtn,'add',true);">添加分类</a></li>
		</ul>
	</div>
	<table class="search_table" width="100%">
		<tr>
			<td><form action="{pigcms{:U('Topic/category')}" method="get">
					<input type="hidden" name="c" value="Topic"/>
					<input type="hidden" name="a" value="category"/>
					筛选:
					<select name="cat_id">
						<option value="0">专题类目</option>
			...
			<tbody>
				...
				<volist name="categories" id="category">
					<tr>
						...
						<td>
							<a url="<?php echo U('Topic/category_del', array('id' => $category['cat_id'])); ?>" class="delete_row"><img src="{pigcms{$static_path}images/icon_delete.png" width="18" title="删除" alt="删除"/></a> |
							<a href="javascript:void(0);" onclick="window.top.artiframe('{pigcms{:U('Topic/category_edit', array('id' => $category['cat_id']))}','修改分类 - {pigcms{$category.cat_name}',480,<if condition="$category['cat_pic']">390<else/>310</if>,true,false,false,editbtn,'edit',true);"><img src="{pigcms{$static_path}images/icon_edit.png" width="18" title="修改" alt="修改"/></a></td>
					</tr>
				</volist>
				</if>
			</tbody>
