        ...
        <tr>
            ...
            <th style="text-align: center;width:90px">
                <span class="cb-enable">
                    <label class="cb-enable <?php if (!empty($store['status'])) { ?>selected<?php } ?>"
                           data-id="<?php echo $store['store_id']; ?>"><span>开启</span><input type="radio" name="status"
                                                                                             value="1"
                                                                                             <?php if (!empty($store['status'])) { ?>checked="checked"<?php } ?>></label>
                </span>
                <span class="cb-disable">
                    <label class="cb-disable <?php if (empty($store['status'])) { ?>selected<?php } ?>"
                           data-id="<?php echo $store['store_id']; ?>"><span>关闭</span><input type="radio" name="status"
                                                                                             value="0"
                                                                                             <?php if (empty($store['status'])) { ?>checked="true" <?php } ?> /></label>
                </span>
            </th>
        <tr/>
    </volist>
