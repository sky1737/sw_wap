    ...
    <volist name="modules" id="m">
        <?php if (!intval($_GET['iskeyword']) || (intval($_GET['iskeyword']) && $m['askeyword'])) { ?>
            <li>
                ...
                <div class="mright">
                    <if condition="$m['sub']"><a
                            href="<?php if (!$m['linkurl']) { ?>?g=Merchant&c=Link&a={pigcms{$m.module}&iskeyword=<?php echo intval($_GET['iskeyword']); ?><?php } else {
                                echo $m['linkurl'];
                            } ?>">详细</a></if>
                    <if condition="$m['canselected']"><a href="###"
                                                         onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])) { ?>{pigcms{$m.linkcode}<?php } else { ?>{pigcms{$m.keyword}<?php } ?>')"
                                                         style="margin-left:14px;">选中</a></if>
                </div>
