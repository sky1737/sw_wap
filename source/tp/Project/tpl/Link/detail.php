...
<div style="background:#fefbe4;border:1px solid #f3ecb9;color:#993300;padding:10px;margin-bottom:5px;">
    ...
    <a href="<if condition="$_GET['iskeyword'] eq 1">{pigcms{:U('Link/insert', array('iskeyword' => 1))}
    <else/>
    {pigcms{:U('Link/insert')} </if>">
    点击这里返回模块选择</a></div>
...
<p>点击“选中”即可</p>
    ...
    <volist name="list" id="m">
        <tr>
            <td>{pigcms{$m.name}</td>
            <td class="norightborder"><a href="###"
                                         onclick="returnHomepage('<?php if (!intval($_GET['iskeyword'])) { ?>{pigcms{$m.linkcode}<?php } else { ?>{pigcms{$m.keyword}<?php } ?>')">选中</a>
                <if condition="$m['sub'] neq NULL"><?php if (!intval($_GET['iskeyword'])) { ?>&nbsp;<a
                        href="{pigcms{$m.sublink}">详情</a><?php } ?></if>
            </td>
        </tr>
    </volist>
</table>
