    ...
    <form name="myform" id="myform" action="" method="post">
        <div class="table-list">
            <table width="100%" cellspacing="0">
                ...
                <td>
                    <if condition="$c.operName eq ''">无
                        <else/>
                        {pigcms{$c.operName}
                    </if>
                </td>
                <td>
                    <if condition="$c.operName eq ''">会员卡支付或积分兑换
                        <else/>
                        {pigcms{$c.notes}
                    </if>
                </td>
                <td>
                    <nobr>{pigcms{$c.time|date='Y-m-d H:i:s',###}</nobr>
                </td>
                <td class="button-column" nowrap="nowrap">
                    <a title="删除" class="red" style="padding-right:8px;"
                       href="{pigcms{:U('Card/useRecord_del',array('itemid'=>$c['id'],'id'=>$thisCard['id']))}">删除</a>
                </td>
                </tr>
                </volist>
                <else/>
