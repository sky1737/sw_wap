    ...
    <form id="myform" method="post" action="" refresh="true" enctype="multipart/form-data">
        <table cellpadding="0" cellspacing="0" class="table_form" width="100%">
            ...
            <tr>
                <td>有效期：</td>
                <td>
                    <input type="text" class="input-text" id="statdate" value="{pigcms{$vip.statdate|date='Y-m-d',###}"
                           onClick="WdatePicker()" name="statdate" style="width:90px;"> （含）到
                    <input type="text" class="input-text" id="enddate" value="{pigcms{$vip.enddate|date='Y-m-d',###}"
                           name="enddate" onClick="WdatePicker()" style="width:90px;">（含）
                </td>
            </tr>
            <tr>
                <td>优惠劵属性：</td>
                <td>
                    <label><input type="radio"
                        <if condition="$vip['attr'] eq '0' or $vip['attr'] eq ''">checked="checked"</if>
                        name="attr" class="attr" value="0">普通券</label>
                    <label><input type="radio"
                        <if condition="$vip['attr'] eq '1'">checked="checked"</if>
                        name="attr" class="attr" value="1">赠送劵(赠送卷只有在添加开卡赠送的时候可以使用)</label>
                </td>
            </tr>
            <tr>
                <td>抵用金额：</td>
                <td>
                    <input type="input" class="input-text" value="{pigcms{$vip.price}" name="price" style="width:50px;">
                    元
                </td>
            </tr>
            <tr>
                <td>优惠劵图标：</td>
                <td>
                    <img src="<if condition="$vip.pic eq '' and $vip.type eq 0">/static/images/cart_info/daijin.png
                    <else/>
                    {pigcms{$vip.pic}</if>" id="pic_src"style="max-width:200px;"><br/>
                    <input type="text" name="pic" id="pic" value="<if condition="$vip.pic eq '' and $vip.type eq
                    0">/static/images/cart_info/daijin.png
                    <else/>
                    {pigcms{$vip.pic}</if>" class="input-text" style="width:200px;">
            ...
            <tr>
                <td>数量：</td>
                <td>每个用户可以获得：<input type="input" class="input-text" id="people" value="{pigcms{$vip.people}"
                                    name="people" style="width:50px;"> 张券
                </td>
            </tr>
        </table>
        <div class="btn">
