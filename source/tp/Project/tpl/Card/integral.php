    ...
    <form name="myform" id="myform" action="" method="post">
        <div class="table-list">
            <table width="100%" cellspacing="0">
                ...
                <td>
                    <if condition="$list['type'] eq 1">无时间限制
                        <else/>
                        {pigcms{$list.statdate|date='Y-m-d',###}
                    </if>
                </td>
                <td>
                    <if condition="$list['type'] eq 1">无时间限制
                        <else/>
                        {pigcms{$list.enddate|date='Y-m-d',###}
                    </if>
                </td>
                <td class="button-column" nowrap="nowrap">
                    ...
                    |
                    <a class="green" style="padding-right:8px;"
                       href="{pigcms{:U('Card/integral_edit', array('itemid'=>$list['id'],'id'=>$thisCard['id']))}">编辑</a>
                    |
                    <a title="删除" class="red" style="padding-right:8px;"
                       href="{pigcms{:U('Card/integral_del',array('itemid'=>$list['id'],'id'=>$thisCard['id']))}">删除</a>
                </td>
                </tr>
                </volist>
                <tr>
                    <td class="textcenter pagebar" colspan="5">{pigcms{$pagebar}</td>
                </tr>
                <else/>
                <tr>
