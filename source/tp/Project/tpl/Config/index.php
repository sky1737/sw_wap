...
<div class="mainbox">
    <div id="nav" class="mainnav_title">
        <ul>
            <volist name="group_list" id="vo">
                <a href="{pigcms{:U('Config/index',array('gid'=>$vo['gid']))}"
                <if condition="$gid eq $vo['gid']">class="on"</if>
                >{pigcms{$vo.gname}</a>|
            </volist>
        </ul>
    </div>
    <form id="myform" method="post" action="{pigcms{:U('Config/amend')}" refresh="true">
        ...
        <div class="btn" style="margin-top:20px;">
            ...
            <input type="button" value="微信API接口填写信息" class="button"
                   onclick="window.top.artiframe('{pigcms{:U('Config/show',array('id'=>$vo['id']))}','API接口信息',560,100,true,false,false,'','add',true);"/>
        </div>
    </form>
</div>
