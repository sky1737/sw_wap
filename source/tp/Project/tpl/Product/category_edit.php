...
<style type="text/css">
    #cate_menu .select2-search-choice {
    ...
    <table cellpadding="0" cellspacing="0" class="frame_form" width="100%">
            ...
            <td><select name="parent_id">
                    ...
                    <volist name="categories" id="all_category">
                        <if condition="$all_category['cat_id'] neq $category['cat_id']">
                            <option value="{pigcms{$all_category.cat_id}"<?php echo $all_category['cat_id'] == $category['cat_fid']?'selected':''; ?>><?php if ($all_category['cat_level'] > 1) { echo str_repeat('|——', $all_category['cat_level']-1); } ?>{pigcms{$all_category.cat_name}</option>
                        </if>
                    </volist>
                </select></td>
        </tr>
        ...
        <tr>
                ...
                <if condition="$propertytype['option'] neq ''">
                    ...
                    <div id="cate_menu">
                        <if condition="$filter_attr">
                            <foreach name="filter_attr" item="val" key="k"><span class="select2-search-choice"
                                                                                 value="{pigcms{$val}">{pigcms{$property_type[0]['option'][$val]}<a
                                        href="#" class="select2-search-choice-close" tabindex="-1"
                                        onclick="$(this).closest('li').remove();$('.popover-chosen .select2-input').focus();"></a></span>
                            </foreach>
                        </if>
                    </div>
            ...
            <select name="property_type[]" class="property_type" onchange="change_property_type($(this))">
                <option value="0">顶级分类</option>
                <foreach name="property_type" item="propertytype" key="k">
                    <option value="{pigcms{$propertytype.type_id}"
                ...
                <div class="cate_menu js-cate-menu">
                    ...
                    if ($tag_list) {
                        foreach ($tag_list as $tmp) {
                            ?>
                            <span class="select2-search-choice js-tag" id="tag_<?php echo $tmp['id'] ?>"
                                  value="<?php echo $tmp['id'] ?>"><?php echo $tmp['name'] ?><a href="javascript:"
                                                                                                class="select2-search-choice-close"
                                                                                                tabindex="-1"
                                                                                                onclick="deleteTag('<?php echo $tmp['id'] ?>')"></a><input
                                    type="hidden" name="tag[]" value="<?php echo $tmp['id'] ?>"/></span>
                        <?php
                        }
                    }
                    ?>
