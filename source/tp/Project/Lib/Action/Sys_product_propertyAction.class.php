...
//        }
	...
	public function propertyType()
	{
		...
		$propertytype =
			$PropertyType->where($where)->order('type_id ASC')->limit($page->firstRow, $page->listRows)->select();
	...
	public function property_add()
	{
		...
		if (IS_POST) {
			...
			//查询同意分类下 不允许相同的属性
			if ($property->where(array('name' => $data['name'], 'property_type_id' => $data['property_type_id']))
				->find()
			) {
				$this->frame_submit_tips(0, '该属性已经存在！');
			}
	...
	public function propertyValue_edit()
	{
		...
		if (IS_POST) {
			...
			if ($productPropertyValue->where(array('value' => $data['value'], 'pid' => $pid,
			                                       'vid'   => array('neq', $vid)))->find()
			) {
				$this->frame_submit_tips(0, '该属性值已经存在！');
			}
