...
//		$store['bank'] = $bank;
		...
		$agents = M('Agent')->where(array('status' => 1))->order('sort asc, agent_id desc')->select();
		$store['agents'] = $agents;
	...
	{
		$store_id = I('get.id', 'intval');
		$refer = I('get.refer');
		if (!$refer) $refer = U('index');
		if (!$store_id) {
			$this->error('参数错误！');
		}
		$db_store = M('Store');
		$uid = $db_store->where(array('store_id' => $store_id))->getField('uid');
		$db_store->where(array('store_id' => $store_id))->delete();
		if ($uid) {
			$count = $db_store->where(array('uid' => $uid, 'status' => 1))->count();
			M('User')->where(array('uid' => $uid))->save(array('stores' => $count));
		}
		$this->success('删除成功！', $refer);
	}
	...
	public function status()
	{
		...
		if (IS_POST) {
			...
			$status = $this->_post('status', 'trim' . (in_array($type,array('supplier_code','name')) ? '' : ',intval') );
...
//    }
	...
	public function inoutdetail()
	{
		...
		if ($store_id) {
			...
			//未处理的提现记录
			$withdrawal_count = M('StoreWithdrawal')->where(array('status' => 1))->count('id');
			$this->assign('withdrawal_count', $withdrawal_count);
		}
		...
		if ($this->_get('start_time', 'trim') && $this->_get('end_time', 'trim')) {
			$where['_string'] = "FinancialRecord.add_time >= '" . strtotime($this->_get('start_time', 'trim')) .
				"' AND FinancialRecord.add_time <= '" . strtotime($this->_get('end_time')) . "'";
		}
		...
		$record_count = $financial_record->where($where)->count('FinancialRecord.id');
		import('@.ORG.system_page');
		$page = new Page($record_count, 10);
		$records = $financial_record->where($where)->order("FinancialRecord.id DESC")->limit($page->firstRow . ',' .
	...
	public function withdraw()
	{
		...
		if ($this->_get('start_time', 'trim') && $this->_get('end_time', 'trim')) {
			$where['_string'] = "StoreWithdrawal.add_time >= '" . strtotime($this->_get('start_time', 'trim')) .
				"' AND StoreWithdrawal.add_time <= '" . strtotime($this->_get('end_time')) . "'";
		}
		else if ($this->_get('start_time', 'trim')) {
		...
		$withdrawal_count = $withdrawal->where($where)->count('StoreWithdrawal.id');
		import('@.ORG.system_page');
		$page = new Page($withdrawal_count, 10);
		$withdrawals = $withdrawal->where($where)->limit($page->firstRow, $page->listRows)
			->order('StoreWithdrawal.status ASC, StoreWithdrawal.id DESC')->select();
	...
	public function withdraw_status()
	{
		...
		if ($withdrawal->where(array('id' => $id))->save(array('status' => $status, 'complate_time' => time()))) {
			if ($status == 4) { //提现失败
	...
	public function category()
	{
		...
		if ($this->_get('cat_id', 'trim,intval')) {
			$where['_string'] = "cat_id = '" . $this->_get('cat_id', 'trim,intval') . "' OR parent_id = '" .
			...
			if ($_FILES['pic']['error'] != 4) {
				...
				if ($upload->upload()) {
					...
					if ($attachment_upload_type == '1') {
						...
						upyunUser::upload('./upload/category/' . $rand_num . '/' . $uploadList[0]['savename'],
							'/category/' . $rand_num . '/' . $uploadList[0]['savename']);
					}
	...
	public function activityManage()
	{
		...
		$_eventsId = array('bargain' => 'id', 'seckill' => 'action_id', 'cutprice' => 'id'); //活动ID转换，调用时用作排序
		...
		$_modelName = array('bargain' => '砍价', 'seckill' => '秒杀', 'crowdfunding' => '众筹', 'unitary' => '一元夺宝',
			'cutprice' => '降价拍', 'red_packet' => '红包'); //活动名称
	...
	public function activityRecommendAdd()
	{
		if ($this->isPost()) {
			foreach ($_POST as $val) {
				$activityRecommend =
					M('Activity_recommend')->where(array('modelId' => $val['modelId'], 'model' => $val['model'],
						'token' => $val['token']))->find();
	...
	public function activityRecommend()
	{
		...
		$activity =
			$activityRecommend->where($condition)->order("id DESC")->limit($page->firstRow . ',' . $page->listRows)
				->select();
	...
	public function check()
	{
		...
		$order_list =
			$order->where($where)->order("order_id asc")->limit($page->firstRow . ',' . $page->listRows)->select();
		...
		$info = array(
			...
			'had_page_liyun_account' => array_sum($page_uncheck_account) -
				array_sum($had_page_should_check_account)
		);
