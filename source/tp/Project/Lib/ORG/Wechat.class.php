...
class Wechat
{
	...
	{
		$this->nonce = //I('request.nonce', '');
			isset($_REQUEST['nonce']) ? $_REQUEST['nonce'] : '';
		$this->sTimeStamp = // I('request.timestamp', time());
			isset($_REQUEST['timestamp']) ? $_REQUEST['timestamp'] : time();
		$this->msg_signature = // I('request.msg_signature');
			isset($_REQUEST['msg_signature']) ? $_REQUEST['msg_signature'] : '';
	...
	public function encodeMsg($sRespData)
	{
		...
		$pc = new WXBizMsgCrypt($this->_config['wechat_token'], $this->_config['wechat_encodingaeskey'],
			$this->_config['wechat_appid']);
	...
	{
		$signature = I('get.signature'); // $_GET["signature"];
