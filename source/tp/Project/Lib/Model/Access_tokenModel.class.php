    ...
    public function get_access_token()
    {
        ...
        if (!$this->checkTokenExpires($access_obj['access_token']) && $access_obj && (intval($access_obj['expires_in']) > $now)) {
            ...
            if ($json->errmsg) {
                ...
                if ($access_obj) {
                    M('Access_token')->where(array('id' => $access_obj['id']))->save(array('access_token' => $json->access_token, 'expires_in' => intval($json->expires_in + $now)));
                } else {
                    M('Access_token')->add(array('access_token' => $json->access_token, 'expires_in' => intval($json->expires_in + $now)));
                }
