...
function logs($str, $type = 'info')
{
    $logs =
        "[ " . date('Y-m-d H:i:s') . " ] {$_SERVER['REMOTE_ADDR']} " .
        "{$_SERVER['HTTP_HOST']} {$_SERVER['REQUEST_URI']} " .
        "[{$type}] {$str}\r\n\r\n";
    error_log($logs, 3, TWIKER_PATH . '/cache/' . date("Ymd", time()) . '.log.php');
}

class cookies
{
    static public function put($key, $val = '', $exp = 30, $path = '/', $domain = '')
    {
        if (!$key) return;
        if (!$val) $val = '';
        if (!$exp) $exp = 30;
        if (!$path) $path = '/';
        if (!$domain) $domain = '.' . option('config.site_domain');
        $exp = 86400 * $exp;
        ...
        setcookie($key, $val, $exp, $path, $domain);
    }
...
function getTwikerUrl($twid)
{
    $uri = $_SERVER['REQUEST_URI'];
    if (strpos(strtolower($uri), 'drp_') == true) {
    ...
    $url .= (strpos($url, '?') !== false ? '&' : '?') . 'twid=' . $twid;
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            return file_put_contents($filename,
                strip_whitespace("<?php\treturn " . var_export($save_file, true) . ";?>"));
        }
    }
...
{
    if (file_exists(TWIKER_PATH . 'library/controller/' . GROUP_NAME . '/' . $class . '.php')) {
        require_once(TWIKER_PATH . 'library/controller/' . GROUP_NAME . '/' . $class . '.php');

        return;
    } else {
        if (file_exists(TWIKER_PATH . 'library/controller/' . $class . '.php')) {
            require_once(TWIKER_PATH . 'library/controller/' . $class . '.php');

            return;
        } else {
            if (file_exists(TWIKER_PATH . 'library/model/' . $class . '.php')) {
                require_once(TWIKER_PATH . 'library/model/' . $class . '.php');

                return;
            } else {
                if (file_exists(TWIKER_PATH . 'library/class/' . $class . '.class.php')) {
                    require_once(TWIKER_PATH . 'library/class/' . $class . '.class.php');

                    return;
                } else {
                    if (file_exists(TWIKER_PATH . 'source/class/' . $class . '.class.php')) {
                        require_once(TWIKER_PATH . 'source/class/' . $class . '.class.php');
    ...
    switch ($file_arr_count) {
        case 1:
            if (substr($file_arr[0], 0, 1) == '@') {
                $load_file = TWIKER_PATH . 'library/class/' . substr($file_arr[0], 1) . '.class.php';
            } else {
                $load_file = TWIKER_PATH . 'source/class/' . $file_arr[0] . '.class.php';
            }
            break;
        case 2:
            if (substr($file_arr[0], 0, 1) == '@') {
                $load_file = TWIKER_PATH . 'library/' . substr($file_arr[0], 1) . '/' . $file_arr[1] . '.class.php';
            } else {
                $load_file = TWIKER_PATH . 'source/' . $file_arr[0] . '/' . $file_arr[1] . '.class.php';
            }
            break;
        case 3:
            $load_file = TWIKER_PATH . $file_arr[0] . '/' . $file_arr[1] . '/' . $file_arr[2] . '.class.php';
            break;
        default:
            $file_data = '';
            ...
            $load_file = TWIKER_PATH . $file_data . '.class.php';
    }
...
function R($group, $mode, $action)
{
    ...
    $mode_file = TWIKER_PATH . 'library/controller/' . $group . '/' . $mode . '_controller.php';
    if (file_exists($mode_file)) {
        ...
        if (file_exists(TPL_PATH . '/' . ACTION_NAME . '.php')) {
            display();
        } else {
            if (DEBUG) {
...
function M($model)
{
    ...
    $model_file = TWIKER_PATH . 'library/model/' . $lower_model . '_model.php';
        ...
        switch ($tpl_arr_count) {
            case 1:
                if ($now_group == 'wap' && option('config.is_diy_template')) {
                    ...
                    if (file_exists(TPL_PATH . $now_group . '/' . $now_theme . '/theme/' .
                        $tpl_arr[$tpl_arr_count - 1] . '.php')) {
                        if (USE_FRAMEWORK) {
                            $tpl_file = $now_group . '/' . $now_theme . '/' . $now_module . '/' . '/theme/' .
                                $tpl_arr[$tpl_arr_count - 1] . '.php';
                        } else {
                            $tpl_file =
                                $now_group . '/' . $now_theme . '/theme/' . $tpl_arr[$tpl_arr_count - 1] . '.php';
                        }
                    } else {

                        if (USE_FRAMEWORK) {
                            $tpl_file = (strpos($tpl_arr[0], '/') === false) ?
                                $now_group . '/' . $now_theme . '/' . $now_module . '/' . $tpl_arr[0] . '.php'
                                : $tpl_arr[0];
                        } else {
                            $tpl_file = (strpos($tpl_arr[0], '/') === false) ?
                                $now_group . '/' . $now_theme . '/' . $tpl_arr[0] . '.php' : $tpl_arr[0];
                        }
            ...
            case 3:
                $tpl_file =
                    $tpl_arr[0] . '/' . $_G['config']['theme_' . $tpl_arr[0] . '_group'] . '/' . $tpl_arr[1] . '/' .
                    $tpl_arr[2] . '.php';
                break;
            default:
...
function url($url = '', $param = array(), $showDomain = false)
{
    global $_G;
    if ($showDomain) {
        $return_url = 'http://' . $_SERVER['HTTP_HOST'] . '/'; // $_G['config']['site_url']
    } else {
...
function url_rewrite($file, $params = array(), $html = "", $rewrite = true)
{
    if ($rewrite) {
        if (strpos($file, ':')) {
            ...
            $url = 'http://' . $_SERVER['HTTP_HOST'] . '/' . $file; // option('config.site_url')
        } else {
            $url = ($file == 'index') ? '' : '/' . $file;
        }
...
{
    if (preg_match("/^([" . chr(228) . "-" . chr(233) . "]{1}[" . chr(128) . "-" . chr(191) . "]{1}[" . chr(128) . "-" .
            chr(191) . "]{1}){1}/", $word) == true ||
        preg_match("/([" . chr(228) . "-" . chr(233) . "]{1}[" . chr(128) . "-" . chr(191) . "]{1}[" . chr(128) . "-" .
            chr(191) . "]{1}){1}$/", $word) == true ||
        preg_match("/([" . chr(228) . "-" . chr(233) . "]{1}[" . chr(128) . "-" . chr(191) . "]{1}[" . chr(128) . "-" .
...
function json_response($true, $msg = '', $url = '')
{
    if (!headers_sent()) {
        header('Content-type:application/json');
    }
    exit(json_encode(array('status' => $true, 'msg' => $msg, 'data' => array('nexturl' => $url))));
}
...
{
    if (is_string($plugin) && file_exists(TWIKER_PATH . 'source/plugins/' . $plugin . '/' . $plugin . '.php')) {
        return true;
    } else {
        return false;
    }
...
{
    return TWIKER_PATH . 'upload/' . $fileUrl;
}
...
function getImUrl($userId, $storeId)
{
    ...
    if (!empty($is_service) && $_SESSION['wap_user']['app_id'] == 0) {
        $key = get_encrypt_key(array('app_id' => option('config.im_appid'), 'openid' => $userId),
            option('config.im_appkey'));
        $url = IM_SERVER_PATH . '?app_id=' . option('config.im_appid') . '&openid=' . $userId . '&key=' . $key .
            '#serviceList_' . $storeId;
    } else {
        ...
        if ($user_tmp['app_id'] == 1 && !empty($store['open_service']) && !empty($store['source_site_url']) &&
            !empty($store['token'])
        ) {
            $url = $store['source_site_url'] . '/index.php?g=Wap&m=Service&a=index&token=' . $store['token'] .
                '&wecha_id=';
        } else {
            if ($user_tmp['app_id'] == 2 && !empty($store['open_service'])) {
                $url = $store['source_site_url'] . '/wap.php?c=Weidian&a=getImUrl&token=' . $store['token'] . '&wecha_id=' .
                    $_SESSION['wap_user']['third_id'];
            } else {
                $url = '';
            }
        }
...
{
    if (isset($_COOKIE['Web_user']) && $_COOKIE['Web_user']) {
