...
set_time_limit(0); //无执行 时间 限制

//get 自定义参数
!isset($_GET['run']) && die('请加参安全运行!');
$wherePid = isset($_GET['pid']) ? $_GET['pid'] : 0; //产品id
$whereCid = isset($_GET['cid']) ? $_GET['cid'] : 0;; //产品的 分类id
$sqlLimit = isset($_GET['limit']) ? $_GET['limit'] : 0;; //sql limt 条件, 例如: 2,1

//得到通用评论
$baseDir   = dirname(__FILE__) . '/comments';
$commFile  = file("$baseDir/_.txt");
$commCount = count($commFile) - 1;

//得到 最大的 用户 id
$maxUser = D('User')->order('uid desc')->find();
$maxUid  = $maxUser['uid'];
...
{
    $catgId       = $p['category_id'];
    $catgFilePath = "$baseDir/{$catgId}.txt";
    ...
    if (file_exists($catgFilePath))
    {
        ...
        for ($i = 0; $i < $incCommNum; $i++)
        {
            if (mt_rand(0, 9) < 3) //30% 用 特定类别 评论
            {
                ...
                $catgCommCount = count($catgFile) - 1;
                $content       = $catgFile[ mt_rand(0, $catgCommCount) ]; //随机取 对应类别 评论
            } else $content = $commFile[ mt_rand(0, $commCount) ]; //70% 用 通用评论
            ...
            do
            {
                ...
                //取一个随机用户
                $randUser = D('User')->where("uid >= " . mt_rand(0, $maxUid))->find();
        ...
        echo "商品ID：$pid\t 新增销量 $incSalesNum\t件，新增评论 $incCommNum\t条\r\n";

    } else empty($notFoundCountMap[ $catgId ]) ? ($notFoundCountMap[ $catgId ] = 1) : $notFoundCountMap[ $catgId ]++; //统计 未找到的分类
}

echo "\r\n未在comments文件夹文案中 找到 catgId 的评论列表 统计：\r\n";
