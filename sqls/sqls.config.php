...
//2015-07-17 10:32:36
array('time'=>mktimes(1970,1,1,9,05),'type'=>'sql','sql'=>"ALTER TABLE  `{tableprefix}product` ADD  `weight` FLOAT( 10, 2 ) NOT NULL COMMENT  '产品重量，单位：克' AFTER  `market_price`",'des'=>'...'),
...
//2015-06-02 10:27:47
array('time'=>mktimes(1970,1,1,8,04),'type'=>'sql','sql'=>"CREATE TABLE `{tableprefix}attachment_user` (  `id` int(11) NOT NULL auto_increment COMMENT '自增ID',  `uid` int(11) NOT NULL COMMENT 'UID',  `name` varchar(50) NOT NULL COMMENT '名称',  `from` tinyint(1) NOT NULL default '0' COMMENT '0为上传，1为导入，2为收藏',  `type` tinyint(1) NOT NULL default '0' COMMENT '0为图片，1为语音，2为视频',  `file` varchar(100) NOT NULL COMMENT '文件地址',  `size` bigint(20) NOT NULL COMMENT '尺寸，byte字节',  `width` int(11) NOT NULL COMMENT '图片宽度',  `height` int(11) NOT NULL COMMENT '图片高度',  `add_time` int(11) NOT NULL COMMENT '添加时间',  `status` tinyint(4) NOT NULL default '1' COMMENT '状态',  PRIMARY KEY  (`id`)) ENGINE=MyISAM DEFAULT CHARSET=utf8 COMMENT='会员附件表'",'des'=>'...'),
