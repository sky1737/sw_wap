...
class case_controller extends base_controller
{
	...
	{
		if (empty($_POST['id'])) json_return(1003, '请选中一些值');
	...
	private function _ad_edit_content()
	{
		...
		$customField = M('Custom_field')->get_field($this->store_session['store_id'], 'common_ad',
	...
	public function save_ad()
	{
		...
		if ($store->setAd($store_id,
			array('has_ad' => $has_ad, 'ad_position' => $position, 'use_ad_pages' => $pages, 'date_edited' => time()))
		) {
			if (!empty($fields)) {
				if (empty($action)) { //添加
					$result = $custom_field->delete_field($this->store_session['store_id'], 'common_ad',
						$this->store_session['store_id']);
					$custom_field->add_field($this->store_session['store_id'], $fields, 'common_ad',
						$this->store_session['store_id']);
				}
				else { //编辑
					$result = $custom_field->delete_field($this->store_session['store_id'], 'common_ad',
	...
	public function create_page()
	{
		...
		if (IS_POST) {
			...
			if (empty($page_id)) {
				if ($page_id = $custom_page->add(array('store_id' => $store_id, 'name' => $name,
				...
				if ($custom_page->save(array('page_id' => $page_id, 'store_id' => $store_id),
					array('name' => $name, 'add_time' => $_SERVER['REQUEST_TIME']))
				) {
