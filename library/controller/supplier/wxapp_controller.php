...
class wxapp_controller extends base_controller
{
	...
	public function create_account()
	{
		...
		else if (($weixin['service_type_info'] == 0 || $weixin['service_type_info'] == 1) &&
			$weixin['verify_type_info'] == 0
		) {
			$wxtype = 4;
		}
			...
			if (empty($_SESSION['session_id']) || $_SESSION['session_id_expire'] < $now) {
				...
				if ('0' == $result['status']) {
					if (!isset($_SESSION['store']['pigcmsToken']) || $_SESSION['store']['pigcmsToken'] == '') {
						$_SESSION['store']['pigcmsToken'] =
						$pigcmsToken = $this->getToken($_SESSION['store']['store_id']);
						D('Store')->where(array('store_id' => $_SESSION['store']['store_id']))
							->data(array('pigcmsToken' => $pigcmsToken))->save();
					}
