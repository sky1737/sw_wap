...
class attachment_controller extends base_controller{
		...
		if(IS_POST && !empty($url)){
			...
			if(file_put_contents($upload_dir.$filename,$image_content)){
				...
				if($add_result['err_code']){
					...
					json_return(0,array('url'=>getAttachmentUrl($rand_num . $filename),'id'=>$add_result['id']));
				}
			}else{
	...
	public function img_upload(){
		...
		if(!empty($_FILES['file']) && $_FILES['file']['error'] != 4){
			...
			if($upload->upload()){
				...
				die('{"jsonrpc":"2.0","result":{"error_code":0,"url":"'. getAttachmentUrl($rand_num.$uploadList[0]['savename']) .'","id":'.$add_result['id'].'},"id":"'.$dom_id.'"}');
			}else{
	...
	public function attachment_del(){
		if(IS_POST && !empty($_POST['id'])){
			...
			$now_attachment = $database_attachment->field('`id`,`file`')->where($condition_attachment)->find();
	...
	public function attachment_add($name,$file,$size,$from=0,$type=0){
		...
		if($id = D('Attachment')->data($data_attachment)->add()){
	...
	public function getImgList(){
		...
		$count = $database_attachment->where($condition_attachment)->count('id');
		import('user_page');
		$p = new Page($count,27);
		$image_list = $database_attachment->field('`id`,`name`,`file`,`size`,`width`,`height`')->where($condition_attachment)->order('`id` DESC')->limit($p->firstRow.','.$p->listRows)->select();
