...
class trade_controller extends base_controller
{
	...
	public function delivery_load()
	{
		...
		if($_POST['page'] == 'delivery_list') {
			...
			$store_physical =
				D('Store_physical')->where(array('store_id' => $this->store_session['store_id']))->select();
	...
	public function delivery_modify()
	{
		if(IS_POST) {
			...
			$data_postage_template['tpl_name'] =
				!empty($_POST['name']) ? $_POST['name'] : json_return(4091, '模板名称不能为空');
			...
			foreach ($area_arr as $key => $value) {
				if($value != '') {
					...
					$new_area_arr[] = $province . ',' . $area_content_arr[1] . ',' . $area_content_arr[2] . ',' .
						$area_content_arr[3] . ',' . $area_content_arr[4];
				}
			}
			$data_postage_template['tpl_area'] =
				!empty($new_area_arr) ? implode(';', $new_area_arr) : json_return(4093, '配送区域解析失败');
	...
	{
		$selffetch = M('Trade_selffetch')->get_selffetch($_POST['id'], $this->store_session['store_id']);
		if($selffetch) {
	...
	{
		$condition_trade_selffetch['id'] = $_POST['id'];
	...
	public function selffetch_delete()
	{
		...
		$store_id = $this->store_session['store_id'];
		if(D('Trade_selffetch')->where(array('id' => $id, 'store_id' => $store_id))->delete()) {
			json_return(0, '删除成功！');
		}
		else {
...
//	}
	...
	private function _withdraw_content()
	{
		...
		if(!empty($_POST['start_time']) && !empty($_POST['stop_time'])) {
			$where['_string'] = "sw.add_time >= '" . strtotime(trim($_POST['start_time'])) . "' AND sw.add_time <= '" .
				strtotime(trim($_POST['stop_time'])) . "'";
		}
		else if(!empty($_POST['start_time'])) {
	...
	public function buyer_selffetch_name()
	{
		...
		$result = D('Store')->where(array('store_id' => $this->store_session['store_id']))
			->data(array('buyer_selffetch_name' => $buyer_selffetch_name))->save();
