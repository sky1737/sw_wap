...
class setting_controller extends base_controller
{
	...
	public function __construct()
	{
		...
		if(empty($this->store_session))
			redirect(url('user:account:index'));
	}
	...
	public function load()
	{
		...
		switch ($action) {
			...
			case 'user_content':
				$this->_user_content();
				break;
			case 'user_add_content':
				$this->_user_add_content();
				break;
...
//				break;
	...
	//授权管理
	private function _user_content()
	{
//		$store_physical = D('Store_physical')->where(array('store_id' => $this->store_session['store_id']))->select();
//		$this->assign('store_physical', $store_physical);
		$list = D('Store_auth')->where(array('store_id' => $this->store_session['store_id']))->select();
		$this->assign('list', $list);
	}

	private function _user_add_content()
	{
		$qrcode =
			M('Recognition')->get_tmp_qrcode('auth', $this->store_session['store_id'],
	...
	{
		$store_physical =
			D('Store_physical')->where(array('store_id' => $this->store_session['store_id'], 'id' => $_POST['id']))
				->find();
	...
	{
		if(IS_POST) {
			$condition_store_physical['id'] = $_POST['id'];
