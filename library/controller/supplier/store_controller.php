...
class store_controller extends base_controller
{
	...
	public function load()
	{
		...
		switch ($action) {
			...
			case 'wei_page_content': //微页面
				//首页的微杂志
				$home_page = D('Wei_page')->field('`page_id`,`page_name`')
					->where(array('is_home' => 1, 'store_id' => $this->store_session['store_id']))
					->find();
			...
			case 'wei_page_edit': //微页面编辑
				...
				if(!empty($now_page)) {
					if($now_page['has_category']) {
						$cat_arr = D('Wei_page_to_category')->field('`cat_id`')
							->where(array('page_id' => $now_page['page_id']))->order('`id` ASC')->select();
						$cat_ids_arr = array();
					...
					if($now_page['has_custom']) {
						$customField = M('Custom_field')->get_field($this->store_session['store_id'], 'page',
							$now_page['page_id']);
			...
			case 'wei_page_category_edit':
				$now_category =
					M('Wei_page_category')->get_category($this->store_session['store_id'], $_POST['cat_id']);
				if(!empty($now_category)) {
					if($now_category['has_custom']) {
						$customField = M('Custom_field')->get_field($this->store_session['store_id'], 'page_cat',
							$now_category['cat_id']);
						$this->assign('customField', json_encode($customField));
					}
	...
	public function wei_page_add()
	{
		if(IS_POST) {
			...
			if($_POST['custom']) {
				$field_result =
					M('Custom_field')->add_field($this->store_session['store_id'], $_POST['custom'], 'page', $page_id);
	...
	public function wei_page_edit()
	{
		if(IS_POST) {
			...
			if($_POST['custom']) {
				$field_result = M('Custom_field')->add_field($this->store_session['store_id'], $_POST['custom'], 'page',
					$_POST['page_id']);
				if(empty($field_result)) {
					json_return(1001, '页面添加成功！自定义内容添加失败。请刷新页面重试');
				}
	...
	public function wei_page_delete()
	{
		...
		$condition_page['page_id'] = $_POST['page_id'];
		$now_page =
			$database_page->field('`page_id`,`store_id`,`has_category`,`has_custom`')->where($condition_page)->find();
		if(empty($now_page)) {
	...
	public function wei_page_category_add()
	{
		if(IS_POST) {
			...
			if($_POST['custom']) {
				$field_result =
					M('Custom_field')->add_field($this->store_session['store_id'], $_POST['custom'], 'page_cat',
						$cat_id);
	...
	public function wei_page_category_edit()
	{
		if(IS_POST) {
			...
			if($_POST['custom']) {
				$field_result =
					M('Custom_field')->save_field($this->store_session['store_id'], $_POST['custom'], 'page_cat',
						$_POST['cat_id']);
				if(empty($field_result)) {
	...
	public function ucenter()
	{
		...
		if(empty($ucenter)) {
			...
			if($ucenter['has_custom']) {
				$customField = M('Custom_field')->get_field($this->store_session['store_id'], 'ucenter',
					$this->store_session['store_id']);
				$this->assign('customField', json_encode($customField));
			}
		}
	...
	public function ucenter_save()
	{
		...
		if($method_name) {
			if($method_name != 'delete_field') {
				// dump($_POST['custom']);
				$field_result =
					M('Custom_field')->$method_name($this->store_session['store_id'], $_POST['custom'], 'ucenter',
						$this->store_session['store_id']);
			}
			else {
				$field_result = M('Custom_field')->delete_field($this->store_session['store_id'], 'ucenter',
					$this->store_session['store_id']);
			}
	...
	public function create()
	{
		...
		if(option('config.payfor_store') * 1 > 0) {
			pigcms_tips('创建店铺只支持手机操作，请使用用微信扫码按提示操作！');
		}

		$store_count = M('Store')->getStoreCountByUid($_SESSION['user']['uid'], 1);
		if(option('config.user_store_num_limit') > 0 && $store_count >= option('config.user_store_num_limit')) {
			pigcms_tips('您创建的店铺数量已达到限定值了！');
		}
		...
		if(IS_POST) {
			...
			$data['logo'] = $this->user_session['avatar'];
			...
			// 物流
			$data['open_logistics'] = 1;
			$data['offline_payment'] = 1;
			$data['open_friend'] = 1;

//			// 对接店铺
//			if (!empty($_SESSION['sync_store'])) {
...
//			}
			...
			if(!empty($result['err_code'])) {
				...
				if(!$company->getCompanyByUid($this->user_session['uid'])) {
					$company->create(array('uid'      => $data['uid'],
					                       'name'     => $data['company'],
...
//				$sale_category->setStoreInc($data['sale_category_fid']);
				...
				if(!$user_info['admin_id']) {
					...
					if(count($store_list) == 1) {
						$user->save_user(array('uid' => $this->user_session['uid']),
							array('admin_id' => $_SESSION['system']['id']));
					}
				}
...
//					$data_custom_field['field_type'] = 'component';
//					// 调用公共模板的数据
//					$data_custom_field['content'] = serialize(array('name' => '通用模板', 'id' => '5',
//					                                                'url'  => option('config.wap_site_url') .
//						                                                '//component.php?id=5'));
//					$database_custom_field->data($data_custom_field)->add();
////                    $data_custom_field['field_type'] = 'rich_text';
////                    $data_custom_field['content'] = serialize(array('content' => '<p>感谢您使用' . $this->config['site_name'] . '，在' . $this->config['site_name'] . '里，微杂志是您日常使用最频繁的模块之一。它相当于是您的一个自定义页面，您可在这里添加多种信息，向您的粉丝展示内容。</p><p>在微杂志里，您可以多个功能模块，例如“<strong>富文本</strong>”模块。在“富文本”里，对文字进行<strong>加粗</strong>、<em>斜体</em>、<span style="text-decoration:underline;">下划线</span>、<span style="text-decoration:line-through;">删除线</span>、<span style="color:rgb(0,176,240);">文字颜色</span>、<span style="color:rgb(255,255,255);background-color:rgb(247,150,70);">背景色</span>、以及<span style="font-size:22px;">字号大小</span>等简单排版操作。</p><p>也可以在这里，通过编辑器使用表格功能</p><table><tbody><tr class="firstRow"><td width="133" valign="top" style="word-break: break-all;">中奖客户</td><td width="133" valign="top" style="word-break:break-all;">发放奖品</td><td width="133" valign="top" style="word-break:break-all;">备注</td></tr><tr>
////      <td width="133" valign="top" style="word-break:break-all;">爱客</td>
////      <td width="133" valign="top" style="word-break:break-all;">优惠券</td><td width="133" valign="top" style="word-break:break-all;"><span style="color:rgb(255,0,0);">已经发放</span></td></tr><tr><td width="133" valign="top" style="word-break:break-all;">分销</td><td width="133" valign="top" style="word-break:break-all;">积分</td><td width="133" valign="top" style="word-break:break-all;"><span style="color:rgb(0,176,240);">领取地址</span></td></tr></tbody></table><p>还可以通过插入图片、并对图片加上超级链接，方便用户点击，当然也可以选中文字，对文字添加超级链接。<br></p><p>另外，你除了可以在左侧预览模块底部，添加你需要的功能模块外，还可以点击预览区域已添加模块后，进行编辑、删除和在模块后面增加另外需要的功能模块，或者拖动模块，调整顺序。</p><p>再次感谢您选择' . $this->config['site_name'] . '！</p>'));
////                    $database_custom_field->data($data_custom_field)->add();
//				}
...
//        $avatar = $user->getAvatarById($this->user_session['uid']);
	...
	private function _select_content()
	{
		...
		$store_return = M('Store')->getStoresByUid($this->user_session['uid'], 1);
	...
	public function setting()
	{
		if(IS_POST) {
			...
			if(isset($_POST['logo'])) {
				$logo = str_replace(array(option('config.site_url') . '/upload/images/', './upload/images/'), '',
					trim($_POST['logo']));
			}
			else {
				$logo = '';
	...
	public function storenav()
	{
		...
		if(IS_POST) {
			...
			if(empty($nav_info)) {
				$result = $nav->add(array('store_id' => $store_id, 'style' => $style_id, 'bgcolor' => $bgcolor,
				                          'data'     => $nav_menus, 'date_added' => $date_added));
			}
			else {
				$result = $nav->edit(array('store_id' => $store_id),
					array('style'      => $style_id, 'bgcolor' => $bgcolor, 'data' => $nav_menus,
	...
	private function _index_content()
	{
		...
		$where['store_id'] = $this->store_session['store_id'];
		$where['_string'] =
			"visited_time >= '" . strtotime($start_time) . "' AND visited_time <= '" . strtotime($end_time) . "'";
...
//        }
	...
	public function syn_service($store_id)
	{
		...
		curl_post($url,
			array('app_id' => option('config.im_appid'), 'label' => $store_id, 'key' => $skey, 'data' => $post_data));
	}
	...
	public function bind_qrcode()
	{
		...
		if($qrcode_return['error_code']) {
			echo '<html><head></head><body>' . $qrcode_return['msg'] .
	...
	public function select_template()
	{
		if(IS_POST) {
			...
			if(D('Store')->where(array('store_id' => $store_id))->data(array('template_cat_id' => $cat_id,
			                                                                 'template_id'     => $page_id))->save()
			) {
				$cate_info = D('Wei_page_category')->where(array('cat_id' => $cat_id))->find();
				$replace_info =
					D('Wei_page')->where(array('store_id' => $cate_info['store_id'], 'page_id' => $page_id))->find();
					...
					if(!empty($ad_list)) {
						foreach ($ad_list as $key => $field) {
							switch ($field['field_type']) {
								case 'image_ad':
									$data_fields[$key]['store_id'] = $store_id;
									$data_fields[$key]['module_name'] =
										htmlspecialchars($field['module_name'], ENT_QUOTES);
			...
			$offical_list =
				D('Wei_page_category')->where(array('store_id' => $store['store_id']))->order('add_time asc')->select();
