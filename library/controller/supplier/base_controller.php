...
//		}
		...
		if(empty($_SESSION['user'])) {
			if(!empty($_COOKIE['session'])) {
				...
				redirect(url('index:account:login', array('referer' => $_SERVER['REQUEST_URI'])));
			}
		}

		if(empty($_SESSION['store'])) {
			$store = M('Store')->getAllStoresByUid($this->user_session['uid'], 0);
			if($store) {
				//$_SESSION['tmp_store_id'] = $store['store_id'];
				if(empty($store['logo']))
					$store['logo'] = $this->user_session['avatar']; //'default_shop_2.jpg';
...
//		if ($_SESSION['store']['drp_supplier_id']) {
//			if (empty($_SESSION['store']['drp_approve'])) { //未审核店铺不能打开
//				$open_store = false;
//			}
//		} else{
...
//		}
	...
	protected function checkDrp($return = false)
	{
		...
		if(!empty($open_store_drp) &&
			(empty($this->store_session['drp_level']) || $this->store_session['drp_level'] < $max_store_drp_level)
		) { //开启排他分销，并且店铺分销级别在允许分销级别内
