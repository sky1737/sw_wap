...
class cart_controller extends base_controller
{
	...
	public function one()
	{
		...
		// 查出购物车里的产品
		$cart_list = D('')
			->field('c.id, c.store_id, c.product_id, c.pro_num, c.pro_price, c.sku_id, c.sku_data, p.name, p.image')
			->table(array('Product' => 'p', 'User_cart' => 'c'))
			->where("`p`.`product_id` = `c`.`product_id` AND c.uid = '" . $this->user_session['uid'] . "'")->select();
	...
	public function two()
	{
		...
		$user_cart_list =
			D('User_cart')->where(array('id' => array('in', $id), 'uid' => $this->user_session['uid']))->select();
		...
		$agent_id = D('Store')->where('status  = 1 and  agent_id > 1 and uid in (select uid from tp_user where status = 1 and  admin_id > 0)')
			->getField('store_id');
		...
		foreach ($user_cart_list as $value) {
			$product_info = $database_product->field('store_id, original_product_id, weight')
				->where(array('product_id' => $value['product_id']))->find();
			if(!empty($product_info['original_product_id'])) {
				$tmp_product_info = $database_product->field('store_id')
					->where(array('product_id' => $product_info['original_product_id']))->find();
...
//		}
		...
		$condition_user_cart = array('uid' => $this->user_session['uid'], 'id' => array('in', $id));
...
//		Notify::createNoitfy($product['store_id'], option('config.orderid_prefix') . $order_no);
	...
	public function three()
	{
		...
		$suppliers = array();
		foreach ($user_cart_list as $value) {
			$product_info = $database_product->field('store_id, original_product_id')
				->where(array('product_id' => $value['product_id']))->find();
			if(!empty($product_info['original_product_id'])) {
				$tmp_product_info = $database_product->field('store_id')
					->where(array('product_id' => $product_info['original_product_id']))->find();
	...
	public function change()
	{
		...
		$user_cart = D('User_cart')->where(array('uid' => $this->user_session['uid'], 'id' => $id))->find();
		if(empty($user_cart)) {
		...
		// 直接删除
		if($number == 0) {
			D('User_cart')->where(array('uid' => $this->user_session['uid'], 'id' => $id))->delete();
		...
		if($product['buyer_quota']) {
			...
			$order_product_number =
				M('Order_product')->getProductPronum($user_cart['product_id'], $this->user_session['uid']);
		...
		D('User_cart')->data(array('pro_num' => $number))->where(array('uid' => $this->user_session['uid'],
		                                                               'id'  => $id))->save();
