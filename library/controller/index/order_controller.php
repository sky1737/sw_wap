...
class order_controller extends base_controller
{
	...
	function add()
	{
		...
		//验证商品
		$product = D('Product')
			->field('`product_id`,`store_id`,`price`,`has_property`,`status`,`supplier_id`,`quantity`, `buyer_quota`, `weight`')
			->where(array('product_id' => $product_id))->find();
		...
		if($product['buyer_quota']) {
			...
			if($_POST['type'] != 'add') {
				$user_cart_num = D('User_cart')->field('pro_num')->where(array('uid'        => $this->user_session['uid'],
				                                                               'product_id' => $product_id))->find();
		...
		if(empty($product['has_property'])) {
			...
			//判断库存是否存在
			$nowSku = D('Product_sku')->field('`sku_id`,`product_id`,`properties`,`price`, `quantity`')
				->where(array('sku_id' => $sku_id))->find();
			...
			if(count($properties) == 1) {
				$findPropertiesArr = D('Product_property')->field('`pid`,`name`')
					->where(array('pid' => $properties[0]))->select();
				$findPropertiesValueArr = D('Product_property_value')->field('`vid`,`value`')
					->where(array('vid' => $propertiesValue[0]))
					->select();
			}
			else {
				$findPropertiesArr = D('Product_property')->field('`pid`,`name`')
					->where(array('pid' => array('in', $properties)))
					->select();
				$findPropertiesValueArr = D('Product_property_value')->field('`vid`,`value`')
					->where(array('vid' => array('in', $propertiesValue)))->select();
			}
...
//		}
		...
		if($_POST['type'] == 'add') {    //立即购买
			...
			$agent_id = D('Store')
				->where('status = 1 and agent_id > 1 and uid in (select uid from tp_user where status = 1 and  admin_id > 0)')
				->getField('store_id');
			...
			if(D('Order_product')->data($data_order_product)->add()) {
				...
				echo json_encode(array('status' => true, 'msg' => '订单添加成功',
				                       'data'   => array('order_no' => option('config.orderid_prefix') . $order_no)));
				// 产生提醒
...
//				Notify::createNoitfy($product['store_id'], option('config.orderid_prefix') . $order_no);
		...
		// 添加购物车
			...
			// 查找购物车里是否有相应的产品
			$user_cart =
				D('User_cart')->where(array('uid'        => $this->user_session['uid'],
				                            'product_id' => $product['product_id'],
				                            'store_id'   => $product['store_id'],
				                            'sku_id'     => $sku_id))->find();
			if(!empty($user_cart)) {
				if(D('User_cart')->where(array('id' => $user_cart['id']))->setInc('pro_num', $quantity)) {
					// 查找购物车数量
					$pro_num = D('User_cart')->where(array('uid' => $this->user_session['uid']))->sum('pro_num');

					echo json_encode(array('status' => true, 'msg' => '添加购物车成功',
					                       'data'   => array('number' => $pro_num)));
					exit;
				}
				else {
	...
	function address()
	{
		...
		$order_store = M('Store')->getStore($order['store_id']);
		//店铺资料
		...
		$offline_payment = $order_store['offline_payment'];
		...
		if(IS_POST) {
			...
			// 是否支付货到付款
			if((!$order_store['offline_payment'] || !$is_all_selfproduct) && $_POST['payment_method'] == 'off') {
			...
			if(!empty($address_id) || !empty($selffetch_id) || $shipping_method == 'friend') {
				...
				if($shipping_method == 'selffetch') {
					...
					// 判断是否为全部自营产品
					if(!$is_all_selfproduct || !$order_store['buyer_selffetch']) {
						$buyer_selffetch_name =
							$order_store['buyer_selffetch_name'] ? $order_store['buyer_selffetch_name'] : '上门自提';
					...
					if(strpos($selffetch_id, 'store')) {
						...
						if(!empty($selffetch)) {
							...
							$selffetch['tel'] =
								($selffetch['phone1'] ? $selffetch['phone1'] . '-' : '') . $selffetch['phone2'];
...
//							// 积分
//							if($reward['score'] > 0) {
//								M('Store_user_data')->changePoint($order['store_id'], $this->user_session['uid'],
//									$reward['score']);
//							}
//
//							// 送赠品
...
//						$trade_no = date('YmdHis', $_SERVER['REQUEST_TIME']) . mt_rand(100000, 999999);
						...
						$data['total'] =
						$data['pay_money'] = max(0, $order['sub_total'] + $order['postage'] + $order['float_amount']);
						...
						echo json_encode(array('status' => true, 'msg' => '确认收货地址成功',
						                       'data'   => array('nexturl' => url('order:pay',
		...
		$address_list = '';
		if($order_store['open_logistics']) {
			$address_list = M('User_address')->getMyAddress($this->user_session['uid']);
		}

		//上门自提
		if($order_store['buyer_selffetch'] && $is_all_selfproduct) {
			$selffetch_list = array();// M('Trade_selffetch')->getListNoPage($now_store['store_id']);

			$store_contact = M('Store_contact')->get($order_store['store_id']);
			$store_physical = M('Store_physical')->getList($order_store['store_id']);

			if($store_contact) {
				$data = array();
				$data['id'] = '99999999_store';
				$data['name'] = $order_store['name'] . '';
				$data['tel'] =
					($store_contact['phone1'] ? $order_store['phone1'] . '-' : '') . $store_contact['phone2'];
			...
			//$selffetch_list = M('Trade_selffetch')->getListNoPage($order_store['store_id']);
