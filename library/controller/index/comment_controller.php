...
class comment_controller extends base_controller{
	...
	public function add() {
		...
		if (!empty($images_id_str)) {
			...
			if (!empty($images_id_arr)) {
				$attachment_user_list = M('Attachment_user')->getList(array('id' => array('in', $images_id_arr), 'uid' => $this->user_session['uid']));
			}
		}
		...
		// 更改相应订单的产品为已评论
		if ($type == 'PRODUCT') {
			D('Order_product')->where(array('id' => $order_product['id']))->data(array('is_comment' => 1))->save();
		}
	...
	public function attachment() {
		...
		if(!empty($_FILES['file']) && $_FILES['file']['error'] != 4){
			...
			if($upload->upload()) {
				...
				$id = $this->_attachmentAdd($uploadList[0]['name'], $rand_num . $uploadList[0]['savename'], $uploadList[0]['size']);
				if(!$id){
					...
					echo json_encode(array('status' => true, 'msg' => '上传成功', 'data' => array('id' => $id, 'file' => getAttachmentUrl($rand_num . $uploadList[0]['savename']))));
					exit;
				}
			} else {
				echo json_encode(array('status' => false, 'msg' => '图片上传失败'));
	...
	private function _attachmentAdd($name, $file, $size, $from=0, $type=0){
		...
		if($id = M('Attachment_user')->add($data)) {
			return $id;
		} else {
			return false;
		}
