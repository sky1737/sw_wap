...
class store_controller extends base_controller{
	...
	public function index(){
		...
		$store['qrcode'] = M('Recognition')->get_tmp_qrcode('store', $id, $id);
...
//		$this->assign('reward_list', $reward_list);
	...
	public function goods() {
		...
		//if (!empty($store_id)) {
			$product_model = M('Product');
		if($self)
			$where = "`status` = 1 and `store_id` = {$store_id} and `is_fx` != 1";
		else
			$where = "`status` = 1 and ((`is_fx` = 1) OR (`store_id` = {$store_id} and `is_fx` != 1))";
