...
//			redirect(url('account:login', array('referer' => url('account:index'))));
//			exit;
//		}
//
//		if(IS_POST) {
...
//
//				// 产生目录结构
//				$rand_num = 'images/' . substr($img_path_str, 0, 3) . '/' . substr($img_path_str, 3, 3) . '/' .
//					substr($img_path_str, 6, 3) . '/' . date('Ym', $_SERVER['REQUEST_TIME']) . '/';
//
...
//					$add_result = $this->attachment_add($uploadList[0]['name'], $rand_num . $uploadList[0]['savename'],
//						$uploadList[0]['size']);
...
//							upyunUser::upload('./upload/' . $data['avatar'],
//								'/' . $rand_num . $uploadList[0]['savename']);
//						}
//					}
//				}
//			}
...
//	{
//		if(empty($this->user_session)) {
//			redirect(url('account:login', array('referer' => url('account:order'))));
//			exit;
//		}
//
//		// 基本参数设定
...
//		if($count > 0) {
//			$order_list =
//				$order_model->getOrders(array('uid' => $this->user_session['uid']), 'order_id desc, status asc',
//					$offset, $limit);
//
//			// 将相应的产品放到订单数组里
...
////	{
////		if(empty($this->user_session)) {
////			redirect(url('account:login', array('referer' => url('account:rebate'))));
////			exit;
////		}
////
////		// 基本参数设定
...
//	{
//		if(empty($this->user_session)) {
//			redirect(url('account:login', array('referer' => url('account:password'))));
//			exit;
//		}
//
//		// post，进行更改密码
...
//	}
	...
	{
		if(empty($this->user_session)) {
			redirect(url('account:login', array('referer' => url('account:address'))));
			exit;
		}
		...
		if(IS_POST) {
			...
			echo json_encode(array('status' => true, 'msg' => $msg,
			                       'data'   => array('nexturl' => 'refresh', 'address' => $data)));
			exit;
		}
...
//			$save_result = $database_user->save_user(array('uid' => $user['uid']),
//				array('login_count' => $user['login_count'] + 1, 'last_time' => $_SERVER['REQUEST_TIME'],
...
//			                       'data'   => array('nexturl' => option('config.site_url'))));
//			exit;
//		}
//
//		$referer = $_GET['referer']
...
//		}
//		if($id = D('Attachment_user')->data($data)->add()) {
//			return array('err_code' => 0, 'id' => $id);
//		}
//		else {
...
//				->where("`uc`.`type` = '2' AND `uc`.`user_id` = '" . $this->user_session['uid'] .
//					"' AND `uc`.`dataid` = `s`.`store_id`")->order("`uc`.`collect_id` DESC")->limit($offset . ',' .
//					$limit)->select();
...
//		if(empty($this->user_session)) {
//			$referer = url('user:account:index');
//			redirect(url('account:login', array('referer' => $referer)));
//			exit;
//		}
//		if(empty($id)) {
...
//
//				$product_list =
//					$coupon_to_prodcut->getSellingCouponProduct($where, $order_by_field, $order_by_method, $offset,
//						$limit);
//				import('source.class.user_page');
//				$user_page = new Page($count, $limit, $page);
//				$pages = $user_page->show();
...
//			                       'data'   => array('nexturl' => url("account:coupon"))));
//			exit;
//		}
//		else {
