...
class goods_controller extends base_controller
{
	...
	public function index()
	{
		...
		//print_r($this->store_session['store_id']);
		$this->product['qrcode'] =
			M('Recognition')->get_tmp_qrcode('product', $this->product['product_id'], $this->store_session['store_id']);
		//print_r($this->product['qrcode']);
		//option('config.site_url') . '/source/qrcode.php?type=good&id=' . $this->product['product_id'];
		$is_preview = $_GET['is_preview'];

		$this->product['point'] = 0;
		$this->product['profit'] = 0.00;
		...
		if($this->product['has_property']) {
			...
			$product_sku_list = D('Product_sku')->field('`sku_id`, `properties`, `quantity`, `price`')
				->where(array('product_id' => $this->id))->order('`sku_id` ASC')->select();
			...
			$property_list = D('')->field('`pp`.`pid`,`pp`.`name`')->table(array('Product_to_property' => 'ptp',
			                                                                     'Product_property'    => 'pp'))
				->where("`ptp`.`product_id`='" . $this->id . "' AND `pp`.`pid`=`ptp`.`pid`")->order('`ptp`.`id` ASC')
				->select();
			if(!empty($property_list)) {
				$property_value_list = D('')->field('`ppv`.`vid`,`ppv`.`value`,`ppv`.`pid`, `ppv`.`image`')
					->table(array('Product_to_property_value' => 'ptpv', 'Product_property_value' => 'ppv'))
					->where("`ptpv`.`product_id`='" . $this->id . "' AND `ppv`.`vid`=`ptpv`.`vid`")
					->order('`ptpv`.`id` ASC')->select();
...
//		}
//		else {
		$store_contact = M('Store_contact')->get($this->store_session['store_id']);
//		}

		// 设置浏览记录，只记录产品的product_id,只记录最近5条记录
		$history_product_list = cookies::get('history_product_list');
		if(empty($history_product_list)) {
			//setcookie('history_product_list', $this->id, $_SERVER['REQUEST_TIME'] + 86400 * 365, '/');
			cookies::put('history_product_list', $this->id, 365);
		}
		else {
			...
			cookies::put('history_product_list', $product_id_str, 365);
		}

		// 更改浏览次数
		M('Product')->analytics(array('product_id' => $this->id));
//		// 主营分类
//		$sale_category = M('Store')->getSaleCategory($this->store_session['store_id'], $this->product['uid']);
		// 检查是否已经收藏
		$user_collect = array('click' => 'userCollect', 'title' => '收藏');
		if(!empty($this->user_session)) {
			$collect = D('User_collect')
				->where(array('type' => 1, 'user_id' => $this->user_session['uid'], 'dataid' => $this->id))
				->find();
...
//		}
		...
		$this->assign('imUrl', getImUrl($_SESSION['user']['uid'], $this->store_session['store_id']));
