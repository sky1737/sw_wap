...
class weixin_controller extends base_controller
{
	...
	{
		$count = D('Source_material')->where(array('store_id' => $this->store_session['store_id']))->count('id');
		import('source.class.user_page');
		$p = new Page($count, 10);
		$list = D('Source_material')->where(array('store_id' => $this->store_session['store_id']))->order('id DESC')->limit($p->firstRow.','.$p->listRows)->select();
		$it_ids = array();
		$temp = array();
		foreach ($list as $l) {
		...
		$result = array();
		$image_text = D('Image_text')->field('id, title')->where(array('id' => array('in', $it_ids)))->order('id asc')->select();
		foreach ($image_text as $txt) {
			if (!isset($result[$temp[$txt['id']]['id']])) {
				$result[$temp[$txt['id']]['id']] = $temp[$txt['id']];//$txt;
			}
			$result[$temp[$txt['id']]['id']]['list'][] = $txt;
		}
	...
	public function one()
	{
		...
		$image_text = array('title' => '标题', 'cover_pic' => '', 'author' => '', 'content' => '', 'digest' => '', 'url' => '', 'dateline' => time(), 'id' => 0);
		if ($data = D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find()) {
			$it_ids = unserialize($data['it_ids']);
			$id = isset($it_ids[0]) ? intval($it_ids[0]) : 0;
			$image_text = D('Image_text')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find();
		}
		$this->assign('id', $id);
	...
	public function multi()
	{
		...
		if ($data = D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find()) {
			$it_ids = unserialize($data['it_ids']);
			$image_text = D('Image_text')->where(array('id' => array('in', $it_ids)))->order('id asc')->select();
			$contents  = array();
			foreach ($image_text as $t) {
				$t['id'] = $t['id'];
				unset($t['store_id'], $t['dateline'], $t['id']);
				$contents[] = $t;
			}
		}
	...
	public function fetch_wentu()
	{
		...
		if ($data = D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find()) {
			...
			if ($data['type'] == 0) {
				$id = isset($it_ids[0]) ? intval($it_ids[0]) : 0;
				$image_text = D('Image_text')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find();
				$image_text['info_url'] = $this->config['wap_site_url'] . '/imagetxt.php?id=' . $image_text['id'];
				$image_text['date'] = date('Y-m-d');
			} else {
				$image_text = D('Image_text')->where(array('id' => array('in', $it_ids)))->order('id asc')->select();
				foreach ($image_text as &$val) {
					$val['info_url'] = $this->config['wap_site_url'] . '/imagetxt.php?id=' . $val['id'];
				}
			}
			exit(json_encode(array('error' => 0, 'data' => $image_text, 'type' => $data['type'], 'id' => $id)));
		} else {
	...
	public function save_image_text()
	{
		...
		if ($id && $thisid) {
			if (D('Image_text')->where(array('id' => $thisid, 'store_id' => $this->store_session['store_id']))->data($data)->save()) {
				D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->data(array('it_ids' => serialize(array($thisid)), 'store_id' => $this->store_session['store_id'], 'dateline' => time()))->save();
	...
	public function save_multi_image_text()
	{
		...
		foreach ($data as $d) {
			...
			if ($d['id'] && $id) {
				if (D('Image_text')->where(array('id' => $d['id'], 'store_id' => $this->store_session['store_id']))->data($d)->save()) {
					$ids[] = $d['id'];
				} else {
					$flag = true;
					break;
		...
		if ($flag) {
			if ($delete_ids) {
				D('Image_text')->where(array('store_id' => $this->store_session['store_id'], 'id' => array('in', $delete_ids)))->delete();
			}
			exit(json_encode(array('error_code' => 1, 'msg' => '操作失败稍后重试！', 'index' => -1)));
		} else {
			if ($id) {
				D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->data(array('it_ids' => serialize($ids), 'store_id' => $this->store_session['store_id'], 'dateline' => $now_time, 'type' => 1))->save();
			} else {
	...
	{
		$id = isset($_GET['id']) ? intval($_GET['id']) : 0;
		if ($source_material = D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find()) {
			D('Image_text')->where(array('id' => array('in', unserialize($source_material['it_ids'])), 'store_id' => $this->store_session['store_id']))->delete();
			D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->delete();
	...
	public function get_wentu()
	{
		...
		$count = D('Source_material')->where($where)->count('id');
		import('source.class.user_page');
		$p = new Page($count, 3);
		$list = D('Source_material')->where($where)->order('id DESC')->limit($p->firstRow.','.$p->listRows)->select();
		$it_ids = array();
		$temp = array();
		foreach ($list as $l) {
		...
		$image_text = D('Image_text')->field('id, title')->where(array('id' => array('in', $it_ids)))->order('id asc')->select();
		foreach ($image_text as $txt) {
			$txt['info_url'] = $this->config['wap_site_url'] . '/imagetxt.php?id=' . $txt['id'];
			if (!isset($result[$temp[$txt['id']]['id']])) {
				$result[$temp[$txt['id']]['id']] = $temp[$txt['id']];//$txt;
			}
			$result[$temp[$txt['id']]['id']]['list'][] = $txt;
	...
	public function auto_load()
	{
		...
		$count = D('Rule')->where($where)->count('id');
		import('source.class.user_page');
		$p = new Page($count, 10);
		$list = D('Rule')->where($where)->order('id DESC')->limit($p->firstRow.','.$p->listRows)->select();
		$rids = array();
		foreach ($list as $l) {
			$rids[] = $l['id'];
		}
...
// 				if ($re['type'] == 0) {
				...
				${"v{$re['type']}_list"}[$re['cid']]['reply'] = $re['id'];//内容id 对应规则id
// 				} elseif ($re['type'] == 1) {
// 					$v1_list[$re['cid']]['rule'] = $re['rid'];//内容id 对应规则id
// 					$v1_list[$re['cid']]['reply'] = $re['id'];//内容id 对应规则id
// 				}
			}
			//0:文本
			if (isset($c_ids[0]) && $c_ids[0]) {
				$texts = D('Text')->where(array('store_id' => $this->store_session['store_id'], 'id' => array('in', $c_ids[0])))->select();
				foreach ($texts as $text) {
					$text['show_content'] = str_replace($this->face, $this->face_image, $text['content']);
					$text['reply_type'] = 0;
					$text['cid'] = $text['id'];
					$text['reply_id'] = $v0_list[$text['id']]['reply'];
					$r_list[$v0_list[$text['id']]['rule']][] = $text;
				}
			}
			//1:图文
			if (isset($c_ids[1]) && $c_ids[1]) {
				$lists = D('Source_material')->where(array('id' =>  array('in', $c_ids[1]), 'store_id' => $this->store_session['store_id']))->select();
				$temp = array();// array(image_text_id => source_material_id, ...)
				foreach ($lists as $l) {
					$tids = unserialize($l['it_ids']);
					if (isset($tids[0]) && $tids[0]) {
						$it_ids[] = $tids[0];
						$temp[$tids[0]] = $l['id']; //cid  sid
					}
				}
				
				if ($it_ids) {
					$image_text_list = array();
					$image_texts = D('Image_text')->where(array('id' => array('in', $it_ids), 'store_id' => $this->store_session['store_id']))->select();
					foreach ($image_texts as $image) {
						$image['info_url'] = $this->config['wap_site_url'] . '/imagetxt.php?id=' . $image['id'];
						$image_text_list[$temp[$image['id']]] = $image;
					}
			...
			foreach ($list as &$li) {
				$li['keylist'] = isset($k_list[$li['id']]) ? $k_list[$li['id']] : array();
				$li['vallist'] = isset($r_list[$li['id']]) ? $r_list[$li['id']] : array();
			}
		}
...
// 		print_r($list);die;
	...
	public function auto_reply_load()
	{
		...
		$keywords = D('Keyword')->where(array('store_id' => $this->store_session['store_id'], 'from_id' => $rule['id']))->select();
		$k_list = array();// = array(rid => array(), ...)
		foreach ($keywords as $key) {
			$key['show_content'] = str_replace($this->face_key, $this->face_image, $key['content']);
			if (isset($k_list[$key['from_id']])) {
		...
		$reply_relations = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'rid' => $rule['id']))->select();
		foreach ($reply_relations as $re) {
			$c_ids[$re['type']][] = $re['cid'];
			${"v{$re['type']}_list"}[$re['cid']]['rule'] = $re['rid'];//内容id 对应规则id
			${"v{$re['type']}_list"}[$re['cid']]['reply'] = $re['id'];//内容id 对应规则id
...
// 		foreach ($list as &$li) {
		$rule['keylist'] = isset($k_list[$rule['id']]) ? $k_list[$rule['id']] : array();
		$rule['vallist'] = isset($r_list[$rule['id']]) ? $r_list[$rule['id']] : array();
// 		}
...
// 		$this->assign('page', $p->show());
	...
	public function save_rule()
	{
		...
		if ($rule = D('Rule')->where("`store_id`='{$this->store_session['store_id']}' AND `name`='{$name}' AND `id`<>'{$id}'")->find()) {
			json_return(1, '规则名已存在');
		}
		$now_time = time();
		if($id && ($rule = D('Rule')->where("`store_id`='{$this->store_session['store_id']}' AND `id`='{$id}'")->find())) {
			if ($flag = D('Rule')->where(array('store_id' => $this->store_session['store_id'], 'id' => $id))->data(array('name' => $name, 'dateline' => $now_time))->save()) {
				exit(json_encode(array('err_code' => 0, 'err_msg' => '规则名修改成功', 'ruleid' => $id, 'iskeyword' => 0)));
	...
	public function delete_rule()
	{
		...
		if ($rule = D('Rule')->where("`store_id`='{$this->store_session['store_id']}' AND `id`='{$rid}'")->find()) {
...
// 				$v_list[$re['cid']] = $re['rid'];//内容id 对应规则id
			...
			if ($txtids) {
				D('Text')->where(array('store_id' => $this->store_session['store_id'], 'id' => array('in', $txtids)))->delete();
			}
			D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'rid' => $rid))->delete();
			D('Rule')->where("`store_id`='{$this->store_session['store_id']}' AND `id`='{$rid}'")->delete();
	...
	public function save_keyword()
	{
		...
		$rule = D('Rule')->where(array('store_id' => $this->store_session['store_id'], 'id' => $rid))->find();
		if (empty($rule))json_return(1, '不存在的规则');
		if ($keyword = D('Keyword')->where("`store_id`='{$this->store_session['store_id']}' AND `content`='{$content}' AND `id`<>'{$kid}'")->find()) {
			json_return(1, '关键词已经存在');
		} else {
			$keyword = str_replace($this->face_key, $this->face_image, $content);
			if ($kid) {
				D('Keyword')->where(array('store_id' => $this->store_session['store_id'], 'id' => $kid))->data(array('content' => $content))->save();
				
				exit(json_encode(array('err_code' => 0, 'err_msg' => '关键词修改成功', 'kid' => $kid, 'keyword' => $keyword)));
				json_return(0, '关键词修改成功');
			} else {
				$count = D('Keyword')->where(array('store_id' => $this->store_session['store_id'], 'from_id' => $rid))->count('id');
				if ($count >= 20) {
	...
	public function delete_keyword()
	{
		...
		D('Keyword')->where(array('store_id' => $this->store_session['store_id'], 'id' => $kid))->delete();
	...
	public function save_reply()
	{
		...
		$rule = D('Rule')->where(array('store_id' => $this->store_session['store_id'], 'id' => $rid))->find();
		if (empty($rule))json_return(1, '不存在的规则');
		$count = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'rid' => $rid))->count('id');
		if ($count >= 10) json_return(1, '最多十条回复');
		$keyword = '';
		$relation = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'id' => $replyid))->find();
		...
		switch ($type) {
			case 0:
				...
				if ($relation) {
					if ($relation['type'] == 0) {
						$cid = $relation['cid'];
						D('Text')->where(array('store_id' => $this->store_session['store_id'], 'id' => $relation['cid']))->data(array('content' => $content))->save();
			...
			case 1:
// 				$cid = isset($_POST['sid']) ? intval($_POST['sid']) : 0;
				if ($source_material = D('Source_material')->where(array('id' => $cid, 'store_id' => $this->store_session['store_id']))->find()) {
					$tids = unserialize($source_material['it_ids']);
					if (isset($tids[0]) && $tids[0]) {
						$image_text = D('Image_text')->where(array('id' => $tids[0], 'store_id' => $this->store_session['store_id']))->find();
						$keyword = $image_text['title'];
						$info_url = $this->config['wap_site_url'] . '/imagetxt.php?id=' . $image_text['id'];
					}
				} else {
...
// 				}
		...
		if ($cid) {
			if ($type && $type != 2) {
				if ($relation['type'] == 0) {
					D('Text')->where(array('store_id' => $this->store_session['store_id'], 'id' => $relation['cid']))->delete();
				} elseif ($relation['type'] == 2);
			}
			if ($relation) {
				D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'id' => $replyid))->data(array('cid' => $cid, 'rid' => $rid, 'type' => $type))->save();
				$err_msg = '回复内容修改成功';
			} else {
	...
	public function delete_reply()
	{
		...
		$rule = D('Rule')->where(array('store_id' => $this->store_session['store_id'], 'id' => $rid))->find();
		if (empty($rule))json_return(1, '不存在的规则');
		$relation = D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'id' => $replyid, 'rid' => $rid))->find();
		...
		D('Reply_relation')->where(array('store_id' => $this->store_session['store_id'], 'id' => $replyid, 'rid' => $rid))->delete();
...
// 	}
	...
	public function save_menu_content()
	{
		...
		if ($id && ($diy_menu = D('Diymenu_class')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find())) {
			...
			if ($type == 1) {
				$image_text = array();
				if ($data = D('Source_material')->where(array('id' => $fromid, 'store_id' => $this->store_session['store_id']))->find()) {
					$it_ids = unserialize($data['it_ids']);
					$image_text = D('Image_text')->where(array('id' => array('in', $it_ids)))->order('id asc')->select();
// 					$contents  = array();
// 					foreach ($image_text as $t) {
...
// 					}
	...
	public function send()
	{
		...
		if ($type == -1) {
			...
			$url && $url = TWIKER_PATH . substr($url, 2);
			...
			if ($data = D('Source_material')->where(array('id' => $id, 'store_id' => $this->store_session['store_id']))->find()) {
				...
				if ($data['type'] == 0) {
					...
					if ($image_text['cover_pic']) {
						$url = TWIKER_PATH . substr($image_text['cover_pic'], 1);
					...
					foreach ($image_text as $image) {
						$media['media_id'] = '';
						if ($image['cover_pic']) {
							$url = TWIKER_PATH . substr($image['cover_pic'], 1);
							$media = Http::curlPost($upload_url, array('media' => '@' . $url));
							if ($media['errcode']) {
								exit(json_encode($media));
							}
