...
class order_product_model extends base_model{
	...
	public function orderProduct($order_id,$getPro=true){
		if($getPro){
			$orderProduct = D('')->table(array('Product'=>'p','Order_product'=>'op'))->where("`op`.`order_id`='$order_id' AND `op`.`product_id`=`p`.`product_id`")->order('`op`.`id` ASC')->select();
		}else{
			$orderProduct = $this->db->where(array('order_id'=>$order_id))->order('`id` ASC')->select();
		}
