...
class product_qrcode_activity_model extends base_model
{
	...
	{
		return $this->db->where(array('store_id' => $store_id, 'product_id' => $product_id, 'id' => $activity_id))->delete();
	}
	...
	{
		$activity = $this->db->where(array('id' => $activity_id, 'product_id' => $product_id, 'store_id' => $store_id))->find();
		return $activity;
	}

	public function getActivityById($activity_id)
	{
		$activity = $this->db->where(array('id' => $activity_id))->find();
		return $activity;
	}
}
