...
class trade_selffetch_model extends base_model
{
	...
	{
		$list_count = $this->db->where(array('store_id' => $store_id))->count('id');
		import('source.class.user_page');
		$p = new Page($list_count, 15);
		$selffetch_list = $this->db->where(array('store_id' => $store_id))->order('`id` DESC')->limit($p->firstRow . ',' . $p->listRows)->select();
	...
	{
		$selffetch_list = $this->db->where(array('store_id' => $store_id))->order('`id` DESC')->select();
	...
	public function get_selffetch($id, $store_id)
	{
		$condition_trade_selffetch['id'] = $id;
