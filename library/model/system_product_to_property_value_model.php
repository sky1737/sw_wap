...
class system_product_to_property_value_model extends base_model
{
	...
	{
		$vids =
			$this->db->field($fields)->where(array('pid' => $pid, 'product_id' => $product_id))->order('order_by ASC')
				->select();
	...
	public function getProductIDByVid($vid_arr)
	{
		...
		$sql = "SELECT `product_id` FROM (SELECT `product_id`, count(product_id) as count FROM `" . $prefix .
			"system_product_to_property_value` WHERE `vid` IN (" . $vid_str .
			") GROUP BY `product_id` ORDER BY `id` DESC) as t WHERE t.count = " . count($vid) . " LIMIT 2000";
		...
		if (empty($data)) {
			...
			$sql = "replace into " . $prefix . "search_tmp (`md5`, `product_id_str`, `expire_time`) values ('" . $md5 .
				"', '" . $product_id_str . "', '" . $expire_time . "')";
			$this->db->execute($sql);
