	...
	public function get_access_token()
	{
		...
		if (!$this->checkTokenExpires($access_obj['access_token']) && $access_obj && intval($access_obj['expires_in']) > $now) {
			...
			$url = 'https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=' .
				trim(option('config.wechat_appid')) . '&secret=' . trim(option('config.wechat_appsecret'));
			...
			if ($json->errmsg) {
				...
				if ($access_obj) {
					D('Access_token')->where(array('id' => $access_obj['id']))
						->data(array('access_token' => $json->access_token,
						             'expires_in'   => intval($json->expires_in + $now)))->save();
				}
				else {
					D('Access_token')->data(array('access_token' => $json->access_token,
					                                      'expires_in'   => intval($json->expires_in + $now)))->add();
				}
