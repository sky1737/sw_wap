...
class product_property_value_model extends base_model {
	...
	public function getProductIDByVid($vid_arr) {
		...
		$sql = "SELECT `product_id` FROM (SELECT `product_id`, count(product_id) as count FROM `" . $prefix . "product_to_property_value` WHERE `vid` IN (" . $vid_str . ") GROUP BY `product_id` ORDER BY `id` DESC) as t WHERE t.count = " . count($vid) . " LIMIT 2000";
