...
class store_physical_model extends base_model{
	public function getOne($id){
        $store_physical = $this->db->where(array('id'=>$id))->find();
	...
	public function getListByIDList($physical_id_list) {
		...
		//$store_physical_list = $this->db->where(array('id' => array('in', $physical_id_list)))->select();
		
		$store_physical_list = D('')->field("`s`.`buyer_selffetch_name`, `sp`.*")->table(array('Store' => 's', 'Store_physical' => 'sp'))->where("`s`.`store_id` = `sp`.`store_id` AND `sp`.`id` in (" . join(',', $physical_id_list) . ")")->select();
