...
class product_model extends base_model
{
	 ...
	 * @param $order_by_method 排序方式
	 * @param $offset
	 * @param $limit
	 * @param string $is_show_distance (0:不调取距离，1:调取距离，)
	 * @return array
	 */
	 ...
	 * @param    $where string
	 * @param    $order_by_field 排序字段
	 * @param    $order_by_method 排序方式 ASC DESC
	 * @param    $offset
	 * @param    $limit
			...
			if($pageCount > 1) {
				if($page == 1) {
					...
					$pagebar = '<a href="./search.php?store_id=' . $store_id . '&q=' . urlencode($key) . '&page=' .
						($page - 1) . '" class="custom-paginations-prev">下一页</a>';
				}
				if($page >= $pageCount) {
					...
					$pagebar .= '<a href="./search.php?store_id=' . $store_id . '&q=' . urlencode($key) . '&page=' .
						($page + 1) . '" class="custom-paginations-next">下一页</a>';
				}
