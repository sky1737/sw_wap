...
class user_income_model extends base_model
{
    ...
    function __construct($model)
    {
        ...
        $this->type_data = array(-3 => '提现', -2 => '活动使用', -1 => '购物使用', 1 => '购物立返', 2 => '一级分销返佣', 3 => '二级分销返佣', 4 => '一级代理利润', 5 => '二级代理利润', 6 => '物流费用', 7 => '推荐奖励', 8 => '活动奖励', 9 => '管理员充值', 10 => '退货返还', 11 => '成本结算', 12 => '积分兑换', 99 => '充值');
    }

    public function typeTxt($type)
    {
        if (!$type) return $this->type_data;
    ...
    {
        $this->db->data(array('uid' => $order['uid'], 'order_no' => $order['order_no'], 'income' => $order['balance'] * -1, 'point' => $order['point'] * -1, 'type' => -1, 'add_time' => time(), 'status' => 1, 'remarks' => '购物余额支付和积分抵现。'))->add();
        ...
        if ($order['balance'] > 0)
        {
            Notify::getInstance()->accountChange($_SESSION['user']['openid'], option('config.wap_site_url') . '/balance.php?a=index', '购物消费使用余额支付', date("Y/m/d H:i", $order['add_time']), $order['balance'], '现金', $_SESSION['user']['balance'], '使用账户余额支付成功，点击查看详情');
        }
        if ($order['point'] > 0)
        {
            Notify::getInstance()->accountChange($_SESSION['user']['openid'], option('config.wap_site_url') . '/balance.php?a=index', '购物消费使用积分抵现', $order['add_time'],//date('Y-m-d h:i:s', ),
                $order['point'], '积分', $_SESSION['user']['point'], '使用积分抵现支付成功，点击查看详情');
        }
    }
    ...
    {
        //如果不是 已 支付 或者 已返
        if ($order['status'] != 2 || $order['profit_status'] != 0) return;
        ...
        import('source.class.Notify');
        $this->db->data(array('uid' => $supplier_uid, 'order_no' => $order['order_no'], 'income' => $factory_price, 'point' => 0, 'type' => 10, 'add_time' => time(), 'status' => 1, 'remarks' => '供应商成本立返'))->add();

        $supplier_user = D('User')->where(array('uid' => $supplier_uid))->find();
        Notify::getInstance()->accountChange($supplier_user['openid'], option('config.wap_site_url') . '/balance.php?a=index', '供应商您好，订单成本现金已到账', date('Y-m-d H:i:s', $order['add_time']), $factory_price, '现金', $supplier_user['balance'] + $factory_price, "返还成本的订单号：{$order['order_no']}");
        ...
        Notify::getInstance()->orderUpdate($_SESSION['user']['openid'], option('config.wap_site_url') . '/order.php?orderid=' . $order['order_id'], '你好，订单已支付成功', $order['order_no'], '支付成功，待发货', '您的订单已支付成功，已通知供商发货啦！');

        $agent_openid = $db_user->where("`uid` = (select `uid` from `tp_store` where `store_id` =
			{$order['agent_id']} and `status` = 1)")->getField('openid');
        if ($agent_openid)
        {
            Notify::getInstance()->notifyAgent($agent_openid, option('config.site_url'), '有新订单啦，请尽快登录代理平台发货', $order['order_no'], '登录代理平台查看', $order['total'], date('Y/m/d H:i:s', $order['add_time']), '平台地址：' . option('config.site_url') . '，扫码登录即可。');
        }
    }
