...
class present_product_model extends base_model
{
    ...
    public function getProductListByPid($pid)
    {
        ...
        $sql = "SELECT p.product_id, p.name, p.quantity, p.price, p.market_price, p.image, p.has_property FROM `" . $db_prefix . "present_product` AS pp LEFT JOIN `" . $db_prefix . "product` AS p ON `PP`.`product_id` = `p`.`product_id` WHERE `pp`.`pid` = '" . $pid . "' AND p.status = '1' AND p.quantity > 0";
