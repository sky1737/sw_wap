...
class financial_record_model extends base_model
{
	...
	{
		return $this->db->field('*,CONCAT("' . option('config.orderid_prefix') . '", order_no) AS order_no')
	...
	public function getRecordsByType($store_id, $type, $offset, $limit, $order = 'fr.id DESC')
	{
		$sql = "SELECT fr.*,fo.total,(fo.total + fr.income) AS commission FROM " . option('system.DB_PREFIX') .
			"financial_record fr, " . option('system.DB_PREFIX') .
			"fx_order fo WHERE fr.order_id = fo.order_id AND fr.store_id = '" . $store_id . "' AND fr.type = '" .
			$type . "' AND fr.income < 0";
		$sql .= ' ORDER BY ' . $order;
	...
	public function getProfitRecords($where, $offset, $limit, $order = 'id DESC')
	{
	...
	/*public function getRecordsByType($store_id, $type, $offset, $limit, $order = 'id DESC')
	{
	...
	{
		$sql = "SELECT COUNT(fr.id) AS count FROM " . option('system.DB_PREFIX') . "financial_record fr, " .
			option('system.DB_PREFIX') . "fx_order fo WHERE fr.order_id = fo.order_id AND fr.store_id = '" . $store_id .
			"' AND fr.type = '" . $type . "'  AND fr.income < 0";
	...
	{
		$sql = "SELECT store_id, SUM(income) AS income, order_id, SUM(profit) AS profit FROM " .
			option('system.DB_PREFIX') . "financial_record WHERE user_order_id = '" . $user_order_id .
			"' GROUP BY store_id";
	...
	public function sns($where = array(), $limit = 5, $offset = 0)
	{
		...
		$sql = "SELECT fr.add_time, fr.profit, s.* FROM " . $db_prefix . "financial_record AS fr, " . $db_prefix .
			"store AS s WHERE s.store_id = fr.store_id AND fr.profit > 0";
		if(!empty($where)) {
		...
		$sql .= " ORDER BY fr.id DESC LIMIT " . $offset . "," . $limit;
