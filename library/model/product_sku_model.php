...
//                }
        ...
        if (count($sku_ids) > 0)
            $this->db->where("`product_id` = {$product_id} AND `sku_id` NOT IN (" . join(',', $sku_ids) . ")")
                ->delete();
    ...
    public function fxEdit($product_id, $skus, $unified_price_setting = 0)
    {
        ...
        $product_info =
            M('Product')->get(array('product_id' => $product_id, 'store_id' => $this->store_session['store_id']),
                'source_product_id');
        foreach ($skus as $sku) {
            if (empty($product_info['source_product_id']) && !empty($unified_price_setting)) {
                $data = array(
                    'drp_level_1_cost_price' => !empty($sku['drp_level_1_cost_price']) ? $sku['drp_level_1_cost_price']
                        : 0,
                    'drp_level_2_cost_price' => !empty($sku['drp_level_2_cost_price']) ? $sku['drp_level_2_cost_price']
                        : 0,
                    'drp_level_3_cost_price' => !empty($sku['drp_level_3_cost_price']) ? $sku['drp_level_3_cost_price']
                        : 0,
    ...
    public function getRandSku($product_id)
    {
        ...
        if (count($properties) == 1) {
            ...
            $findPropertiesArr =
                D('Product_property')->field('`pid`,`name`')->where(array('pid' => array('in', $properties)))->select();
            $findPropertiesValueArr =
