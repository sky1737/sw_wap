...
class user_coupon_model extends base_model
{
	...
	function getPeopleCountByCoupon($coupon_id)
	{
		...
		$sql = "SELECT count(DISTINCT uid) as counts,coupon_id  FROM  `" . $db_prefix .
			"user_coupon` WHERE `coupon_id` IN (" . $where . ") GROUP BY `coupon_id`  ";
	...
	public function getListByProductId($product_id_arr, $store_id, $uid)
	{
		...
		$sql = "SELECT distinct u.* FROM `" . $db_prefix . "user_coupon` AS u LEFT JOIN `" . $db_prefix .
			"coupon_to_product` AS cp ON `u`.`coupon_id` = `cp`.`coupon_id` WHERE `u`.`store_id` = '" . $store_id .
			"' AND (`u`.`is_all_product` = 0 or `cp`.`product_id` in (" . join(',', $tmp_product_id_arr) .
			")) AND `u`.`uid` = '" . $uid . "' AND `u`.`start_time` <= '" . $time . "' AND `u`.`end_time` >= '" .
			$time . "' AND `u`.`is_use` = '0' AND `u`.is_valid = '1' AND `u`.`delete_flg` = '0'";
