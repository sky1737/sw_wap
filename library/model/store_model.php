...
class store_model extends base_model
{
	...
	public function goodExcellentList($offset = "0", $limit = "10", $has_other_info = false)
	{
		...
		if($has_other_info) {
			...
			$store_list = $this->db->table("Store as s")->join('Store_contact sc  ON  s.store_id=sc.store_id', 'LEFT')
				->join('Sale_category ss ON ss.cat_id = s.sale_category_id', 'left')
				->where($where)
				->limit($offset . ',' . $limit)
				->order($order)
		...
		if(!empty($store_list)) {
			foreach ($store_list as &$value) {
				$value['url'] = 'http://' . $value['store_id'] . option('config.site_domain') . '/wap';
				// option('config.wap_site_url') . '/home.php?id=' . $value['store_id'] . '&platform=1';
				$value['pcurl'] = 'http://' . $value['store_id'] . option('config.site_domain');
				// url_rewrite('store:index', array('id' => $value['store_id']));
				/*if ($has_other_info) {
				...
				//店铺二维码
				$value['wx_image'] =
					option('config.site_url') . '/source/qrcode.php?type=home&id=' . $value['store_id'];
				// 'http://' . $value['store_id'] . option('config.site_domain') .'/source/qrcode.php?type=home&id=' . $value['store_id'];
			}
		}
	...
	public function getStoresByUid($uid, $status = 1)
	{
		...
		if(option('config.user_store_num_limit') > 0 && option('config.user_store_num_limit') <= 15) {
			$return['store_list'] =
				$this->db->where($where)->limit('0,' . option('config.user_store_num_limit'))->order($orderby)
					->select();
			$return['page'] = '';
		}
		else {
			...
			$return['store_list'] =
				$this->db->where($where)->limit($p->firstRow . ',' . $p->listRows)->order($orderby)->select();
			$return['page'] = $p->show();
		}
	...
	public function getStoreCountByUid($uid, $status = 1)
	{
		if($status == 1 || $status == 0) {
			$where = array('uid' => $uid, 'status' => $status);
		}
	...
	{
		if($store_id) {
			return $this->db->field('store_id,name')->where(array('store_id' => $store_id, 'uid' => $uid,
			                                                      'status'   => 1))->select();
		}
		else {
	...
	public function getWeidianStoreListBySaleCategoryId($sale_category_id, $limit, $is_fid = false, $is_approve = '')
	{
		...
		$store_list = $this->db->field('`store_id`,`name`,`logo`')->where($condition_store)->order('`store_id` DESC')
			->limit($limit)->select();
	...
	{
		$admin_user = D('User')->field('uid')->where(array('status' => 1, 'admin_id' => array('>', 0)))->find('uid');
		if(!$admin_user) {
			return false;
		}
		$store = $this->db->where(array('uid' => $admin_user['uid'], 'status' => 1))->find();
		if(!empty($store)) {
	...
	public function getStore($store_id)
	{
		...
		if(!empty($store)) {
			$store['url'] = 'http://' . $store_id . '.' . option('config.site_domain') .
				'/wap/'; // home.php?id=' . $store['store_id'];
			...
			$store['ucenter_url'] = 'http://' . $store_id . '.' . option('config.site_domain') .
				'/wap/ucenter.php'; // ?id=' . $store['store_id'];
...
//						redirect($store['oauth_url'] . '?return_url=' . $return_url . '&store_id=' . $store_id .
//							'&token=' . $store['token']);
//					}
//					else {
//						redirect($store['oauth_url'] . '&return_url=' . $return_url . '&store_id=' . $store_id .
...
////							if (!empty($wx_bind) && $wx_bind['service_type_info'] == 2 &&
////								$wx_bind['verify_type_info'] == 0
////							) {
////								$oauthUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' .
////									$wx_bind['authorizer_appid'] . '&redirect_uri=' . urlencode($customeUrl) .
////									'&response_type=code&scope=snsapi_userinfo&state=' . $_SESSION['weixin']['state'] .
////									'&component_appid=' . option('config.wx_appid') . '#wechat_redirect';
////							}
////							else {
////								//店铺非认证服务号走总后台授权
////								$oauthUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' .
////									option('config.wechat_appid') . '&redirect_uri=' . urlencode($customeUrl) .
////									'&response_type=code&scope=snsapi_userinfo&state=' . $_SESSION['weixin']['state'] .
////									'#wechat_redirect';
////							}
////							redirect($oauthUrl);
...
////							($_GET['state'] == $_SESSION['weixin']['state'])
////						) {
...
////								$tokenUrl = 'https://api.weixin.qq.com/sns/oauth2/component/access_token?appid=' .
////									$wx_bind['authorizer_appid'] . '&code=' . $_GET['code'] .
////									'&grant_type=authorization_code&component_appid=' . option('config.wx_appid') .
////									'&component_access_token=' . $component_token;
////							}
////							else {
////								$tokenUrl = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' .
////									option('config.wechat_appid') . '&secret=' . option('config.wechat_appsecret') .
...
////								$url =
////									'https://api.weixin.qq.com/sns/userinfo?access_token=' . $jsonrt['access_token'] .
////									'&openid=' . $jsonrt['openid'] . '&lang=zh_CN';
...
//			}
			...
			$store['url'] = 'http://' . $store['store_id'] . '.' . option('site_domain');
			//option('config.wap_site_url') . '/home.php?id=' . $store['store_id'];
			if(empty($store['logo'])) {
				$store['logo'] = getAttachmentUrl('images/default_shop_2.jpg', false);
			}
			...
			$store['ucenter_url'] = $store['url'] . '/ucenter.php?id=' . $store['store_id'];
			$store['physical_url'] = $store['url'] . '/physical.php?id=' . $store['store_id'];
	...
	public function getStoreByUid($uid)
	{
		...
		if(!empty($store)) {
			$store['url'] = 'http://' . option('config.site_domain') . '/?twid=' . $store['uid']; // home.php?id=' .
	...
	{
		$store =
			$this->db->field('sale_category_id,sale_category_fid')->where(array('store_id' => $store_id, 'uid' => $uid))
				->find();
...
//	}
	...
	public function setAd($store_id, $data)
	{
		return $this->db
			...
			->data(array('has_ad'       => $data['has_ad'],
			             'ad_position'  => $data['ad_position'],
			             'use_ad_pages' => $data['use_ad_pages'],
...
//	{
//		$store = $this->db->field('1')->where(array('store_id' => $store_id, 'drp_approve' => 1))->find();
//
...
//	}
	 ...
	 */
	public function getStoreByRoundDistance($where, $limit = "10", $offset = "0", $order = "", $sort = "asc",
		$get_distance = "", $is_distance_order = false)
	{
		...
		if(empty($WebUserInfo['long'])) {
			$store_list =
				D('')->table("Store as s")->where($where)->limit($offset . ',' . $limit)->order($orders)->select();
		}
		else {
			if($get_distance) {//指定范围内的降序
				...
				$store_list =
					$this->nearshops($WebUserInfo['long'], $WebUserInfo['lat'], $order, $sort, $offset, $limit,
						$get_distance, $where);
			}
			else {    //纯地理降序
				if($is_distance_order) {
					...
					$store_list =
						D('')->table("Store as s")->where($where)->limit($offset . ',' . $limit)->order($order)
							->select();
				}
			}
		}
	 ...
	 * @param: 补充搜索条件
	 * */
	public function nearshops($long, $lat, $order, $sort = "asc", $offset = "0", $limit = "10", $get_distance = "",
		$wheres = "")
	{
		...
		$sql =
			"select * from (select `s`.`store_id`, `s`.`name`, `s`.`logo`, `s`.`intro`,`sc`.`long`,`sc`.`lat`, ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli from " .
			$db_prefix . "store_contact as sc, " . $db_prefix . "store as s where " . $wheres .
			" s.status = 1 and sc.store_id = s.store_id " . $where . $orders . " ) as c " . $julis . $order_juli .
			"  limit {$offset},{$limit}";
	...
	public function nearshop_count($long, $lat, $get_distance = "", $wheres = "")
	{
		...
		$sql =
			"select count(c.juli) as count from (select ROUND(6378.138 * 2 * ASIN(SQRT(POW(SIN(({$lat}*PI()/180-`sc`.`lat`*PI()/180)/2),2)+COS({$lat}*PI()/180)*COS(`sc`.`lat`*PI()/180)*POW(SIN(({$long}*PI()/180-`sc`.`long`*PI()/180)/2),2)))*1000) AS juli from " .
			$db_prefix . "store_contact as sc, " . $db_prefix . "store as s where " . $wheres .
			" s.status = 1 and sc.store_id = s.store_id ) as c " . $julis;
	...
	public function getAgentId($nowOrder, $address)
	{
		return $this->db
